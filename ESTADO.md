...
*Última actualización: martes 8 septiembre 2026 — 🚀 **RE-POSTULACIÓN A PRODUCCIÓN ENVIADA el 8 sep 9:28, dos días antes de lo previsto.** Google tachó los **3 requisitos** (versión cerrada, 12 testers, 14 días). La respaldó la **medición del 7 sep: 13 activos de 23 con cuenta** (`snnunez1` se sumó; `sfaille` es el único sin cuenta). Las respuestas del formulario se actualizaron con evidencia de esta ronda: los 13 activos, los bugs corregidos y el onboarding en Android. **Respuesta por correo en ≤7 días, pero la señal se busca en Play Console.** 🔴 **Mientras revisan: no se toca el canal cerrado ni se sube versión nueva a Play** (el código y `huella.lat` siguen libres). 🟢 **La cola priorizada de 5 piezas quedó DESBLOQUEADA, y las DOS PRIMERAS ya están CERRADAS CON QA el mismo día:** la **barra de edad del Cerebro** (`7104044` + `bd8b989` + `af0d611`, feedback de Diego) y la **micro-respuesta al "¿cómo te sentiste tú?"** (`d3edf44` + `b06d2ae`, con Haiku, un disparo por episodio y marco anti-vergüenza). Sigue la pieza 3, el motor de rasgos alimentado también por hitos. Quedan por delante la entrada única con IA y "qué puedes hacer en esta etapa". ✅ **EL TAP AL CEREBRO EN ANDROID QUEDÓ CERRADO CON QA DE DANIEL** (`7cdd878`), después de dos semanas abierto. 🟡 **Pieza 1 de la cola HECHA Y DESPLEGADA el mismo día**: el marcador con la foto del hijo en la barra de edad del Cerebro (`7104044` + `bd8b989` + `af0d611`), pendiente solo del QA final del tick. **Contexto del hallazgo:** 🟡 **EL TAP AL CEREBRO EN ANDROID: CAUSA RAÍZ ENCONTRADA, ARREGLADA Y DESPLEGADA (`7cdd878`) — PENDIENTE EL QA DE DANIEL, así que NO está cerrado.** La tarjeta sí abría: el `click` sintético que Chrome Android fabrica del mismo tap caía sobre el velo recién montado y la cerraba en el mismo gesto. Se suprime ese click en el canvas (`preventDefault` en `touchend`) y el umbral de tap sube de 9px a 12px. **La descripción vieja del síntoma quedó corregida** (no es que "se ilumina y no abre": no pasa nada, porque abre y cierra al instante), y `d46366c` quedó descartado como sospechoso. Los **6 pasos del QA** están en el bloque del 6 sep. ✅ **ONBOARDING EN ANDROID: QA APROBADO**, con dos ajustes al acto C ya en producción (`d30db3c`: título nuevo con el nombre del hijo y marca de agua reposicionada). **Contexto del 4 sep:** 🐛 **BUG CERRADO CON QA EN PC Y ANDROID REAL: los overlays `position: fixed` (el bottom sheet de la rutina y 8 más) quedaban DEBAJO de la barra de navegación en Android.** Causa raíz: las tres clases de entrada de `PageTransition` usan `animation-fill-mode: both`, y Chrome deja al wrapper como stacking context (y containing block, en las que deslizan) para siempre. Fix en la raíz (commit `d7ce582`): al terminar la animación la clase animada se reemplaza por `pageLlegado`, sin animation ni transform. Cubre a los 9 overlays sin portal de una vez. 🔎 De paso se verificaron y descartaron dos tareas con contexto viejo ("+ Registrar" en Logros y la tuerca de Estrategias: ninguna existe desde el 15 ago) y se investigó el "Marco aplicado" de los episodios: **no era un bug, se quitó a propósito el 27 ago**. 🧭 **Daniel decidió que el marco VUELVE, por el camino determinístico, y quedó implementado el mismo día** (`marcoDelEpisodio` + `PieCientifico` con etiqueta "Lente", en el post-guardado y en el Historial; sin IA, sin columna), commit `c801cf1`, **QA de Daniel aprobado en claro, oscuro y Android**. 📊 **Medición del 4 sep: 12 activos de 22 con cuenta** (el 2 sep eran 6); 7 sin actividad desde el 27 ago, 3 con hijo y cero registros, 2 nunca registrados. **La ventana cierra ~10 sep.** **Contexto que sigue vigente del 3 sep:** 🎨 **EL ONBOARDING QUEDÓ COMPLETO: los cinco bloques (1-5) están en producción.** Hoy se cerraron los tres que faltaban: **bloque 3** (el texto del acto B es el **primer episodio real** del hijo, con extracción y orientación en segundo plano y el Home refrescándose solo), **acto C** (pantalla de cierre con 6 autores elegidos por la edad del hijo, la línea "más de 80 expertos" calculada desde el corpus, un único CTA "Entrar a Huella", y el persistor corriendo detrás mientras el padre la lee) y la **pasada visual completa según los mockups de Claude Design** (marca de agua del escarabajo por pantalla, progreso en 5 segmentos, campos de 56px, slots de foto strawberry, tokens nuevos). De paso se contó el corpus científico real: **82 autores e investigadores distintos** (`AUTORES_CORPUS`), no los 23 del banco de la Acción Rápida. ✅ **Migración 015 (`episodios.origen`) corrida y verificada por Daniel en Supabase.** ✅ **QA de Daniel: onboarding completo aprobado en PC, claro y oscuro.** ⬜ **Falta la pasada en Android real** (que A5 con dos slots y el acto C quepan sin scroll en 390px) **y una pasada con cuenta nueva de verdad** (que el primer episodio llegue al Home con tipo y orientación, y que las dos fotos se guarden). Ya no hay "Saltar" en el acto A; el "Saltar este paso" del acto B se queda a propósito. **Próximo:** viernes 4 medición de testers; después, la cola del Cerebro (bug del tap en Android, QA tablet Samsung, enlace del paso 8).*
...
## Cerrado HOY — martes 8 septiembre 2026 — **El tap al Cerebro quedó CERRADO con QA en Android, la re-postulación salió, y la pieza 1 de la cola ya está en producción**
