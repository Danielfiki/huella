...
- **`api/push-test.js` es TEMPORAL — ELIMINAR ANTES DE LA BETA** — desde **6 jul 2026** — endpoint de prueba que dispara una push con el secreto en la URL (`GET ?secret=CRON_SECRET`, destino hardcodeado al owner). Ya cumplió (verificó el push end-to-end). Borrarlo antes de abrir la beta.
- **ROTAR `CRON_SECRET` de nuevo + redeploy antes de la beta** — desde **6 jul 2026** — su valor nuevo quedó **expuesto en el chat de Daniel**. Generar uno nuevo en Vercel y redeployar.
- **Control permanente de Notificaciones en la pantalla Cuenta — PENDIENTE (fix (a) aprobado por Daniel)** — desde **6 jul 2026** — hoy el `NotifBanner` es la ÚNICA puerta para activar push y su **X descarta para SIEMPRE** (`localStorage` `huella_notif_banner_dismissed`, sin expiración) → un descarte accidental deja al usuario **sin push y sin forma de reactivar desde la UI** (en iPhone no hay consola para limpiar el flag; hoy solo se arregla reinstalando la PWA). Agregar un botón/toggle "Notificaciones" en Cuenta que llame a `requestPermission()` del hook. **Prioridad para la beta.**
...
**App stores — 🟡 EN EJECUCIÓN · TWA por Bubblewrap · BLOQUEANTE DE LA BETA** (arrancó **2 jul 2026**; etapas 1, 2 y 3 CERRADAS, etapa 4 EN CURSO — cuenta Google Play creada **4 jul 2026**, esperando verificaciones de Google)
...
- **Etapa 4 — EN CURSO · CUENTA GOOGLE PLAY CREADA (4 jul 2026), esperando verificaciones de Google:**
  ...
  - **Verificaciones pendientes DE GOOGLE** (bloquean el botón **Crear aplicación**):
    1. **Identidad:** documentos **ENVIADOS (4 jul), en revisión** — puede tomar días; el resultado llega por correo a `contacto@huella.lat`.
    2. **Acceso a dispositivo Android:** **pendiente de Daniel** — pedir un Android prestado, instalar la app **Google Play Console** e iniciar sesión con `contacto@huella.lat` (5 min, **no vincula el teléfono**).
    3. **Teléfono de contacto:** se **desbloquea automáticamente** tras aprobarse la identidad.
  - **PRÓXIMA SESIÓN (cuando las verificaciones pasen):** crear la app en Play Console (nombre **Huella**, español, tipo **Aplicación**, **GRATIS** — irreversible y correcto porque Pro se cobra vía web/Mercado Pago), armar la ficha de la store (textos + capturas), subir **`app-release-bundle.aab`**, **copiar el SHA-256 de Play App Signing (App Integrity) al `assetlinks.json`** reemplazando `PENDIENTE_FINGERPRINT_BUBBLEWRAP` + redeploy, y **enviar a revisión**. **OJO CRÍTICO:** el fingerprint correcto es el de **Play App Signing**, NO el del keystore local — Google re-firma con su propia clave. **Nota:** la **barra de navegador visible en la TWA es esperada** hasta que ese reemplazo del fingerprint esté hecho y verificado.
