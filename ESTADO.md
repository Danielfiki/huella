...
#### MOTOR DE RASGOS (piezas 1, 2, 3 EN MAIN; pieza 4 EN CURSO — 4A + 4B hechas, faltan 4C/4D)
...
- **PRÓXIMO PASO — FASE B del pendiente obligatorio:** **afinar el `PROMPT_DETECTAR_RASGOS` para equilibrar las 4 familias** con datos reales de la beta (calibrar para que no se sesgue, ej. demasiadas `fortalezas` vs. `mueve`/`calma`).
- **PRÓXIMOS PASOS (resto de la pieza 4):**
  - **4C (las 4 familias):** **reemplazar las 3 cards actuales** (episodios/logros/avances) por los **rasgos confirmados** agrupados en las **4 familias** (mueve/fortalezas/cuesta/calma en orden fijo).
