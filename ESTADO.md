...
3. ✅ **HECHO (29 ago) — se reclutaron 5 testers nuevos y ya están adentro.** Entraron al grupo de Google, recibieron código y mensaje de bienvenida. La lista oficial pasó de 18 a **20 personas**. Detalle en el bloque de cierre del 29 ago.
...
1. **Medir la actividad en la ventana 2-4 sep** (día ~7 del reloj), con las queries de métricas cruzadas contra la **LISTA OFICIAL DE TESTERS** de la sección de abajo (esa tabla manda: son **20 personas**, y si un número no cuadra, manda la tabla). Antes del 2 sep el dato no sirve: son pocos días y cae en fin de semana.
...
**Son 20 personas (actualizado el 29 ago 2026).** La cuenta: eran **18** al 11 ago, **− 3 que nunca respondieron** (`be.delacerda`, `snackclub2022`, `nicolas.arriagada` — sus códigos `H-01`, `H-06` y `H-09` se reciclaron el 29 ago tras verificar por query que seguían sin canjear), **+ 5 testers nuevos** = **20**.
...
| 16 | *(falta el nombre)* | `krishna.baquedano77@gmail.com` | **H-06** | 🆕 nueva (29 ago) — código reciclado |
| 17 | *(falta el nombre)* | `mariaadrianaq@gmail.com` | **H-01** | 🆕 nueva (29 ago) — código reciclado · ⚠️ incidencia de acceso abierta |
| 18 | *(falta el nombre)* | `calomose@gmail.com` | **H-09** | 🆕 nueva (29 ago) — código reciclado |
| 19 | *(falta el nombre)* | `conny.aguero@gmail.com` | **H-12** | 🆕 nueva (29 ago) |
| 20 | *(falta el nombre)* | `snnunez1@gmail.com` | **H-26** | 🆕 nueva (29 ago) — primera del lote 2 |

*Notas de la tabla: **(a)** la columna de código trae solo los **confirmados** (los 4 que no canjearon y los 5 nuevos); el resto de la asignación vive en el tablero de `/beta`, que es estado local del navegador y **no** la base — por eso van en "—". **(b)** **Faltan los nombres de las 5 personas nuevas**: Daniel los tiene en WhatsApp, hay que vaciarlos acá. **(c)** Salieron de la lista `be.delacerda`, `snackclub2022` y `nicolas.arriagada` — nunca respondieron y sus códigos se reciclaron el 29 ago.*
...
**Frente 1 sigue su curso:** el reloj de los 14 días corre desde el 27 ago, la postulación apunta a **~10 sep**, y lo próximo es **la medición del 2-4 sep**. **El plazo del 31 ago del SDK se cumple hoy y se pasa, como estaba asumido** — desde ahora no se puede subir ninguna actualización nueva a Play hasta que la v4 llegue a producción. *(Recordatorio útil: eso NO bloquea arreglos web, que es como llegó el fix del arranque.)*
