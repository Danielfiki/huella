...
- ⬜ **¿MARCO APLICADO DE VUELTA EN LOS EPISODIOS?** — desde **4 sep 2026** — hoy NO se muestra, a propósito desde `6cf1302` (27 ago): el prompt lo prohíbe y `OrientacionSecciones` lo filtra; nunca existió como columna. Si se quiere de vuelta, el camino recomendado es **derivarlo determinísticamente** (`accion_rapida_autor` + `accion_rapida_dimension`, o `inferirDimensionCentral` en cliente cuando no hay acción rápida) y pintarlo junto al descargo, sin pedírselo al modelo. Detalle en el bloque del 4 sep, punto 3.
...
- **Si Daniel quiere el marco de vuelta:** derivarlo **determinísticamente**, sin pedírselo al modelo: desde `accion_rapida_autor` + `accion_rapida_dimension` (ya validados contra `MAPA_DIMENSIONES`), o con `inferirDimensionCentral` en cliente cuando no hay acción rápida (el primer episodio del onboarding). Se pinta junto al descargo, como `PieCientifico` en patrones. Sin prompt nuevo, sin columna, sin migración. Advertencia honesta: ese autor guió la Acción Rápida, no necesariamente la orientación larga. **Queda como decisión en PENDIENTES.** Detalle menor: el comentario de `PieCientifico.jsx` todavía dice que en episodios la frase la escribe el modelo; ya no es así.

### 📊 Frente 1 — Google Play

La medición de testers del 4 sep la corrió Daniel directo en Supabase; **los resultados no pasaron por Code** y no están en este archivo. Si hace falta el número, la fuente es Daniel (y su Sheets).

### ⏭️ Próximo

1. **Decisión de Daniel:** marco en episodios (sí/no) y "+ Registrar hito" desde el filtro Logros del Historial (sí/no).
