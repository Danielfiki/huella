...
**HALLAZGO QUE APLICA A TODA LA APP: el viewBox del `Escarabajo` tiene muchísimo aire.** Medido, el dibujo ocupa **solo el 49.4% del ancho** del viewBox y el 68.2% del alto — tinta en `x[124.35, 356.20]`, `y[63.86, 357.17]` sobre `469.55 × 429.86`. **La caja del SVG NO es el tamaño del bicho.** En el disco del botón, una caja de 27px daba un escarabajo de 13px de ancho real, y por eso se veía tímido. Para que la tinta llegue a ~32px dentro del disco de 44 hay que darle una caja de **51×47**, que desborda el disco: lo que sobra es padding transparente y no se recorta. Esto explica el comentario que ya estaba en `Escarabajo.jsx` sobre que los badges necesitan "48-52px y el 90% del cuadro" — venían compensando lo mismo a mano. **Si algún día se ve chico en otra pantalla, la causa es esta y no el grosor del trazo.**

**El escarabajo está descentrado 1.17% a la derecha en su propio viewBox.** El eje de simetría del dibujo está en `x=240.27` y el centro del viewBox en `234.78` (la tinta además está 1.03% hacia arriba). Se corrige con un `translate` en porcentajes dentro del disco — **nunca editando el SVG, que lo comparten 19 pantallas.**

**Lo que NO es:** el dibujo **no tiene inclinación**. Se verificaron los 6 pares espejo de patas y antenas (todos dan eje 240.27–240.285), que los dos élitros son la misma secuencia de números en orden inverso espejada sobre ese eje, y que las matrices de las elipses son mirrors exactos. Tampoco hay `transform` heredado que rote, y el latido del disco es puro `scale`. **La única rotación del botón es la de la marca de agua** (`ctaDeriva`, hasta -6deg), que es por spec.
...
- **ABIERTA: la inclinación del escarabajo del disco.** Daniel lo vio levemente inclinado a la derecha en el QA. Se midió el asset y **no hay rotación** (ver arriba); lo único real es el descentrado del 1.17%, que ya se corrigió pero a ese tamaño son 0.6px, o sea que no explica el síntoma. Daniel re-evalúa en producción ahora que el escarabajo está al doble de tamaño: si la inclinación persiste en el disco, se retoma. Los dos candidatos que sí rotan de verdad son la marca de agua (-6deg, por spec) y el pantallazo.
