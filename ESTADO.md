...
**App stores — 🟡 EN EJECUCIÓN · TWA por Bubblewrap · BLOQUEANTE DE LA BETA** (arrancó **2 jul 2026**; etapas 1 y 2 CERRADAS, etapa 3 EN CURSO)
...
- **Etapa 3 — EN CURSO (2 jul 2026):** `bubblewrap init` **HECHO** por Daniel en `C:\Users\dundu\OneDrive\Desktop\huella-twa` (carpeta hermana, FUERA del repo git de la web): proyecto Android generado + keystore creado (alias `huella`, packageId `lat.huella.app`, host `www.huella.lat`, orientación **portrait**, **notification delegation SÍ**). **PRÓXIMO PASO INMEDIATO:** Daniel corre **`bubblewrap build`** en su terminal (interactivo, pide las contraseñas del keystore) → genera `app-release-bundle.aab` (lo que sube a Play) y `app-release-signed.apk` → **instalar el APK en un teléfono para QA de la TWA**.
- **Etapa 4 — PENDIENTE:** crear cuenta Google Play (**US$25 pago único**, la crea Daniel) + subir el `.aab` + **copiar el SHA-256 de Play App Signing desde Play Console (sección App Integrity) al `assetlinks.json`** reemplazando `PENDIENTE_FINGERPRINT_BUBBLEWRAP` + redeploy. **OJO CRÍTICO:** el fingerprint correcto es el de **Play Console (Play App Signing)**, NO el del keystore local — Google re-firma la app con su propia clave.
- **Recordatorio de seguridad:** respaldar `android.keystore` + sus contraseñas en el Drive de Daniel (fuera del repo). Si se pierden, no se pueden firmar futuras actualizaciones de la app con la misma clave.
