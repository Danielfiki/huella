...
- ⬜ **ONBOARDING — PASADA CON CUENTA NUEVA DE VERDAD** — desde **3 sep 2026** — el bloque 3 nunca se vio con una cuenta real: verificar que el **primer episodio llega al Home** (primero como "Otro", luego con su tipo real, luego con la orientación, sin recargar), que la fila queda con `origen = 'onboarding'`, y que **las dos fotos** (hijo y cuidador) quedan guardadas en `hijos.avatar_url` y `perfiles.avatar_url`.

**Decisiones abiertas de Daniel (sin código hasta que decida)**
- ✅ **MARCO DE VUELTA EN LOS EPISODIOS — CERRADO CON QA el 4 sep 2026** (commit `c801cf1`, camino determinístico, `marcoDelEpisodio` + `PieCientifico` con etiqueta "Lente"). **QA de Daniel aprobado en claro, oscuro y Android** (episodio nuevo y uno anterior al 27 ago). Detalle en el bloque del 4 sep, punto 3.
- ⬜ **¿"+ REGISTRAR" DIRECTO A HITO DESDE EL FILTRO "LOGROS" DEL HISTORIAL?** — desde **4 sep 2026** — la pantalla Logros ya no existe (15 ago). `NuevoPage` sigue entendiendo `location.state.vistaInicial === 'hito'`, así que cablearlo es trivial; lo que falta es decidir si se quiere y dónde.
...
2. 🟢 **MICRO-RESPUESTA AL "¿CÓMO TE SENTISTE TÚ?" — RESPUESTA + MEMORIA, CERRADA CON QA el 8 sep 2026** (`d3edf44` + `b06d2ae` + `e11d82f` + `9ca50fc`): al guardar, aparecen 1 o 2 frases que acompañan lo que el padre escribió, con Haiku, un solo disparo por episodio y marco anti-vergüenza; se ven también al reabrir el episodio. **Con memoria (2b):** el prompt lleva sus 5 reflexiones anteriores —filtradas por autor, nunca las de la pareja— y puede **nombrar el hilo** cuando lo hay. **La capa (b), el resumen semanal, sigue abierta** y se movió al **retrato del padre** (pieza 3.5). ⬜ **Calibrar con 10 respuestas reales de testers antes de tocar el prompt de nuevo** — ya van tres ajustes de tono contra casos únicos. 👀 **Patrones a vigilar: la muletilla "especialmente cuando" y cualquier lectura de deterioro.** Detalle en el bloque del 8 sep. — **Síntoma original: el padre escribía cómo se sintió y no recibía absolutamente nada de vuelta.** Es el único campo de la app que es **solo suyo**, y hoy cae al vacío. Va en **dos capas**:
   - **(a) AHORA — la respuesta del momento.** La IA devuelve **1-2 líneas** apenas termina de escribir: **valida sin juzgar** y ofrece **UNA sola perspectiva**, nunca una lista de consejos. **Marco anti-vergüenza, y es la regla dura de esta pieza:** el padre acaba de admitir algo incómodo (que gritó, que se le acabó la paciencia); si la respuesta lo corrige, lo felicita o lo aconseja, deja de escribir para siempre. Acompañar, no evaluar.
   - **(b) DESPUÉS — pieza propia, con su plan.** Acumular esas reflexiones en un resumen tipo **"tu semana como papá/mamá"**. 🪝 **Esto es gancho de retención**, del mismo orden que la Fase 3 de las notificaciones nobles: le da al padre un motivo propio para volver, separado del registro del hijo. No se diseña junto con (a): primero que (a) exista y se vea si el padre escribe más.

3. ❌ **MOTOR DE RASGOS — LOS HITOS TAMBIÉN DEBEN ALIMENTARLO — SALE DE LA COLA el 9 sep 2026: LA PREMISA ERA FALSA.** La pieza decía *"hoy el motor lee solo episodios difíciles"*. **No es cierto, y no lo fue nunca.** El motor lee episodios **y** hitos desde que se escribió: el prompt marca cada momento con su `origen` y le pide explícitamente al modelo que las familias positivas (`mueve`, `fortalezas`, `calma`) se nutran sobre todo de los avances (`anthropic.js`, `PROMPT_DETECTAR_RASGOS`); los dos enganches del contexto cuentan **episodios + hitos juntos** para el gatillo, y la evidencia guardada distingue el tipo de cada momento. **Medido en producción el 9 sep: 28 de 70 rasgos tienen evidencia de hito.** No había nada que construir.

   🔴 **LA CAUSA DE LOS 0 CONFIRMADOS ERA UN BUG: LA CARD DE CANDIDATO NUNCA SE RENDERIZÓ, A NADIE.** De 70 rasgos en la tabla, **61 son emergentes, 9 candidatos y CERO confirmados** — tampoco hay descartados. **No era visibilidad ni volumen: era una carrera de efectos en `HijoPage.jsx`.** Había **dos** efectos, uno que fijaba el candidato y otro que lo reiniciaba a null al cambiar de hijo activo. Cuando terminaba la carga, `hijo` pasaba de null al hijo real y eso disparaba **los dos en el mismo commit**, en orden de declaración: el primero fijaba, el segundo borraba. El valor neto iba **de null a null**, así que React no volvía a renderizar y el primero no corría nunca más, porque ninguna de sus dependencias había cambiado. **`rasgoPropuesto` quedaba en null para siempre.**

   **Reproducido en Chrome con harness** (los dos efectos copiados literales y los datos reales de Pipa, 2 candidatos con `evidencia_count` 4 y 2 emergentes): caso real → `SIN CARD`, traza `B: reinicia a null | A: fija C1 | B: reinicia a null`. Control sin el efecto de reinicio → `CARD VISIBLE`. **Descartadas con evidencia:** la consulta no filtra por estado (`HuellaContext.jsx:570` y `:634` piden `select('*')`), la RLS lee bien (el badge del Home se enciende leyendo el mismo arreglo), la condición no exige nada más que `estado === 'candidato'`, y la card es el primer elemento del tab. **Por qué el badge sí y la card no:** el badge lee `state.rasgos` **directo en el render** (`PanelPage.jsx:160-168`); la card la leía a través de un `useState` intermedio, que es lo que se rompía.

   ✅ **ARREGLADO el 9 sep 2026:** un solo efecto, con el candidato fijado guardado **junto al `hijoId`** al que pertenece (`{ hijoId, rasgo }`). Conserva las dos reglas de diseño: uno por visita —si ya hay uno fijado para ese hijo no se reemplaza, así al resolverlo no salta el siguiente— y al cambiar de hijo el id guardado deja de coincidir y se busca de nuevo.

   **El volumen es un factor secundario, no la causa.** Un rasgo llega a `candidato` con **3 momentos** que compartan patrón, lo que pide del orden de **10 momentos**. Pascual tiene **13 momentos**, y hay **9 candidatos** vivos entre todos los hijos: la detección llegó. Lo que fallaba estaba después.

   🪝 **Adónde se mueve el trabajo:** el volumen se apoya en la **pieza 5 (entrada única)** y la **pieza 7 (recordar sin reclamar)**, ya en la cola. La **pieza 3.5 queda reducida** a lo que sobra después del fix.

   ✅ **Lo que sí salió de este diagnóstico, en dos pasos chicos:**
   - **Gatillo robusto** (`HuellaContext.jsx`). El motor corría con `total % 5 === 0`: si el conteo brincaba de 4 a 6 —un borrado, o momentos registrados desde otro dispositivo con el estado local viejo— **ese tramo no corría nunca**. Ahora compara contra el último total analizado, guardado por hijo en `localStorage`, y corre cuando el hijo **cruza** un tramo de 5. El marcador se pone antes de disparar, para que dos guardados seguidos no llamen dos veces a la IA.
   - **Migración 017** (`017_rasgos_estado_emergente.sql`). El cuarto estado `emergente` se agregó a la base viva **a mano** durante la Capa 1 y nunca quedó como archivo: `006_rasgos.sql` todavía declara el CHECK viejo de tres estados. **Ya está aplicado en producción** (hay 61 filas emergentes), así que el archivo solo alinea los archivos con la base; si alguna vez se reconstruyera desde cero, los emergentes fallarían al insertar en silencio.

   ⚠️ **Deuda que quedó anotada:** el primer episodio del onboarding **no pasa por `addEpisodio`** (`onboardingPersistor.js` inserta directo, y está documentado ahí el porqué), así que **no dispara el motor** y descuadra el conteo de tramos en uno. No se tocó.

3.5. ⏸️ **VISIBILIDAD POST-FIX — a decidir DESPUÉS de ver cómo responden los testers** — desde **9 sep 2026**. **Nació como "confirmación visible", con la premisa de que la card estaba escondida. Esa premisa cayó:** la card no se veía porque **nunca se renderizó** (carrera de efectos, arreglada el mismo día). Con el fix en producción, el papá ya tiene dónde confirmar, así que **lo que queda es chico y no se decide a ciegas**:
   - **El badge "Algo nuevo" del Home se enciende con candidato O emergente** (`PanelPage.jsx:451`, `hayNovedad={rasgoCandidato || hayEmergente}`), pero la card **solo existe para candidatos**. Con 61 emergentes contra 9 candidatos, el badge promete algo que la pantalla casi siempre no puede mostrar. **Evaluar dejarlo solo con candidato real.**
   - **Evaluar subir la card al Home**, si aun con el fix los candidatos siguen sin resolverse.

   🔴 **Ninguna de las dos se toca hasta tener datos de testers reales con el fix arriba.** El aprendizaje de esta sesión es justamente ese: se rediseñó una pantalla sobre un síntoma que era un bug de código.

   📊 **FECHA DE LA MEDICIÓN: viernes 12 sep 2026, tres días después del fix.** Contar **cuántos de los 9 candidatos de testers se resolvieron** (confirmados o descartados) desde que la card se ve. **Ese número decide la pieza, no una opinión de diseño:** si se resolvieron varios, el tab Perfil basta y esto se cierra; si siguen en cero con la card funcionando, ahí sí hace falta subirla al Home. **Al medir, evaluar también el badge**, que hoy se enciende con emergentes y promete algo que la pantalla no puede mostrar.

4. ⬜ **RETRATO DEL PADRE** — desde **8 sep 2026**, nace de la pieza 2 — **va DESPUÉS del motor de rasgos y ANTES de la entrada única.** Las **recurrencias emocionales del padre**, por tipo de momento y en el tiempo: qué siente, con qué se repite, qué cambió. **Tres salidas:** la **micro-respuesta con memoria** (ya viva, pieza 2b), el **resumen semanal "tu semana como papá/mamá"** (🪝 retención, es la capa (b) de la pieza 2) y la **orientación del hijo adaptada a lo que a ESE padre le cuesta**. 🔒 **Aislado del motor de rasgos del hijo:** son dos retratos distintos y no deben mezclarse ni compartir tablas — lo que le pasa al padre no es un rasgo del niño.

5. ⬜ **ENTRADA ÚNICA CON CLASIFICACIÓN POR IA** — desde **8 sep 2026** — **sugerencia de Seba**. Hoy el padre tiene que **elegir primero** qué va a registrar (episodio, avance o patrón) y recién después escribir; eso le pide clasificar antes de contar. **Qué se hace:** una **sola caja por defecto — "Cuéntame qué pasó"**; la IA **propone** si eso es un episodio, un avance o un patrón, y **el padre confirma con un tap ANTES de guardar**. Los **3 caminos actuales siguen disponibles** para quien ya sabe qué quiere registrar: esto es el default, no un reemplazo. ♻️ **Reusa la tubería que el onboarding ya tiene probada en producción:** texto libre → INSERT con `tipo 'otro'` → `extraerEpisodio` en segundo plano (bloque 3 del onboarding, `onboardingPersistor.js`). 🎨 **Primero mockup con Claude Design**, después Code sobre el código real. 🔴 **REGLA DURA: nunca clasificación automática sin confirmar.** Un episodio mal clasificado **ensucia el motor de rasgos**, que solo lee episodios, y ese daño no se ve hasta mucho después.

6. ⬜ **CEREBRO — "QUÉ PUEDES HACER EN ESTA ETAPA"** — desde **8 sep 2026** — **feedback de Diego (`H-02`)**. **Síntoma en una frase: hoy el Cerebro explica muy bien qué madura y cuándo, pero no dice qué hacer con eso; debería cerrar con algo accionable.** **Qué se hace:** cada **tarjeta de zona** gana un bloque con **2-3 prácticas concretas calibradas por edad**, sacadas del corpus (no inventadas), y un **enlace a Estrategias** para quien quiera el plan completo. Cierra el salto entre entender y actuar, que es justo la costura que el principio del momento dice que no debe existir.

7. ⬜ **RECORDAR SIN RECLAMAR** — desde **8 sep 2026** — **feedback de Seba** (usuario con experiencia en apps), **confirmado por Daniel como usuario**. **Síntoma en una frase: el momento pasa, el padre se pierde en lo cotidiano y a la noche ya no registró; debería haber algo que lo traiga de vuelta sin hacerlo sentir mal.** 🪝 **Es el hueco de retención más grande que tiene la app hoy**, más que cualquier pieza de contenido.
   - 🔴 **REGLA DURA: nunca "no has registrado", nunca racha, nunca contador de días.** Todo eso convierte la app en una deuda, y una app de crianza que le cobra al padre lo que no hizo se desinstala. Es la misma línea del copy de re-enganche que ya se reescribió sin culpa.
   - **Tres caminos, en este orden:**
     1. **Una pregunta diaria a la hora que ELIJA el padre**, tipo *"¿Cómo estuvo hoy con {nombre}?"*, que **abre directo la caja de registro**. Depende de los **push reales**: es exactamente la **Fase 3, "la notificación noble"** del gancho de retención (pipeline Web Push ya ENCENDIDO y verificado end-to-end; hoy solo hay 3 reglas en `push-remind.js`). **Esta pieza es el contenido que le faltaba a esa fase.**
     2. **Registrar más corto.** 🔗 **La entrada única (pieza 5) es PARTE de esta solución, no algo aparte:** si volver cuesta menos, el recordatorio tiene adónde llegar. No se diseñan por separado.
     3. **Explorar un atajo en Android:** widget o acceso directo en la pantalla de inicio de la PWA que abra **en la caja de registro con un tap**. Sin pasar por el Home.
   - 📊 **PRIMERO MEDIR, y esto va antes de decidir nada:** sacar de la base **a qué hora del día registran hoy los testers**, para elegir el default de la pregunta diaria. Poner una hora a ojo es la forma más fácil de que la notificación llegue cuando el padre no puede abrirla, y de quemar el permiso de notificaciones, que se pide una sola vez.
...
**Técnico / costos**
- ⬜ **Fluidez del dictado — evaluar throttle del render durante la grabación** — desde **1 sep 2026** — **BAJA PRIORIDAD, y ojo con el diagnóstico**: se anotó pensando que cada parcial re-renderizaba la burbuja, pero **eso NO es lo que hace el código**. Durante la grabación `onresult` escribe **solo en refs** y no llama a `setState` (`VoiceTextarea.jsx`, el `setTranscriptText` está detrás de `if (!isRecordingRef.current)`), y la burbuja del conversacional **no muestra los parciales**. El único re-render mientras se graba es **uno por segundo**, del contador (`setSegundos`). O sea que hoy no hay nada que throttlear: si el dictado se siente pesado en un Android lento, **primero hay que medir dónde**, no asumir que son los parciales.
...
- 🟠 **REGISTRO DE TAREAS DE ESTRATEGIA — EL LOOP ESTÁ ABIERTO** — desde **31 ago 2026** — **IMPORTANTE, diseñar después del ciclo actual** (onboarding + postulación a Google). 🔎 **Origen: uso real de Daniel como papá**, no una auditoría. **El síntoma:** las estrategias generan tareas del tipo *"registra cuántas veces pasó esto"* o *"haz esto y anota tanto"*, pero **la app no tiene dónde capturar eso**. Lo único que existe es el **check-in semanal** (*"¿qué viste cambiar esta semana?"*), que es **reflexión, no bitácora** — no sirve para anotar un dato repetido a lo largo de los días. **Doble valor si se hace:** **(1)** cierra el loop de uso — la estrategia pide un dato y la app lo recibe, en vez de pedirle al padre que lo lleve en la cabeza; **(2)** ese dato **debe alimentar la personalización de la semana siguiente** de la estrategia, que hoy se genera sin saber qué pasó de verdad. 🧭 **Conecta directo con el principio del momento:** hoy las tareas de estrategia están **desconectadas del registro**, que es justo la costura que el principio dice que no debe existir. **No implementar ahora.**
- ⬜ **EPISODIOS HUÉRFANOS TRAS BORRAR O RESETEAR UN HIJO** — desde **31 ago 2026** — **BAJA PRIORIDAD**. Al borrar o resetear un hijo, **sus episodios quedan sin padre**. **Origen:** el caso Ignacia — su cuenta tenía **un episodio del 28 abr y cero hijos**, porque Daniel reseteó en su momento el perfil compartido de Pascual y el episodio quedó colgando. **Qué falta: (a)** verificar que un episodio huérfano **no rompa el Historial ni el motor de patrones**; **(b)** decidir la política — **borrado en cascada vs. archivarlos**. **Por qué es baja prioridad:** es data vieja de cuentas de prueba, no afecta a los testers nuevos, y **no hay ningún síntoma reportado** — se anota para que no se pierda, no para atacarlo ahora. ⚠️ **No confundir con el pendiente de abajo** (episodios sin orientación): ese es un fallo de la IA, este es un huérfano de borrado.
- 🔴 **RECUPERACIÓN DE EPISODIOS SIN ORIENTACIÓN** — desde **5 ago 2026** — hoy un episodio cuya orientación falló **queda muerto y el usuario ni se entera: el Historial no lo marca de ninguna forma**. Falta **(a) marcarlo visualmente en el Historial** y **(b) permitir regenerar la orientación desde ahí**. **Sin esto, cualquier caída futura de la IA deja episodios vacíos de forma permanente.** (Contexto del incidente que lo destapó: bloque del 5 ago, punto 6.) ⚠️ **DECISIÓN DE DANIEL (26 ago 2026) — ALCANCE ACOTADO: solo fix SISTÉMICO, sin reparación retroactiva.** O sea: se previene el fallo silencioso hacia adelante (que no vuelva a pasar y que el usuario se entere), y **NO se salen a reparar ni regenerar los episodios que ya quedaron vacíos**. No busques esos episodios ni propongas migraciones para arreglarlos.
- 🔴 **RECUPERAR CONTRASEÑA NO DEJA CREAR UNA NUEVA** — desde **24 ago 2026** — detectado por Daniel **en producción**. El correo *"Recuperemos tu contraseña"* **llega bien**, pero el botón *"Crear contraseña nueva"* **abre la app con la sesión ya iniciada y NUNCA pide la clave nueva**: el evento de `recovery` de Supabase no queda capturado por ninguna pantalla, así que el usuario entra y el flujo se termina ahí. **Impacto: quien olvidó su clave no puede crear una** — le queda solo el enlace mágico o Google, y si se registró con correo y clave se queda sin camino de vuelta. **Prioridad media-alta: es recuperación de cuenta de padres.** ⚠️ **Pista de solo lectura (24 ago, NO verificada contra el correo real):** `ResetPasswordPage` **sí** escucha `PASSWORD_RECOVERY` (`src/pages/auth/ResetPasswordPage.jsx:61`), y es el **único** lugar del código que lo hace. O sea que el manejador existe pero vive en `/reset-password`, y el enlace del correo probablemente aterriza en la raíz del Site URL en vez de esa ruta → el evento se dispara donde nadie lo escucha. Si se confirma, **el arreglo es de configuración (plantilla de correo / redirect en Supabase), no de código.** Verificar antes de tocar nada.
...
- **Autoasignador de códigos del tablero de `/beta` elige a ciegas** (`src/pages/beta/BetaPage.jsx:845`) — desde **1 ago 2026** — toma el primer código libre **según el tablero**, sin consultar la tabla `codigos_beta` en Supabase. Si un código se gasta fuera del tablero (QA, canje manual), el tablero no se entera y se lo asigna igual a un tester. Decisión explícita de Daniel de anotarlo y no arreglarlo ahora. ⚠️ **ACTUALIZADO 29 ago: la frase "hoy no molesta porque los 20 están libres" YA NO VALE.** Hoy hay **30 códigos en la base** (se crearon `HUELLA-21` a `30`) y **11 canjeados**, pero `BetaPage.jsx:31` sigue con la lista **hardcodeada a 20** (`Array.from({ length: 20 })`). Consecuencia concreta: **el tablero no conoce los códigos del lote 2** — hay que escribirlos a mano en la columna de código — y además puede reasignar uno ya canjeado. Es un cambio de una línea, pendiente de decisión de Daniel.
...
**Gancho de retención**
- **Fase 3 — "la notificación noble"** (gatillo de retorno con valor, nunca culpa) — 🔗 **su contenido ahora tiene nombre y dueño: la pieza 7 de la cola, "RECORDAR SIN RECLAMAR"** (la pregunta diaria a la hora que elija el padre). Lo que falta acá no es pipeline, es eso. — el pipeline de Web Push está **ENCENDIDO y verificado end-to-end en iPhone real (6 jul 2026)** y el copy de re-enganche ya se reescribió sin culpa. El **control permanente de Notificaciones en Cuenta** ya quedó RESUELTO (6 jul). Falta: **más contenido/triggers nobles** (hoy solo hay 3 reglas en `push-remind.js`). Es lo que MÁS mueve la retención.
...
- 🎨 **`NotifBanner` — PASADA DE CLAUDE DESIGN, PEDIDO EXPRESO DE DANIEL** — desde **6 jul 2026**, precisado el **24 ago 2026** — el banner *"Activa recordatorios para no perder el hilo con {hijo}"* (`src/components/NotifBanner.jsx:37`, el prompt de notificaciones push) **está en azul genérico, con un estilo ajeno a la app**: no es paleta Mocha Mix ni tono Huella, se ve como un componente de otra pieza de software. **Entra a la cola del próximo bloque de Design** para alinearlo a la paleta y a la voz. (Este pendiente ya existía como *"se ve FEO"*; el 24 ago quedó con el diagnóstico concreto y la prioridad de Daniel.)
...
- 🔴 **SOLICITUD DE ACCESO A PRODUCCIÓN — RECHAZADA POR GOOGLE el 23 ago 2026.** Se envió el **21 ago 2026 13:11** y Google la rechazó dos días después: **exige 14 días de prueba cerrada con testers ACTIVOS**, no solo inscritos. **Nueva postulación: ~10 sep 2026** — ✅ **la ventana de 14 días arrancó el 27 ago**, cuando salió el mensaje de reactivación al grupo de testers y la v4 ya estaba aprobada en Alpha. ⚠️ **Esto vuelve a poner el foco en que los 12 testers se MANTENGAN activos** — es lo único que mueve esta aguja. **Y acopla el plazo del 31 ago del nivel de API:** la advertencia de SDK solo se cierra cuando la v4 llegue a PRODUCCIÓN, y producción no llega antes del 31 de agosto. **Ese plazo se va a pasar y hay que asumirlo.**
- 🔴 **SDK 36 — plazo 31 ago 2026, SE VA A PASAR** — desde **21 ago 2026**, actualizado el **26 ago** — ✅ **la v4 (SDK 36) quedó APROBADA y publicada en el canal cerrado Alpha desde el 24 ago 13:13** (verificado en Play Console el 27 ago), pero la advertencia **solo se cierra cuando llegue a PRODUCCIÓN**, y con el rechazo del 23 ago la nueva postulación es **~10 sep**. O sea: **el 31 de agosto nos pasa por encima**. Y **desde el 31 ago 2026 toda actualización nueva a Play exige SDK 36 (Android 16)**: la próxima subida obliga a **regenerar el AAB con Bubblewrap apuntando a SDK 36**. Hay **extensión posible hasta el 1 nov 2026** si se pide. ⚠️ **SDK 36 trae reglas nuevas que sí tocan la app: sin bloqueo de orientación en pantallas grandes, y edge-to-edge obligatorio** — es lo mismo que ya avisan los 2 mensajes de Play Console sobre la versión 3, así que **se resuelven todos juntos en esa pasada**. (Verificado contra la documentación oficial de Google el 21 ago.)
...
- **NIVEL DE API — 🟢 v4 SUBIDA AL CANAL CERRADO (Alpha) EL 24 AGO 2026, EN REVISIÓN.** Construida y verificada el 22 jul, quedó guardada un mes por la regla de no tocar el canal mientras corría la beta con la v3; se subió el 24 ago. **La advertencia de nivel de API NO se cierra con esto: solo se cierra cuando la v4 llegue a PRODUCCIÓN**, y con el rechazo del 23 ago producción no llega antes del **31 ago 2026**, así que **ese plazo se pasa**. Historia de cómo se construyó: Google avisó que la app apunta a un nivel de API antiguo y debe estar en **Android 16 / API 36**. **Construida la versión 4:** `targetSdkVersion` **35 → 36** y `versionCode` **3 → 4**, editados **a mano** en `huella-twa\app\build.gradle` (solo esas 2 líneas; `compileSdk` ya estaba en 36, `minSdk 21` y `versionName "1"` sin tocar).
  ...
  - **✅ REGLA CUMPLIDA Y YA VENCIDA: "no subir la v4 mientras corra la beta con la v3".** Se respetó del 22 jul al 24 ago; ese día se subió al canal cerrado (Alpha) y quedó en revisión. **Ya no aplica.**
  - **🔴 EL PLAZO SE PASA, Y YA ESTÁ ASUMIDO:** la advertencia de nivel de API **solo se cierra cuando la v4 llegue al canal de PRODUCCIÓN — NO basta con la prueba cerrada**, y la v4 está en el canal cerrado desde el 24 ago. Con el rechazo del 23 ago, la nueva postulación a producción es **~10 sep**, o sea **después del 31 ago 2026**. La consecuencia concreta: **desde el 31 ago no se puede subir ninguna actualización nueva** hasta que la v4 esté en producción.
...
- 🧠 **CEREBRO HUELLA — FASE B EN CURSO, PRIORIZADA POR DANIEL (ago 2026)** — desde **24 ago 2026** — un **cerebro 3D manipulable del hijo**. No es una pantalla suelta: es una pieza que vive en cinco lugares. **(a)** card en el Home, *"En el cerebro de {hijo}"*, como **gancho diario**; **(b)** lugar permanente en la **página del hijo**; **(c)** momento contextual dentro de la **orientación post-episodio**; **(d)** **push mensual**; **(e)** **compartible**.
  ...
  - ✅ **PASO 4 — CERRADO, EN PRODUCCIÓN, QA DE DANIEL APROBADO EN CLARO Y EN OSCURO (26 ago 2026).** Se hizo en dos bloques con QA entre medio, y después dos rondas más de arreglos que salieron del QA en teléfono real. **Bloque A (baseline visual, `0361e30`):** sexta luz que faltaba, materiales completos (saturación + emissive), silueta del vidrio, sombra de contacto, encuadre medido y viñeta/hairline de la vitrina. **Bloque B (capa de producto, `1502ed7`):** slider 0-18 con la coreografía gris→índigo, latido de la amígdala, haces director↔alarma con punto viajero, chips con porcentaje vivo, franja "Ahora mismo" y tarjeta de zona. **Arreglos del QA (`a6d24fb`, `3fa59b3`) y ajuste de producto (`ecbe003`)**, detallados en el bloque de la sesión de hoy.
  - 📐 **Cómo quedó armado el paso 4** — el contenido salió a `src/pages/cerebro/contenidoCerebro.js` (**extraído por script del prototipo, no transcrito a mano**: el copy de Fase A no se toca). La escena se monta **una sola vez** y todo lo que cambia entra por refs; **los porcentajes los escribe el loop directo en el DOM**, nunca por `setState`, porque cambian 60 veces por segundo. Los colores de zona viven **duplicados a propósito**: en `ZONAS[slug].color` para el material WebGL y en los tokens `--cerebro-zona-*` para el CSS. **Si cambia uno, cambia el otro.**
  - ⚠️ **VERIFICACIÓN QUE AHORRÓ UNA RONDA ENTERA:** el GLB del repo es **byte a byte el mismo** que el del prototipo (mismo sha1). Eso descartó al modelo como sospechoso desde el principio: toda diferencia visual venía del código. **Si alguna vez el cerebro se ve distinto al prototipo, ese es el primer chequeo.**
  - 🌙 **QA DE `/cerebro` EN MODO OSCURO — PENDIENTE APARTE (26 ago 2026).** El prototipo **no tiene modo oscuro**, así que la Fase B se portó 1:1 con los valores de la vitrina clara. Consecuencia conocida: en oscuro **las luces y los materiales 3D no están recalibrados**, y la franja y la tarjeta se ven como **islas de modo claro** (legibles, pero claras). Los tokens `--cerebro-*` de la capa de producto están **deliberadamente sin override de oscuro**, y así está documentado en `index.css`.
  - ✅ **PASO 5 — CERRADO, EN PRODUCCIÓN, QA APROBADO (commit `0d041fb`, 26 ago 2026).** El slider ya no arranca fijo en 4: parte en la **edad real del hijo activo con precisión de mes**, y el cerebro se dibuja en ese estado. Entró **`calcularEdadDecimal`** en `HuellaContext.jsx`, al lado de `calcularEdad` — **`calcularEdad` quedó intacta, +28 líneas y 0 borradas**, verificado en el diff. El hijo sale de `state.hijo`, la misma fuente que el nombre del header. **El rótulo se dice en años y meses** ("4 años y 7 meses"), con los meses **solo hasta los 6 años**, porque de la edad escolar en adelante nadie los dice; bajo el año se cuentan meses, y el día cero dice "recién nacido". Al arrastrar el slider vuelve al formato entero. **QA aprobado con un hijo de 14 años**, incluida la carga tras cierre completo de la app.
  - 🧠 **Dos trampas del paso 5, por si se vuelven a cruzar.** (1) **`state.hijo` es null en el primer render**: la edad real llega DESPUÉS de que cargan los datos, así que hay un efecto que la adopta cuando llega, pero **solo si el usuario todavía no tocó el slider**. Sin eso el slider se quedaba pegado en el respaldo y el defecto era intermitente según la velocidad de la red — de los que no aparecen en el QA. (2) **Un `input[type=range]` con `step=1` sanitiza su valor a la grilla de pasos**: al pasarle 4,58 el navegador pone el pulgar en 5 y el relleno del riel queda ~8px antes que el pulgar, que sí se nota. **El slider y `--pos` van con la edad redondeada; el cerebro y el rótulo, con la exacta.**
  - ✅ **PASO 6 — CERRADO, EN PRODUCCIÓN, QA APROBADO EN CLARO Y OSCURO (26 ago 2026; commits `c906574` + `d46366c`). El cerebro por fin tiene puertas.** Dos entradas: el **tercer tab "Su cerebro" en `HijoPage`** (el hogar canónico) y **el CTA al pie de `TarjetaCerebro`** en el Home, que es **la entrada principal** — la que hace que la pieza se descubra. Para lograrlo se extrajo **`CerebroContenido.jsx`**: la pieza sin cáscara de página, que ahora tiene dos casas. `CerebroPage` quedó como cáscara (barra mocha + contenido) y **la ruta `/cerebro` sigue viva porque el paso 8 la necesita**.
  - 🎯 **EL CTA DEL HOME, como quedó.** Fila completa tappeable, **centrada**, con fondo `--color-surface-alt`, un **punto índigo que late** con el mismo ritmo del punto terracota de la franja "Ahora mismo", y chevron. Copy: *"Mira su cerebro por dentro — tócalo, gíralo, velo crecer"*. **Nombra las tres acciones a propósito: la pieza es manipulable y eso no se adivina desde afuera.** Está en los **cuatro** estados, **sin X y permanente**. Empezó siendo un enlace discreto y Daniel pidió subirlo a CTA: **la versión discreta no invitaba**. La flecha iba a ir en `--color-primary` pero **en claro daba 2,53:1**, así que va en `--color-text-muted` (4,61:1 claro / 6,82:1 oscuro); el texto da 13,32:1 y 13,10:1.
  - 🚫 **POR QUÉ EL ENLACE DEL HOME NO ES UNA CARD PROPIA.** Se evaluó y se descartó: `TarjetaCerebro` **ya se llama "Esta semana en el cerebro de {hijo}"**, así que una card nueva dejaría **dos tarjetas hablando del cerebro del mismo hijo en la misma pantalla** — justo la duplicación que la decisión del 24 ago mandó evitar ("el cerebro 3D ABSORBE la `TarjetaCerebro`"). El enlace adentro no suma altura al Home, cae donde ya está la cabeza del usuario y **es especialmente valioso en los estados `cero` y `pobre`**, donde la tarjeta tiene poco que mostrar. Es permanente a propósito: una X lo volvería una tarea pendiente, y desaparecer tras la primera visita dejaría la pieza inencontrable justo para quien vuelve semanas después a buscarla.
  - 📐 **La altura del tab, medida y no adivinada.** El `RetratoSendero` mide **428px** (su `aspect-ratio` es 394/432 a todo el ancho) y los tabs ~45px, así que bajo la fila de tabs quedan **~673px** útiles en un iPhone de 390×844. Con la vitrina en 56dvh el conjunto suma ~693px y **el pie se cae por debajo del borde**; con **48dvh** suma ~625px y entra todo. Por eso `CerebroContenido` acepta `compacto`, que solo el tab usa (`.vitrinaCompacta`). El margen negativo que cancela el padding del `.main` **se quedó en `CerebroPage`**: dentro del tab habría sacado el contenido de su contenedor.
  - 🧹 **CABO SUELTO ANOTADO, NO TOCADO — el `bancoEducativo` se superpone con la matriz del cerebro.** El estado `pobre` de `TarjetaCerebro` muestra contenido educativo del cerebro **por edad**, desde `src/components/panel/bancoEducativo.js`. Eso cuenta lo mismo que la matriz zona × lente × edad del Cerebro Huella, **desde una fuente distinta**, así que pueden contradecirse. Cuando la absorción decidida el 24 ago se complete, ese banco debería salir de ahí o pasar a alimentarse de la matriz. **No se tocó en el paso 6.**
  - 🟡 **PASO 8 — IMPLEMENTADO Y EN PRODUCCIÓN, PERO EL DISEÑO NO ESTÁ CERRADO (27 ago 2026; commits `71c098f` + `6cf1302`, migración 014 corrida y columna verificada por Daniel).** Funciona de punta a punta: **la zona la decide la IA** al generar la orientación (marcador al final del texto, separado en el servicio), el **enlace aparece al cierre de la orientación**, `/cerebro?zona=amigdala` **abre la tarjeta de esa zona** en la edad real del hijo, el **texto se guarda siempre limpio** (el marcador nunca llega al PDF) y el **doble descargo quedó eliminado de raíz**. ⚠️ **PERO el enlace NO convence a Daniel: "no me cierra".** Ver el pendiente de diseño en la sección de abajo — **es lo PRIMERO de la próxima sesión de cerebro**.
  - ✅ **EL TAP AL MODELO 3D EN ANDROID — CERRADO CON QA DE DANIEL EN ANDROID (`7cdd878`, 8 sep 2026).** Estuvo abierto desde el 31 ago. Detectado por Daniel el 31 ago en teléfono real. ⚠️ **La descripción vieja del síntoma era incorrecta** (decía *"el raycast detecta, el chip se ilumina, pero la tarjeta no abre"*): la evidencia del 6 sep muestra que **no pasa nada de nada**, y que **la tarjeta SÍ abría y se cerraba sola en el mismo gesto**. 🔴 **Causa raíz confirmada por experimento:** un tap táctil genera, además del `pointerup` que abre la tarjeta, un **`click` sintético**; Chrome en Android resuelve su destino contra el árbol del momento en que lo dispara, y para entonces el **velo** de la tarjeta (`fixed`, `inset: 0`, `pointer-events: auto` apenas abre) está encima del punto del dedo, así que su `onClick={cerrar}` la cierra al instante. El chip tampoco se enciende porque depende de `abierta`. En iPhone no pasa porque Safari usa el destino del toque inicial. Los chips nunca fallaron: son botones y reciben un solo click. **Arreglo:** `preventDefault` en un `touchend` sobre el canvas (`{ passive: false }`, con cleanup), más el umbral de tap de **9px → 12px**. ⛔ **`d46366c` (el fix de la deriva) quedó DESCARTADO como sospechoso**, igual que `touch-action`, `pointercancel`, el conteo de dedos y el orden de listeners. **Los 6 pasos del QA que falta están en el bloque del 6 sep.** 🎯 Sigue siendo prioridad ALTA: la beta es 100% Android.
  - ⏭️ **PASO 7 — PASA A OBLIGATORIO (31 ago 2026), y lo sigue bloqueando el hardware, no el código:** QA en la **tablet Samsung** y decisión **GLB v1 vs v2**. **Daniel avisará cuando tenga la tablet a mano.** ⚠️ **Ya no es opcional ni paralelo: el bug del tap en Android lo volvió obligatorio, y el QA DEBE incluir ese caso** (tocar una zona del modelo y verificar que la tarjeta abra, no solo que el chip se ilumine). El paso 8 se hizo antes sin esperarlo, y no pasó nada: no se bloquean entre sí.
  - ⚖️ **PRIORIDAD ACORDADA (26 ago 2026)** — ⚠️ **PARCIALMENTE SUPERADA EL 31 AGO: el bug del tap en Android subió al paso 7 a obligatorio y con prioridad alta.** Lo que sigue vale como contexto de por qué el cerebro no bloqueaba nada hasta ahora: **los pasos 7 y 8 van EN PARALELO y NO son lo primero.** Lo primero de la próxima sesión es el **Frente 1 — Google Play** (ver la sección de PRIORIDAD INMEDIATA). **La pieza YA está lista para mostrarse a los testers con los pasos 4-6**, así que no hay que esperar al 7 ni al 8 para usarla como novedad en el grupo. Dicho de otra forma: **el cerebro deja de ser trabajo pendiente y pasa a ser munición para reactivar la beta.**
  - 📌 **De dónde venía el paso 6:** antes de esto **`/cerebro` no tenía NINGÚN punto de entrada**. Se verificó el 26 ago que, fuera de la línea que declara la ruta en `App.jsx:188`, **no había un solo enlace ni `navigate` a `/cerebro` en toda la app**: los siete commits del paso 4 y del paso 5 estaban en producción y **ningún tester los había visto**. Vale recordarlo como patrón: **una pieza sin puerta no existe**, por terminada que esté.
  - ✅ **El panel `?diag=1` (`513b4c2`) YA SE ELIMINÓ (26 ago 2026)**, junto con toda su instrumentación: el objeto `diag`, el prop `onDiag`, la lectura de GPU y de tamaños de canvas, el cronómetro de carga del modelo y las dos filas de la fila de chips que se habían agregado en `a6d24fb`. **Se conservó a propósito** el listener de `webglcontextlost` con su `console.error`: eso no es instrumentación del panel sino registro de un fallo que **no deja ningún rastro visible** (el canvas queda en blanco sin error), y su `preventDefault` es lo que permite que el contexto se pueda restaurar.
  - 🪶 **Pendiente menor — hacer la ruta `/cerebro` lazy en `App.jsx`.** Desde el **26 ago 2026**. `contenidoCerebro.js` lo importan las dos capas, y como `CerebroPage` se importa **estático** en `App.jsx`, el copy y el CSS del cerebro entran al **bundle inicial de todos los usuarios**, abran o no la pantalla: **+6,4 kB gzip** (JS 261,89 → 266,90 · CSS 45,44 → 46,85). **Decisión de Daniel: se queda así por ahora.** El arreglo es hacer la ruta lazy, pero **hoy NINGUNA página de `App.jsx` es lazy**, así que introduce un patrón nuevo en el router y merece su propia pasada. El chunk de `three` **sigue lazy y no se tocó**.
...
**Deuda de diseño**
- 🆕 **El verde de "Ver orientacion" no pasa contraste en claro** — desde **1 sep 2026** — `.iaInline` en `EpisodioCard.module.css` usa `--color-accent-green` sobre `--color-surface`: da **2,68:1** y es letra de **11px bold**. Se detectó al medir los tokens para el toggle del relato, que por eso NO copió ese verde y usa `--color-text-muted` (5,81:1 claro / 6,23:1 oscuro). **No se tocó** para no mezclar con el fix de voz: va en la próxima pasada de Design, junto con los otros hex.
...
- 🔴🥇 **PASO 8 — EL ENLACE AL CEREBRO NO CONVENCE. ES LO PRIMERO DE LA PRÓXIMA SESIÓN DE CEREBRO.** Desde **27 ago 2026**. **Veredicto de Daniel: "no me cierra"** — lo encuentra **poco llamativo incluso ya convertido en fila CTA**. Ya subió un grado esta misma sesión (`6cf1302`): pasó de línea de texto pelada a fila con fondo `--color-surface-alt`, `--radius-md`, padding de tap, chevron y **punto de la zona latiendo** a 2,2s, con copy *"Mira lo que se encendió en su cerebro"*. **No alcanzó.** ⚠️ **NO se volvió a tocar sin decisión suya: subir el contraste a ciegas arriesga romper el lente minimalista de las últimas sesiones, que es una decisión de producto, no un detalle.** **Palancas ya identificadas para subir OTRO grado sin romper ese lente:** (1) **borde de la fila en el color de la zona** — la tiñe de identidad sin agregar peso; (2) **tinte suave del color de la zona en el fondo** en vez del `--color-surface-alt` neutro. **Y queda abierto un rediseño mayor si Daniel lo pide** (card propia, o mover el enlace fuera del plegable de "Orientación completa", que es donde hoy vive y puede ser la causa real de que se sienta escondido: el plegable está cerrado por defecto). Archivos: `src/components/registro/EnlaceZona.jsx` + `.module.css`.
- ⬜ **QA 2 DEL PASO 8 — INCOMPLETO, hay que terminarlo antes de declararlo cerrado.** Desde **27 ago 2026**. Lo único que se alcanzó a ver fue el enlace en pantalla (y de ahí salió el "no me cierra"). **Falta TODO lo demás:** el **veredicto final de diseño**, confirmar que el **punto late** de verdad, que el **tap abre la zona correcta**, el **QA en claro Y en oscuro** (ojo con la trampa conocida: el incógnito del teléfono fuerza oscuro, así que el de claro va en pestaña normal), y los **puntos 3 a 8 de la lista original**: episodio nuevo SIN zona (no debe aparecer nada), **episodio viejo** (sin enlace y sin error), **PDF limpio** (que el marcador no salga impreso — el más importante), **slug inválido** en `?zona=hipotalamo` (se ignora), y **cerrar la tarjeta** (no debe volver a abrirse sola).
- ⬜ **Decisión pendiente de Daniel — aplicar el filtro de letra chica también en `InformePDF`.** Desde **27 ago 2026**. El doble descargo ya se arregló en pantalla y en el prompt, así que los episodios NUEVOS nacen limpios. Pero los episodios **ya generados guardaron el descargo largo dentro de `orientacion_ia`**, y el informe **imprime ese texto crudo** (`InformePDF.jsx:917`), así que ahí el descargo se sigue leyendo dos veces. **El arreglo es barato:** el mismo `esLetraChicaDelModelo` que ya vive en `OrientacionSecciones.jsx`, aplicado también en el PDF. **No se hizo porque nadie lo pidió**, y tocar el informe sin QA de Daniel es justo lo que el protocolo prohíbe.
- 🔴 **El escarabajo del alivio ("huella te lee") no respira: solo pulsa de color, sin cambiar de tamaño** — reportado por Daniel el **27 ago 2026** durante el QA del paso 8 — **debe respirar agrandándose y achicándose, con el MISMO efecto del escarabajo del loader inicial** (`SplashArranque.module.css` → `@keyframes latido`: `scale(1)` ↔ `scale(1.06)` con opacidad a 0.85). Hoy es `.gSelloBicho` en `RegistroPage.module.css:701` con `@keyframes gRespira`, **solo opacidad**. ⚠️ **NO es un descuido: la ausencia de escala está documentada a propósito en ese mismo archivo (líneas 710-745) y viene de CUATRO intentos.** Los tres primeros con escala fallaron por imperceptibles (`4.5s/1.03`, `3.2s/1.045`, `2.4s/1.055`), y el que sí se veía **salía BORROSO en Safari iOS**: al animar `transform: scale` sobre un SVG, WebKit lo rasteriza una vez y después estira esos píxeles, así que el dibujo pierde filo justo mientras crece. Por eso se dejó solo opacidad. **Decisión de Daniel ("como habíamos quedado"): igual quiere la escala.** Entonces el fix NO es volver a poner `scale` y ya — es resolver el borroso de iOS primero. Caminos a evaluar antes de tocar: animar el CONTENEDOR y no el SVG, forzar capa propia con `will-change: transform`, o dar al SVG un tamaño de render mayor y escalarlo hacia abajo en reposo. **Y el QA de este arreglo tiene que incluir iPhone real, no solo Android** — que es exactamente lo que destapó el problema la vez pasada.

---

## 📌 REGLA DE SOPORTE A TESTERS (dura, desde el 30 ago 2026)

> **Los testers NUNCA reciben instrucciones técnicas de soporte. Todo se arregla del lado nuestro y les llega solo.**

**Por qué es dura y no cortesía:** un tester al que se le pide diagnosticar **se cae de la beta**, y cada uno que se cae **reinicia el reloj de los 14 días de Google**. Salió del episodio del bug de arranque (bloque del 30 ago): el arreglo correcto fue de código, no un instructivo por WhatsApp.

**Y la palanca que lo hace posible:** el arreglo es **web (Vercel), no Android** — le llega a todos al abrir la app, **sin pasar por Play**. O sea que **el bloqueo del 31 ago para subir versiones nuevas NO impide arreglar cosas**.

**Corolario de diagnóstico:** antes de pedirle algo a un tester, revisar si Daniel puede verificarlo solo. Casi siempre puede — el caso Vale lo mostró: tiene una cuenta del mismo dominio Workspace y podía probar la hipótesis sin tocarla.

---

## 🧪 RED DE SEGURIDAD DEL ARRANQUE (desde el 30 ago 2026)

**Si tocas `src/context/AuthContext.jsx` o `src/components/ui/SplashArranque.jsx`, corre esto antes de dar nada por bueno:**
...
Debe dar **24 OK, 0 fallas**. Detalle en `verificacion/README.md`. Cubre el bug del splash eterno del 30 ago, y **demuestra el defecto** en la versión anterior además de demostrar el arreglo. ⚠️ Ahí están documentadas **dos trampas que daban falsos verdes**: si la suite pasa sospechosamente fácil, verifica `grep -c createClient verificacion/dist/entrada.mjs` → debe dar **0**.

---

## 🧭 REGLAS TÉCNICAS DE LA APP (aprendidas a la mala — leer antes de tocar UI)

*Cinco reglas que salieron de bugs reales, todos del 26 ago 2026. Las tres primeras tienen la misma forma —eran evitables si alguien hubiera leído lo que el propio repo ya decía— y las dos últimas son trampas de librería que van a volver a aparecer en cualquier escena 3D.*

**1. Todo overlay de pantalla completa va por `createPortal` a `document.body`.** — desde el **26 ago 2026**. ✅ **ACTUALIZADA EL 4 SEP 2026: la raíz quedó arreglada** (commit `d7ce582`). `PageTransition` ahora quita la clase animada cuando termina la entrada (`onAnimationEnd` filtrando `e.target === e.currentTarget`) y la reemplaza por `.pageLlegado` (opacidad 1, sin animation, sin transform), así que el wrapper **ya no es stacking context ni containing block** de nada. **Consecuencia:** un overlay `fixed` dentro de una página ya tapa la barra sin portal; los 9 que había sin portal (RutinaDiaria, Leyenda y Detalle de medallas, ModalPuenteCiclo, hoja de EstrategiaDetail, EstrategiaNueva, RegistroConversacional, consejoBubble, CerebroPage) quedaron cubiertos de una vez, y los 4 con portal no cambian. El portal sigue siendo el patrón recomendado para capas nuevas de z-index 1000 (es más explícito), pero **ya no es lo que evita el bug**. Ojo con el detalle que costó encontrar: con `fadeIn` (solo opacidad) Chrome mantiene el stacking context **aunque la opacidad final sea 1**, porque trata a la animación terminada-pero-rellena como si siguiera corriendo. Lo que sigue es el diagnóstico original del 26 ago, que sigue siendo correcto sobre el porqué.
`.pageWrap` (`Layout.jsx`) anima la entrada de cada pantalla con `animation-fill-mode: both`, así que **queda con el `transform` aplicado para siempre** después de la animación. Un ancestro con `transform` hace dos cosas: **(a) crea un contexto de apilado**, y ahí el `z-index` del overlay se mide *dentro* de `.pageWrap` en vez de contra la raíz, así que **la barra de navegación le gana**; y **(b) se vuelve el bloque contenedor del `position: fixed`**, así que el overlay **se mueve con el scroll** en vez de quedarse pegado al viewport. **Los dos síntomas salen del mismo transform.** `UpgradeModal`, `CerrarPatronModal` y `RegistroPage` ya lo hacían bien; la tarjeta del cerebro fue el único overlay de z-index 1000 que se renderizó en su lugar, y falló exactamente así. **El comentario que lo explica ya estaba escrito en `UpgradeModal.jsx:70` y no se leyó.**

**2. Fondo fijo → texto fijo. Fondo del tema → texto del tema. NUNCA mezclar las dos capas en un mismo componente.** — desde el **26 ago 2026**.
Hay dos familias de color: los tokens del **tema** (`--color-*`, cambian solos en oscuro) y los **fijos** (`--cerebro-*` y similares, no cambian). Poner un fondo del tema debajo de un texto fijo deja el texto invisible en el modo contrario. Pasó literal: la tarjeta del cerebro tenía `background: var(--color-surface)` —que se vuelve `#221A10` en oscuro— con textos en `--cerebro-tinta` fijo, y quedó en **1,31:1**. La pista de que la causa era esa: el único bloque legible era el que tenía fondo fijo. **Y aplica al revés también:** un texto fijo sobre el fondo de la **página** (que sí cambia) es el mismo error espejado — así quedó el valor de la edad en **2,69:1**.

**3. Toda pantalla nueva se hace QA en modo CLARO *y* en modo OSCURO antes de aprobarse.** — desde el **26 ago 2026**.
No es opcional ni "para después": los dos defectos de arriba **solo se ven en oscuro** y llegaron a producción aprobados. ⚠️ **Ojo con la trampa del teléfono: la ventana de incógnito fuerza modo oscuro**, así que si el QA se hace ahí se está probando *solo* oscuro sin saberlo. **El QA de claro va en pestaña normal.** Y el atajo que ahorra tiempo: cuando un color de texto vaya sobre un fondo, **medir el contraste antes de mandarlo**, no después — en esta sesión la medición encontró que **ninguno** de los 6 colores de zona servía como texto, incluido el que a ojo parecía seguro.

**4. `autoRotate` de OrbitControls NO basta: solo se aplica si el control está en reposo.** — desde el **26 ago 2026**.
La condición literal de la librería es `if (this.autoRotate && this.state === _STATE.NONE)`. **Si un gesto termina sin su `pointerup` —el sistema lo cancela, o el elemento pierde la captura del puntero— el control queda atascado en `ROTATE` y la deriva no vuelve NUNCA**, aunque `autoRotate` valga `true`. Fue exactamente el defecto del tab: el cerebro quedaba fijo al volver, pero **la amígdala seguía latiendo** — o sea el loop corría y el problema era solo del giro. **Esa pregunta, "¿lo demás se sigue moviendo?", es la que separa un problema de render de uno de controles, y cuesta un vistazo.** El arreglo tiene tres piezas: llevar la cuenta de los punteros efectivamente apoyados, escuchar `pointercancel` y `lostpointercapture`, y un centinela en el loop que devuelve el control al reposo cuando no queda ningún dedo encima. Dato útil para la próxima: **OrbitControls llama `setPointerCapture` en el `pointerdown`**, así que los eventos siguen llegando al canvas aunque el dedo salga, y `lostpointercapture` dispara siempre al terminar el gesto — es el cierre garantizado.

**5. `pointermove` llega SIN dedo apoyado.** — desde el **26 ago 2026**.
El evento también dispara cuando un puntero solo pasa por encima (el hover del mouse, un dedo que cruza). Cualquier lógica de "el usuario está interactuando" colgada de `pointermove` a secas se activa sola. Hay que comprobar que ese puntero esté efectivamente apoyado.
...
## ⏭️ PRIORIDAD INMEDIATA — 🚀 BETA REACTIVADA EL 27 AGO · ⏱️ LA VENTANA DE 14 DÍAS DE TESTERS ACTIVOS CORRE DESDE HOY · lo que toca ahora: sostener la actividad y sumar colchón
...
- ✅ **Google tachó los 3 requisitos** en la solicitud: **versión cerrada**, **12 testers** y **14 días**. Los tres cumplidos.
- 📊 **Medición del 7 sep, la que respalda el envío: 13 activos de 23 con cuenta.** Subió desde los 12 del 4 sep: **`snnunez1` se registró y quedó activo**. **`sfaille` sigue sin crear cuenta** — es el único de la lista oficial que nunca se registró.
...
**Esto ya no bloquea el código.** Mientras Google revisa, la cola de producto queda **desbloqueada** (ver la cola priorizada en PENDIENTES).

**Los tres pasos acordados el 26 ago, al cierre del 27 ago:**

1. ✅ **HECHO — la v4 (SDK 36) fue APROBADA.** Verificado por Daniel directo en Play Console el **27 ago**: la versión está **publicada en el canal cerrado Alpha desde el 24 ago 13:13**, **activa para testers**, en **177 países**. **El correo de aprobación nunca llegó** — igual que pasó con la v3 el 18 jul. Regla que ya vale dos veces: **la señal de Google se busca en Play Console, no en la bandeja de entrada**.
2. ✅ **HECHO — el mensaje de reactivación se envió al grupo de WhatsApp de testers el 27 ago.** El anzuelo fue **el Cerebro Huella 3D como novedad**, y el mensaje **agradeció el feedback de los testers como el motivo de la remodelación**. Es verdad, y además es exactamente el tipo de motivo que hace que alguien vuelva a abrir la app: ver que lo que dijo se convirtió en algo.
3. ✅ **HECHO Y AMPLIADO — se reclutaron 7 testers nuevos.** **5 el 29 ago** (entraron al grupo, recibieron código y mensaje) **+ 2 el 31 ago** (`rlazod`, `sfaille` — ya en el grupo; mensajes y códigos salen ese día). La lista oficial pasó de 18 a **22 personas** *(hoy 23: Ceci Caro se sumó el 2 sep)*. Detalle en los bloques del 29 y del 31 ago.

**⏱️ EL RELOJ, CON FECHA:** los **14 días de testers ACTIVOS** que exige Google **corren desde el 27 ago**, el día del mensaje de reactivación. **La nueva postulación a producción apunta a ~10 sep 2026.** La estimación anterior era ~7 sep y se movió porque la ventana no arrancaba hasta que los testers tuvieran un motivo real para volver a abrir la app.

**🔑 CÓMO SE AGREGA UN TESTER (dato operativo, aprendido el 29 ago):** el acceso va por el **GRUPO DE GOOGLE `testers-huella@huella.lat`**, **NO** por una lista de correos en Play Console. Un tester nuevo se agrega en **`groups.google.com`**, no en la consola. Si alguien "no puede entrar a la prueba", lo primero que se revisa es si está en el grupo.

**Lo próximo del Frente 1, en orden:**

1. **Medir la actividad en la ventana 2-4 sep** (día ~7 del reloj), **sobre las 24 personas**, con las queries de métricas cruzadas contra la **LISTA OFICIAL DE TESTERS** de la sección de abajo (esa tabla manda: son **24 personas**, y si un número no cuadra, manda la tabla). Antes del 2 sep el dato no sirve: son pocos días y cae en fin de semana.
2. **Empujar a los 3 registrados que NO canjearon su código** — `v.ramirez` (H-13), `javiera.zamora.r` (H-17), `nene.pepita` (H-11). *(Eran 4: `ignacopardo` canjeó `H-15` el 31 ago.)* **Hipótesis fuerte: buscaron el bloque de canje en el inicio, donde ya no está** (se mudó a "Tú"). El recordatorio debe llevar la instrucción corregida.
3. **Pedirle a Igna que abra la app de Play desde SU teléfono** — ya tiene cuenta, hijo y código canjeado, pero **hasta que no abra desde su dispositivo no le cuenta a Google**. Mensaje simple, sin nada técnico.
4. **Seguimiento de la incidencia de `mariaadrianaq`** (ver el bloque del 29 ago).
5. **Vale — en observación, sin acción.** La capa 2 pasó a segundo plano el 31 ago; **se evalúa sola en la medición del 2-4 sep** (ver el bloque de ese día).

**El Cerebro Huella (pasos 7 y 8) sigue en paralelo y NO bloquea nada de esto.** Es trabajo de código, que puede esperar; el frente de Google no.

---

### 📋 LISTA OFICIAL DE TESTERS — LA FUENTE DE VERDAD (fijada el 11 ago 2026)

**Son 24 personas (actualizado el 2 sep 2026).** La cuenta: eran **18** al 11 ago, **− 3 que nunca respondieron** (`be.delacerda`, `snackclub2022`, `nicolas.arriagada` — sus códigos `H-01`, `H-06` y `H-09` se reciclaron el 29 ago tras verificar por query que seguían sin canjear), **+ 5 testers nuevos el 29 ago** = 20, **+ 2 más el 31 ago** (`rlazod` y `sfaille`) = 22, **+ Ceci Caro el 2 sep** = 23, **+ anto el 2 sep** = **24**.

*Historia del número, para que no se vuelva a enredar: los 17 del lanzamiento (2 ago) **+ la mamá** (`nene.pepita`, 5 ago) = 18, **− Nicole** (falta de espacio en el teléfono) = 17, **+ Nicolás Arriagada** = 18, y de ahí a las 24 de hoy. En días anteriores del archivo quedó escrito "17" en varios lados porque cada movimiento se anotó por separado y nunca se rehízo la suma.*
...
| # | Nombre | Correo | Código | Estado al 2 sep |
|---|---|---|---|---|
| 1 | Diego Diaz | `ddiegodiaz@gmail.com` | `HUELLA-02` | registrado, sin actividad desde 4 ago |
| 2 | Ceci Leiva | `cecilialeivausset@gmail.com` | `HUELLA-03` | registrado, sin actividad desde 5 ago |
| 3 | Seba | `s.undurragar@gmail.com` | `HUELLA-04` | registrado, sin actividad desde 1 ago |
| 4 | Negra | `bundurraga.r@gmail.com` | `HUELLA-05` | registrado, sin actividad desde 5 ago |
| 5 | Krishna | `krishna.baquedano77@gmail.com` | `HUELLA-06` | **ACTIVO (2 sep)** |
| 6 | Pia Heap | `pia.heap@gmail.com` | `HUELLA-07` | registrado, sin actividad desde 5 ago |
| 7 | Matias Cartagena | `matias.lagos.cartagena@gmail.com` | `HUELLA-08` | registrado, sin actividad desde 2 ago |
| 8 | Paulina Moraga | `paulinajmg.88@gmail.com` | `HUELLA-10` | registrado, sin actividad desde 6 ago |
| 9 | Tomas Vergara | `tomas.vergara88@gmail.com` | `HUELLA-14` | registrado, sin actividad desde 29 jul |
| 10 | Maria Adriana | `mariaadrianaq@gmail.com` | `HUELLA-01` | **ACTIVO (2 sep)** |
| 11 | Vale Ramirez | `v.ramirez@pacificschoolpichilemu.com` | `HUELLA-13` | registrado, sin actividad desde 11 ago |
| 12 | Cata Grez | `catagrez9363@gmail.com` | `HUELLA-16` | hijo creado, sin registros |
| 13 | Javi Zamora | `javiera.zamora.r@gmail.com` | `HUELLA-17` | hijo creado, sin registros |
| 14 | Max Lund | `mlundv@gmail.com` | `HUELLA-18` | registrado, sin actividad desde 2 ago |
| 15 | Igna | `ignacopardo@gmail.com` | `HUELLA-15` | **ACTIVO (1 sep)** |
| 16 | Carolina Morales | `calomose@gmail.com` | `HUELLA-09` | **ACTIVO (31 ago)** |
| 17 | Anita Gomez | `amgomezsc1@gmail.com` | `HUELLA-19` | registrado, sin actividad desde 2 ago |
| 18 | Mama | `nene.pepita@gmail.com` | `HUELLA-11` | **ACTIVO (30 ago)** |
| 19 | Constanza Aguero | `conny.aguero@gmail.com` | `HUELLA-12` | **ACTIVO (1 sep)** |
| 20 | Iti Fuenzalida | `snnunez1@gmail.com` | `HUELLA-26` | nunca se registro |
| 21 | Rodrigo Lazo | `rlazod@gmail.com` | `HUELLA-29` | hijo creado, sin registros |
| 22 | Titi Scout | `sfaille@gmail.com` | `HUELLA-30` | nunca se registro |
| 23 | Ceci Caro | `cecicarob@gmail.com` | `HUELLA-27` | nunca se registro |
| 24 | anto | `antonia.venegas@gmail.com` | `HUELLA-20` | agregada 2 sep, aun sin registrarse |

**📊 Medición 2 sep: 6 activos desde el 27 ago** (Maria Adriana, Krishna, Constanza, Igna, Carolina, Mama). **Push de WhatsApp enviado el 2 sep a los 17 restantes.** **📊 Medición 4 sep: 12 activos de 22 con cuenta** (7 registrados sin actividad desde el 27 ago, 3 con hijo y cero registros, 2 nunca registrados). **📊 Medición 7 sep, la que respaldó el envío: 13 activos de 23 con cuenta** — `snnunez1` se registró y quedó activo; **`sfaille` es el único que nunca creó cuenta**. **🚀 Re-postulación ENVIADA el 8 sep 9:28, con los 3 requisitos tachados por Google.** 📲 **Instalaciones al 2 sep, según el Sheets de Daniel: 14 con la app puesta.** Google exige **12 simultáneos**, así que hoy el colchón es de **2**: si tres desinstalan, el reloj se reinicia. **Fuente de verdad de códigos: el Sheets de Daniel** — si el Sheets y este archivo difieren, gana el Sheets.

*Notas de la tabla: **(a)** ✅ **Desde el 2 sep la columna de código trae los 24 códigos**, tomados del Sheets de Daniel (antes solo estaban los confirmados y el resto vivía en el tablero de `/beta`, que es estado local del navegador y **no** la base). **(b)** Los nombres quedaron completos el 31 ago. **(c)** Salieron de la lista `be.delacerda`, `snackclub2022` y `nicolas.arriagada` — nunca respondieron y sus códigos se reciclaron el 29 ago.*
...
⚠️ **Con 15 instalados hay 3 de colchón sobre los 12 que exige Google, y el requisito es de 12 SIMULTÁNEOS: si cuatro desinstalan, el reloj se reinicia.** *(Cifras del 11 ago; el estado vigente es la tabla y la nota de medición de arriba.)*
...
2. **🔴 GOOGLE RECHAZÓ EL ACCESO A PRODUCCIÓN el 23 ago 2026:** exige **14 días de prueba cerrada con testers ACTIVOS**, no solo inscritos. **Nueva postulación ~10 sep.** La **v4 (SDK 36) se subió al canal cerrado Alpha el 24 ago** y quedó en revisión, pero la advertencia de nivel de API **solo se cierra en PRODUCCIÓN** → el plazo del **31 ago 2026 se va a pasar** y desde esa fecha no se puede subir ninguna actualización nueva. **Lo único que mueve la aguja es que los 12 testers se mantengan ACTIVOS**, y esa es la dependencia más importante del calendario ahora mismo. ✅ **ACTUALIZACIÓN 27 ago: la v4 quedó APROBADA y publicada en Alpha, y el mensaje de reactivación salió al grupo de WhatsApp — la ventana de 14 días corre desde el 27 ago y la nueva postulación apunta a ~10 sep.**
...
🏆 **Daniel confirmó los 2 candidatos de Pipa. El hero marca "2 de 12": son los primeros rasgos confirmados que existen en la base**, después de meses con el motor detectando y nadie pudiendo resolver nada.

### 1. ❌ PIEZA 3 (motor de rasgos + hitos) — FUERA DE LA COLA: LA PREMISA ERA FALSA

La pieza decía *"hoy el motor lee solo episodios difíciles"*. **No es cierto, y no lo fue nunca.** El motor lee episodios **y** hitos desde que se escribió: el prompt marca cada momento con su `origen` y pide que las familias positivas se nutran de los avances; los dos enganches cuentan episodios + hitos juntos para el gatillo. **Medido en producción: 28 de 70 rasgos tienen evidencia de hito.** No había nada que construir.

**Lección de proceso, y es la grande del día:** la pieza llevaba semanas en la cola con una premisa que nadie había verificado contra el código. El paso 0 —una consulta de lectura— la desarmó en cinco minutos. **Medir antes de construir.**

### 2. 🔴 LA CAUSA REAL DE LOS 0 CONFIRMADOS: LA CARD NUNCA SE RENDERIZÓ, A NADIE (`6f69ac1`)

De 70 rasgos: **61 emergentes, 9 candidatos, 0 confirmados y 0 descartados.** No era visibilidad ni volumen. Era una **carrera de efectos** en `HijoPage.jsx`.

- **El mecanismo:** había **dos** efectos, uno que fijaba el candidato y otro que lo reiniciaba a null al cambiar de hijo activo. Al terminar la carga, `hijo` pasaba de null al hijo real y eso disparaba **los dos en el mismo commit**, en orden de declaración. El primero fijaba, el segundo borraba. **El valor neto iba de null a null**, así que React no volvía a renderizar y el primero no corría nunca más: ninguna de sus dependencias había cambiado.
- **Reproducido en Chrome con harness**, los dos efectos copiados literales y los datos reales de Pipa: caso real `SIN CARD`, traza `B reinicia | A fija C1 | B reinicia`. Control sin el efecto de reinicio: `CARD VISIBLE`.
- **Descartadas con evidencia:** la consulta no filtra por estado, la RLS lee bien, la condición no exige nada más que `estado === 'candidato'`, y la card es el primer elemento del tab.
- **Por qué el badge sí y la card no:** el badge lee `state.rasgos` **directo en el render**; la card pasaba por un `useState` intermedio, que era lo que se rompía.
- **El fix:** un solo efecto, con el candidato guardado **junto al `hijoId`**. Conserva las dos reglas de diseño (uno por visita, y al cambiar de hijo se busca de nuevo).

### 3. ✅ Motor de rasgos — dos pasos que sí salieron del diagnóstico

- **Gatillo por tramo (`9484026`).** Corría con `total % 5 === 0`: si el conteo brincaba de 4 a 6 —un borrado, o momentos desde otro dispositivo— **ese tramo no corría nunca**. Ahora compara contra el último total analizado, guardado por hijo en `localStorage`, y corre cuando el hijo **cruza** un tramo de 5. El marcador se pone antes de disparar, para que dos guardados seguidos no llamen dos veces a la IA.
- **Migración 017 (`72bac32`).** El estado `emergente` se agregó a la base viva a mano durante la Capa 1 y nunca quedó como archivo: `006_rasgos.sql` seguía declarando el CHECK viejo de tres estados. **Ya está aplicada en producción** (hay 61 filas emergentes); el archivo solo alinea el repo con la base. **NO hay que correrla.**

### 4. 🎨 EL HERO DEL RETRATO, TERMINADO (`802dd8b` + `0d801f5` + `8ad0bc1`)

- **Marca de agua.** El escarabajo del logo (`soloSimbolo`, sin wordmark) sangrando por la esquina inferior derecha, 176x192, `z-index 0`, sin eventos, `aria-hidden`. El crema sale de `--color-on-mocha`, que vale el **mismo hex en claro y oscuro**, así que no necesita override de tema y no entró ni un hex nuevo.
- **El caminante parte centrado bajo la foto.** Antes arrancaba en 168,292, corrido a la izquierda, y **se dibujaba de lado**: sin tramo recorrido la tangente degenera y el ángulo caía en 90°. Ahora nace en 197,265 (40px de aire bajo la foto) y en reposo mira hacia ella con rotación 0.
- 🔴 **EL SENDERO CRUZABA POR ENCIMA DE LA FOTO — y la causa no era la que parecía.** Síntoma de Daniel en "La brava" (6 de 12): el caminante montado sobre el medallón. **Medido: el camino NUNCA cruzaba el círculo** (mínimo 82 contra radio 75). **El que invadía era el caminante**, que medía 72 de ancho: su radio de 36 se comía los 7 de aire del trazo y entraba **27px dentro de la foto**.
  - **El corredor de arriba no daba para un bicho de 72:** entre el borde de la foto (y 75) y el techo del lienzo hay 75, o sea **3px de holgura total** para un diámetro de 72. Por eso **el caminante baja a 60px**, decisión de Daniel.
  - **Cuatro waypoints empujados radialmente hasta ~112**, conservando su ángulo: `wp7 165,60→159,44` · `wp8 197,68→197,38` · `wp9 232,58→237,45` · `wp12 300,150→309,150`. **El wp12 no estaba en el diagnóstico inicial**: apareció al re-medir con el umbral nuevo. Los otros 14 no se movieron, siguen siendo 18 y la serpiente conserva su forma. Los controles vecinos se recalcularon con la misma fórmula Catmull-Rom del handoff.
  - **Holguras finales, parejas por los tres lados:** 6.1 contra la foto, 8.0 contra el filo del lienzo, 7.2 contra el gear.

### 5. 🔤 Copy de la card + HALLAZGO: 12 mapeos de género duplicados (`67d0dbe` + `7da2dbf`)

- Botón primario: **"Sí, lo reconozco"**, con tilde.
...
- **Las 2 consultas de diagnóstico que quedaron escritas y nunca se corrieron** —las corridas del motor por hijo, y el detalle de los rasgos de Pascual— **quedan DESCARTADAS.** Se escribieron para explicar por qué Pascual no tenía rasgos confirmados; el bug de la card respondió esa pregunta entera y con evidencia más dura. Correrlas ahora no cambiaría ninguna decisión.
...
## Sesión martes 8 septiembre 2026 — **El tap al Cerebro cerrado con QA en Android, la re-postulación enviada, y las piezas 1 y 2 de la cola en producción**

**🎯 Tres cosas: un bug de dos semanas cerrado, el Frente 1 despachado, y la primera pieza de la cola nueva construida el mismo día en que se desbloqueó.**

### 1. ✅ CERRADO CON QA EN ANDROID — el tap al modelo 3D del Cerebro (`7cdd878`)

**Daniel lo probó en su teléfono y quedó aprobado.** El detalle completo de la causa raíz —el `click` sintético de Chrome Android cayendo sobre el velo recién montado y cerrando la tarjeta en el mismo gesto que la abría— está en el bloque del 6 sep, junto con los cinco candidatos que quedaron descartados con evidencia. **Este bug estuvo abierto desde el 31 ago y le pegaba al 100% de los testers**, porque la beta es toda Android y Daniel prueba en iPhone.

**La lección que vale para la próxima, y que ya se llevó dos bugs:** cuando algo "no hace nada" al tocar, hay que preguntarse **si no está pasando dos veces**. Acá el tap abría y cerraba; el síntoma anotado en agosto ("el chip se ilumina pero la tarjeta no abre") describía la misma falla vista un instante antes.

### 2. 🚀 FRENTE 1 — la re-postulación a producción se envió (ver el bloque del Frente 1, arriba)

Enviada el **8 sep 9:28**, dos días antes de lo previsto, con los **3 requisitos tachados** por Google y la medición del 7 sep (**13 activos de 23 con cuenta**) como respaldo. Respuesta esperada en ≤7 días. **Mientras revisan no se toca Play**, pero el código quedó libre: por eso la cola de producto arrancó el mismo día.

### 3. 🟡 PIEZA 1 DE LA COLA — el marcador de edad del Cerebro, DESPLEGADO (`7104044` + `bd8b989` + `af0d611`)

**Feedback de Diego (`H-02`):** la barra de edad se movía a años enteros y la edad real del hijo —2 años y 3 meses— se perdía **sin camino de vuelta**.

- **La causa:** el decimal solo vivía en el estado `edad`; el input tiene `step="1"` y su `onChange` lo pisaba con un entero. El efecto que adopta la edad real está detrás de `tocado`, que **nunca vuelve a bajar**.
- **La solución:** la **foto del hijo cuelga bajo el riel**, en su edad, y **tocarla restaura la edad decimal completa** y baja `tocado`, así el rótulo vuelve a decir los meses. Sin foto va la **inicial del nombre**. Si el hijo todavía no cargó, o no tiene fecha ni edad guardada, **el marcador no se dibuja**: estaría apuntando al respaldo de 4 años, que no es la edad de nadie.
- **El `step="1"` se queda, y es decisión tomada:** la matriz de contenido está indexada por años enteros y `porTope`/`rotuloNota` usan `Math.floor`, así que arrastrar hasta los meses no cambiaría nada de lo que el padre ve, y rompería la alineación de perilla y relleno que ya costó una ronda.
- **Dos ajustes que salieron del QA de Daniel, ya aplicados:**
  - **`bd8b989` — el desfase.** El marcador iba en la edad decimal y la perilla en el entero: quedaban ~5px corridos al volver, y eso se leía como defecto, no como precisión. Ahora el marcador va en la **misma grilla que la perilla** (edad redondeada) y coinciden **al píxel**. Lo que no se redondea es lo que restaura el toque.
  - **`af0d611` — el tick asomaba.** La línea que une riel y foto arrancaba **dentro** del riel, confiando en que el riel la taparía por `z-index`; en Android eso no se cumplió y asomaba sobre la línea. Ahora **nace en el borde inferior del riel** y nunca lo pisa.
- **Cómo se verificó:** harness a 390px en claro y oscuro, midiendo geometría en cinco edades (0, 2a3m, 9a11m, 17a1m y 18) y, para el tick, **leyendo los píxeles de su columna** con el riel sin rellenar y la perilla lejos. Resultados: **desvío perilla-foto de 0px en todas las edades**, área de toque de 40px con círculo de 28, **22px de aire hasta los chips sin choque**, marcador siempre dentro de pantalla, y **cero índigo por encima del riel o dentro de su franja**.
- ✅ **QA de Daniel APROBADO, incluido el tick** (`af0d611`). **La pieza 1 queda CERRADA.**

### 4. ✅ PIEZA 2 DE LA COLA — la micro-respuesta al "¿Cómo te sentiste tú?", con MEMORIA — CERRADA CON QA (`d3edf44` + `b06d2ae` + `e11d82f` + `9ca50fc`)

El campo de la reflexión es **lo único de la app que es solo del padre**, y hasta hoy caía al vacío: escribía cómo se sintió y no recibía nada. Ahora, al guardar, aparecen debajo **1 o 2 frases que acompañan ese sentimiento**, y quedan guardadas para verlas al reabrir el episodio.

- **La regla dura es el marco anti-vergüenza, y es la razón de ser de la pieza.** El padre acaba de admitir algo incómodo —que gritó, que no pudo más—: si la respuesta lo corrige, lo felicita o le aconseja, **deja de escribir para siempre** y se pierde el campo entero. El system lo prohíbe explícitamente: acompañar y nunca evaluar, UNA sola perspectiva, sin "deberías" ni "la próxima vez", sin hablar del desarrollo del hijo.
- **Migración 016** (`episodios.reflexion_respuesta`), corrida y verificada por Daniel.
- **Backend con LISTA BLANCA de modelos** (`claude-sonnet-4-6`, `claude-haiku-4-5`), con el de siempre por defecto. No es ceremonia: el body lo arma el navegador, así que sin lista blanca cualquiera con un token de sesión válido podría pedir el modelo que quisiera y facturárselo a la cuenta. Lo que no esté en la lista cae al default **en silencio**, así una versión vieja del cliente sigue funcionando.
- **`generarRespuestaReflexion`**, calcada de `generarReflexionCheckin`: **Haiku** (`claude-haiku-4-5`), `max_tokens` 120 y **system propio corto**. El clínico del backend son ~6.700 tokens de andamiaje sobre desarrollo infantil que acá no aplica —no se interpreta al hijo, se acompaña al adulto— y además **empujaría justo al consejo que esto tiene prohibido**. Haiku cuesta un tercio y sobra para dos frases: del orden de **8 centavos de dólar al mes** con 20 testers.
- 🔒 **UN SOLO DISPARO POR EPISODIO, con candado en la propia columna.** Si el padre corrige su texto, el texto nuevo se guarda pero **la respuesta no se regenera**. Sin eso, cada corrección gastaría una llamada de la **cuota diaria de 20 por usuario**, y reescribir se volvería la forma de pedir otra respuesta —una máquina tragamonedas—.
- **La respuesta va en un UPDATE APARTE** del que guarda el texto, por la misma razón que `orientacion_zona`: si viajaran juntos y la columna faltara, PostgREST rechazaría la fila entera y **se perdería lo que el padre escribió**. La respuesta es un extra; su texto no. Si la IA falla, no se dibuja nada.
- 🔴 **UN AJUSTE DE TONO QUE SALIÓ DEL QA Y VALE COMO LECCIÓN** (`b06d2ae`): ante *"fue agresiva y me sentí pésimo"*, la respuesta devolvió *"ser recibido con tanta agresividad duele… de alguien tan cercano como {nombre}"*. **Tomó el adjetivo que el padre usó en caliente sobre su hija y se lo devolvió como si fuera una descripción de ella**, y de paso dramatizó. **Acompañar al adulto NO es calificar al niño.** Se agregaron dos reglas duras: nunca calificar al hijo ni repetir adjetivos sobre él —el hijo aparece solo por su nombre, como contexto, **nunca como sujeto de la frase**—, y sin dramatizar, sobrio y concreto.
- ✅ **QA de Daniel APROBADO**, con el tono verificado contra el caso duro.
- 🧠 **PIEZA 2b — LA MEMORIA (mismo día).** Sin esto cada reflexión se leía como la primera. Ahora el prompt lleva las **5 reflexiones anteriores del padre** (fecha relativa —"hace 3 días", "hace 2 semanas"—, tipo de momento y texto recortado a 200 caracteres), y el system le pide **nombrar el hilo de forma concreta** cuando lo hay (cuántas veces, en qué tipo de momentos), **no inventarlo** cuando no lo hay, y **nunca** referirse al pasado en la primera reflexión. `max_tokens` 120 → 160 y el tope de palabras 45 → 55, para que el hilo quepa. 🔒 **El filtro por autor no es opcional:** `state.episodios` trae los episodios de **toda la familia**, y la reflexión es lo único privado de quien la escribe; sin filtrar por `userId`, a un padre le llegarían de vuelta **las reflexiones íntimas de su pareja**. Si no hay `userId`, no se manda memoria. **Sin consulta nueva a la base:** el contexto ya tenía todo. **Costo:** ~330 tokens extra en el peor caso (5 reflexiones al tope), ~230 típico, más ~90 del system; con Haiku, cuatro diezmilésimas de dólar por llamada.
- 🔴 **SEGUNDO AJUSTE DE TONO, el de la memoria** (`9ca50fc`): con el hilo a la vista aparece una tentación nueva — **narrarlo como un deterioro** ("cada vez más", "te deja más desarmado", "peor que antes"). **Nombrar que algo se repite le sirve al padre; decirle que va empeorando lo hunde**, y encima suele ser una lectura que el dato no sostiene. Dos reglas más: después del hilo, la segunda frase **sostiene, nunca amplifica**; y **una perspectiva no es un consejo** — reencuadra lo que siente, no dice qué hacer.
- ✅ **QA de Daniel APROBADO: la memoria nombra el hilo REAL.** La pieza 2 (respuesta + memoria) queda **CERRADA**.
- ⬜ **CALIBRAR CON 10 RESPUESTAS REALES DE TESTERS ANTES DE TOCAR EL PROMPT DE NUEVO.** Ya van **tres ajustes de tono seguidos, cada uno contra un caso único** (el adjetivo sobre la hija, la dramatización, la espiral). Así se llega a un prompt sobreajustado: arregla lo último visto y rompe lo demás. 👀 **Patrones a vigilar: la muletilla "especialmente cuando" y cualquier lectura de deterioro.**
- **La orientación del hijo no se tocó:** prompt, streaming y guardado quedaron igual.

### ⏭️ Próximo

1. **Seguir la cola priorizada:** sigue la **pieza 3 — el motor de rasgos alimentado también por hitos** (es pieza propia y necesita su plan antes de tocar código).
2. **Juntar 10 respuestas reales** de la pieza 2 antes de volver a tocar su prompt.
3. **Frente 1:** esperar la respuesta de Google, buscándola **en Play Console**.
4. **Onboarding:** queda la pasada con **cuenta nueva de verdad**.
5. **Cola del Cerebro:** QA en tablet Samsung y el enlace del paso 8.

---

## Sesión domingo 6 septiembre 2026 — **El tap al cerebro en Android: causa raíz encontrada y arreglada, desplegada y esperando el QA de Daniel**

**🎯 Un bug de dos semanas, con la causa confirmada por experimento. Sigue ABIERTO hasta que Daniel lo pruebe en su teléfono.**

### 1. 🟡 DESPLEGADO, PENDIENTE QA EN ANDROID — el tap al modelo 3D del Cerebro (commit `7cdd878`)

- **⚠️ La descripción vieja del síntoma era incorrecta, y esto la corrige.** El archivo decía *"el raycast detecta y el chip se ilumina, pero la tarjeta no abre"*. **La evidencia del 6 sep dice otra cosa: no pasa nada de nada — ni se ilumina el chip ni abre la tarjeta.** Y el raycast nunca fue el problema: **la tarjeta SÍ abría, y se cerraba sola en el mismo gesto**, tan rápido que no se alcanza a ver. El chip tampoco quedaba encendido porque su clase `chipOn` depende de `abierta && zonaVista === slug` (`CerebroContenido.jsx:247`), y `abierta` volvía a `false` al instante. Las dos observaciones (la del 31 ago y la de hoy) son la MISMA falla vista en dos momentos.
- **🔴 CAUSA RAÍZ CONFIRMADA: un tap táctil produce DOS activaciones.** Primero el `pointerup`, que hace el raycast y abre la tarjeta (`EscenaCerebro.jsx:419`). Después el navegador fabrica del mismo gesto un **`click` sintético**. Chrome en Android resuelve el destino de ese click **contra el árbol del momento en que lo dispara**, y para entonces el velo de la tarjeta ya está montado: es `position: fixed; inset: 0` con `pointer-events: auto` apenas abre (`.veloOn`, `CerebroPage.module.css:412`), o sea que está justo encima del punto donde estaba el dedo. El click le llega a él y su `onClick={cerrar}` (`CerebroContenido.jsx:290`) **cierra la tarjeta en el mismo gesto que la abrió**.
- **Por qué en iPhone no pasa:** Safari le asigna a ese click el destino del **toque inicial** (el canvas). Chrome vuelve a resolverlo al dispararlo. *(Única parte no verificada en esta sesión: no hay Safari en este equipo.)*
- **Por qué los chips nunca fallaron:** son botones y reciben **un solo** click; el velo se monta después de que ese click ya se despachó. Medido: chip → `CLICK_NAVEGADOR destino BUTTON` → `ABRIR`, y la tarjeta queda abierta. Canvas → `TAP_ZONA` → `ABRIR` → `CLICK_NAVEGADOR destino _velo_ _veloOn_` → `CERRAR`.
- **⛔ El sospechoso anotado el 31 ago quedó DESCARTADO.** El fix de la deriva (`d46366c`) no tiene nada que ver. También se descartaron con evidencia: `touch-action` (el canvas sí lo tiene en `none`), `pointercancel` interrumpiendo el gesto (no llega ninguno), el conteo de dedos (vuelve a cero después de cada toque) y el orden de listeners (el `pointerup` llega siempre).
- **El arreglo:** se suprime el click sintético **en su origen**, con `preventDefault` en un listener de `touchend` sobre el canvas (`{ passive: false }`, con su cleanup al desmontar). Nada del canvas necesita ese click, y el velo conserva intacto su comportamiento para los clicks de verdad. **De paso el umbral que separa un tap de un arrastre subió de 9px a 12px:** un dedo real en Android recorre más que uno en iPhone y se estaban descartando taps legítimos.
- **Cómo se comprobó (mismo método que el bottom sheet):** harness en Chrome real vía CDP con el **componente real y el velo real**, toque emulado de Android a 390px (`Input.dispatchTouchEvent` + `setTouchEmulationEnabled`, nunca mouse). Primero se **reprodujo** el bug, después se verificó el arreglo. Barrido del umbral: 0, 8, 10, 11 y 12px **abren**; 14 y 20px **giran**. Con mouse en escritorio el click va al canvas y la tarjeta abre igual. Todos los archivos temporales del harness se borraron; `git status` quedó limpio.
- **Desplegado:** el cerebro viaja en su propio chunk de carga diferida. Verificado bajando `assets/EscenaCerebro-CZov6bx5.js` desde **huella.lat**: trae el `touchend` y el umbral de 12, sin rastro del 9 viejo.

**⬜ LOS 6 PASOS DEL QA QUE FALTA (Daniel, en Android, claro y oscuro):**

1. Abrir `huella.lat/hijo?tab=cerebro` y tocar una zona del modelo (p. ej. la parte baja). **La tarjeta debe abrirse y QUEDARSE abierta, y el chip de esa zona debe quedar encendido.**
2. Tocar el velo oscuro fuera de la tarjeta: **debe cerrarse**. Repetir con el botón de la equis.
3. Tocar una zona **moviendo un poco el dedo**, como pasa naturalmente: debe abrir igual.
4. **Arrastrar para girar** el cerebro: NO debe abrirse ninguna tarjeta, y la deriva debe volver sola a los pocos segundos.
5. Tocar **dos zonas distintas seguidas**, sin cerrar entre medio: la tarjeta debe cambiar de zona.
6. Comprobar que **los chips de abajo** siguen abriendo su tarjeta.

Repetir el paso 1 en las **dos rutas**: el tab del hijo y `huella.lat/cerebro`.

### 2. ✅ ONBOARDING EN ANDROID — QA APROBADO por Daniel

La **pasada en Android real con `?onboarding=1`** quedó **aprobada**: A5 con sus dos slots de foto y el acto C con sus 6 filas caben sin scroll en 390px. De esa pasada salieron **dos ajustes al acto C, ya en producción (`d30db3c`)**:

- **Copy del título.** *"Lo que acabas de leer viene de acá"* atribuía el texto del acto B a los 6 autores de la lista, y eso no es cierto: son una **muestra del corpus** elegida por la edad. Pasó a **"Las voces detrás de Huella en la etapa de {nombre}"**, con el nombre del hijo del acto A. La línea de "más de 80 expertos" se mantiene.
- **Marca de agua.** El escarabajo del acto C iba abajo-izquierda y quedaba **casi entero detrás de la tarjeta de autores**. Se movió arriba-derecha, al hueco entre el título y la tarjeta, con mismo tamaño y opacidad (`--opacity-watermark`) y rotación propia, siguiendo el patrón por paso. Medido en 390x844 con título de tres líneas: la cabeza arranca ~13px bajo el título, no se cruza con el texto, y el acto sigue **cabiendo sin scroll**.

### ⏭️ Próximo

1. **El QA del tap en Android** (los 6 pasos de arriba). Hasta que Daniel lo corra, el bug **no está cerrado**.
2. **Onboarding:** queda la pasada con **cuenta nueva de verdad** (la de Android ya está aprobada).
3. **Cola del Cerebro:** QA en tablet Samsung y el enlace del paso 8.
4. **Decisión pendiente:** "+ Registrar hito" desde el filtro Logros del Historial (sí/no).

---

## Sesión viernes 4 septiembre 2026 — **El bottom sheet dejó de quedar debajo de la barra: se arregló la raíz en PageTransition, y de paso quedaron cubiertos los 9 overlays sin portal**

**🎯 Un bug cerrado con causa raíz y QA en PC y Android real, dos tareas descartadas por contexto viejo, y una investigación que terminó en decisión, no en código.**

### 1. ✅ BUG CERRADO — los overlays `fixed` quedaban tapados por la barra de navegación en Android

- **Síntoma (reproducido por Daniel en celular):** en `huella.lat/hijo?tab=rutina`, al tocar "Agregar bloque" se abría el sheet "Nuevo bloque", pero la barra Inicio / Registrar / Tú se dibujaba **encima** y tapaba el botón naranja de guardar (asomaban ~4px). El scrim parecía cubrir solo el contenido: sobre el header mocha no se nota, y la barra se pintaba por encima.
- **Causa raíz:** las tres clases de entrada de `PageTransition` (`.pageVisible`, `.pageSlideRight`, `.pageSlideLeft` en `Layout.module.css`) usan `animation-fill-mode: both`. Chrome trata al wrapper con animación **terminada-pero-rellena** como si siguiera animando: con `fadeIn` queda un **stacking context** propio aunque la opacidad ya sea 1; con `slideIn` queda `transform: translateX(0)`, que además lo vuelve **containing block** de los `position: fixed`. El `z-index: 200` del overlay valía solo dentro del wrapper, y la barra (`position: relative; z-index: 1` en el contexto raíz) le ganaba. Framer Motion no participa; el `filter` del retrato está en la foto, no en un ancestro.
- **Cómo se confirmó:** harness con el mismo DOM y CSS en Chrome real vía CDP a 390px, con un hijo animado para probar el filtro de burbujeo. Antes del fix, el toque sobre la barra lo recibía la barra en los tres casos. Después, lo recibe el botón Guardar del sheet y el overlay cubre la ventana completa en los tres.
- **Fix, en la raíz (commit `d7ce582`):** al terminar la animación (`onAnimationEnd`, ignorando eventos de hijos con `e.target === e.currentTarget`), la clase animada se reemplaza por `.pageLlegado` (opacidad 1, `animation: none`, `transform: none`). El reset por ruta es automático porque el Layout monta `PageTransition` con `key={location.key}`. La entrada se ve idéntica. Cero hex nuevo.
- **Alcance:** cubre a los **9 overlays fixed sin portal** (RutinaDiaria, Leyenda y Detalle de medallas en Perfil, ModalPuenteCiclo, la hoja de EstrategiaDetail, EstrategiaNueva, RegistroConversacional, consejoBubble y CerebroPage). Los **4 con `createPortal`** (UpgradeModal, CerrarPatronModal, CerebroContenido y la capa post-guardado de RegistroPage) no cambian. **La regla técnica 1 quedó actualizada** más arriba.
- **QA de Daniel:** aprobado en PC y en Android real: el scrim cubre la barra, el botón de guardar se ve completo y el bloque de rutina se guarda.
- **Descartado a propósito:** el botón sticky al pie del sheet. Con el teclado abierto, Chrome en Android no achica el viewport de layout, así que un sticky quedaría igual de tapado por el teclado. Es otro problema, menor, y hoy se resuelve cerrando el teclado.

### 2. 🔎 Dos tareas que llegaron con contexto anterior al 15 ago, verificadas y descartadas (sin código)

- **"+ Registrar" en el header de Logros → vista de hito:** la pantalla Logros no existe desde `89b1497` (15 ago, fusionada en `/historial`; `/hitos` redirige). `NuevoPage` sigue leyendo `location.state.vistaInicial === 'hito'`, pero hoy nadie se lo manda. Si se quiere, el lugar natural sería el filtro "Logros" del Historial: decisión de Daniel.
- **Tuerca sin función en Estrategias:** `HeaderMocha` no tiene tuerca desde el mismo commit (era código muerto); verificado también en el bundle de producción. La única tuerca de la app está en el retrato del hijo (`RetratoSendero`) y navega a `/perfil`.

### 3. 🔎 "Marco aplicado" en los episodios — NO es un bug: se quitó a propósito el 27 ago

- **Verificado en código y git:** los episodios **nunca** guardaron el marco en una columna (solo `patrones.orientacion_ia.marco_aplicado` existe). Antes del 27 ago, cuando aparecía, era una línea que el modelo escribía por inercia dentro de `orientacion_ia`; ningún prompt de episodios la pidió nunca. Desde `6cf1302` (27 ago, paso 8 del Cerebro) el prompt de `analizarEpisodio` lo **prohíbe** explícitamente y `OrientacionSecciones` filtra cualquier línea "Marco aplicado" (`esLetraChicaDelModelo`). Todos los caminos (normal, streaming, segundo plano del onboarding) devuelven `{ texto, zona }` sin marco, por diseño.
- **Los "fixes anteriores" no eran de esto:** `61015cb` (KEYWORDS_DUELO) es la Acción Rápida; `a5c1595` y `2b2e28f` son el marco de **patrones**; `9757ccb` eliminó el render de la línea; `6cf1302` la prohibió. La historia fue en una sola dirección.
- **DECISIÓN DE DANIEL (mismo día): el marco VUELVE a los episodios, por el camino determinístico. IMPLEMENTADO Y CERRADO CON QA** (commit `c801cf1`; QA de Daniel aprobado en claro, oscuro y Android, con un episodio nuevo y uno anterior al 27 ago):
  - **`marcoDelEpisodio({ episodio, hijo })`** en `src/services/anthropic.js`: devuelve `{ autor, lente }` **siempre**. Toma `accionRapida.autor` + `accionRapida.dimension` si existen y el autor está en el banco `AUTORES`; si no, infiere la dimensión con `inferirDimensionCentral` y elige autor con `seleccionarAutor` (sin anti-repetición). Un autor guardado fuera del banco se descarta. Sin IA, sin prompt nuevo, sin columna, sin migración.
  - **El pie es el MISMO componente que patrones:** `PieCientifico` ganó un prop `etiqueta` (default `"Marco"`, en episodios `"Lente"`). Copy en episodios: *"Esto se apoya en evidencia del desarrollo infantil. No es un diagnóstico. Lente: Daniel Siegel · Desarrollo cerebral."* "Lente" y no "Marco" por honestidad: es el lente con que Huella miró el episodio, no el autor que escribió el texto. Mismo estilo (11px, itálica, muted, separador de 1px); el `.disclaimer` propio de `OrientacionSecciones` se eliminó.
  - **Dónde se ve:** en la pantalla post-guardado (`OrientacionSecciones`, que ahora recibe `episodio` e `hijo`; RegistroPage busca el episodio en el contexto por id, así el pie ya tiene la Acción Rápida) **y en el panel "Orientación de Huella" del Historial** (`OrientacionIA`, con el episodio crudo del contexto). Lo del Historial no estaba en el pedido literal, pero es el único lugar donde se abre un episodio viejo, y el QA pedía justamente uno anterior al 27 ago.
  - El comentario desalineado de `PieCientifico.jsx` quedó corregido. El prompt de `analizarEpisodio` y la base no se tocaron.

### 📊 Frente 1 — Google Play: medición del 4 sep (la corrió Daniel en Supabase)

**12 activos de 22 con cuenta** (activo = al menos un episodio o hito desde el 27 ago). El 2 sep eran **6**: el WhatsApp 1:1 del 2 sep duplicó la actividad. El resto de los 22: **7 registrados sin actividad desde el 27 ago**, **3 con perfil de hijo pero cero registros**. Además **2 nunca se registraron** (de los 24 de la lista oficial). **La ventana de 14 días cierra ~10 sep**; ahí va la re-postulación a producción. Fuente de verdad de testers, códigos e instalaciones: el Sheets de Daniel.

### ⏭️ Próximo

1. **Decisión pendiente:** "+ Registrar hito" desde el filtro Logros del Historial (sí/no). El lente en episodios ya quedó cerrado con QA.
2. **Onboarding:** las dos pasadas de QA que faltan (Android real con `?onboarding=1`, y cuenta nueva de verdad).
3. **La cola del Cerebro:** bug del tap en Android, QA en tablet Samsung, enlace del paso 8.

---

## Sesión jueves 3 septiembre 2026 — **El onboarding quedó completo: bloque 3, acto C y la pasada visual de Design, todo en producción**

**🎯 Tres cierres grandes y uno de fondo.** Todo compilado, verificado en el bundle de producción y con QA visual de Daniel en PC (claro y oscuro) antes de dar el día por cerrado.

### 1. ✅ Bloque 3 — el texto del acto B es el PRIMER EPISODIO real del hijo

Es el principio rector hecho código: todo cuelga del momento, y el primer momento ya no se pierde.

- **Dónde:** `onboardingPersistor.crearPrimerEpisodio`, al final de `persistirPerfilOnboarding`, con el `hijo_id` que acaba de devolver la RPC. **No** usa `addEpisodio` del contexto porque ese inserta con `state.hijoActivoId`, y el hijo recién creado todavía no está en el contexto.
- **Cero segundos extra para el padre:** el INSERT sale al tiro con `tipo 'otro'` e `intensidad 3` (no hay pantalla donde la elija). La **extracción** (`extraerEpisodio`, la misma del registro conversacional) corre en segundo plano y hace UPDATE de tipo/emoción/contexto; **encadenada** a ella corre la **orientación completa** (`analizarEpisodio`, la misma del registro normal) y escribe `orientacion_ia` y `orientacion_zona` en dos UPDATEs separados (patrón de la migración 014).
- **El Home se refresca solo:** `persistirPerfilOnboarding` acepta `onEpisodioActualizado`, que el Layout conecta a `reloadData`. Se dispara al cerrar cada etapa de fondo, solo si su UPDATE salió bien. Sin realtime: es un reload diferido por etapa. El padre ve el episodio como "Otro", luego con su tipo, luego con la orientación, sin tocar nada.
- **Reglas:** "Saltar este paso" → no se crea nada. Modo ensayo (con o sin `ia=1`) → corta antes del persistor, cero episodios, cero llamadas. Cualquier fallo del bloque se loguea y el onboarding termina igual: **el padre nunca ve un error por esto.**
- ✅ **Migración 015 — `episodios.origen` — CORRIDA Y VERIFICADA POR DANIEL HOY** (`origen | text | YES`). Columna nullable con CHECK (`null` = registrado a mano, `'onboarding'` = nació del acto B). El código la escribía en un UPDATE aparte y la leía con `?? null`, así que se desplegó antes que el SQL sin romper nada.
- **Costo en llamadas:** 2 de las 20 diarias (extracción + orientación), más la del primer encuentro: 3 en el primer minuto de una cuenta nueva. Si el 429 llega, el episodio queda guardado sin orientación, como uno manual donde la IA cayó.

### 2. ✅ Acto C — la pantalla de cierre de confianza

Entre la respuesta del acto B (o su "Saltar") y el Home. `OnboardingCierre.jsx`.

- **Copy:** eyebrow "Antes de entrar", título "Lo que acabas de leer viene de acá", 6 filas "Autor · Lente" en una tarjeta con la misma piel que la respuesta del acto B, la línea **"Huella se apoya en el trabajo de más de 80 expertos en crianza y desarrollo infantil y juvenil."** y un único CTA **"Entrar a Huella"**. Sin "Saltar".
- **Autores por edad:** `autoresParaCierre(edad)` en `anthropic.js` prioriza a los autores del banco que **el marco de esa edad nombra** (descontando `TEMAS_CONTEMPORANEOS`, que se anexa a los cuatro marcos y hacía pasar por transversales a Wolfelt, Prizant y Aron), descarta los de piso etario y a esos tres de dimensión específica, y rellena con los más transversales. Si el padre saltó el acto B, van los transversales. Probado contra el banco real por tramo: 2-6 → Siegel, Perry, Greene, Shanker, Maté, Faber & Mazlish.
- **El persistor corre detrás:** `onComplete` se dispara al **salir** del acto B y el acto C tapa la espera. El CTA entra al instante si ya terminó; si sigue guardando, muestra "Guardando…" y entra solo al terminar; si falló, aviso + "Reintentar". El Layout separó `onComplete` (solo persiste + reload) de `onEntrar` (cierra), con un latch `onboardingEnCurso` para que el `reloadData` —que trae el hijo y pone `yaTieneCuenta` en true— no desmonte el onboarding antes de tocar "Entrar".
- **El corpus, contado de verdad:** la primera versión decía "más de 20" contando solo el banco `AUTORES` (23). Se contó el corpus completo que calibra a la IA (banco + los cuatro marcos de `marcoEdad` + `TEMAS_CONTEMPORANEOS` + la lista cerrada de patrones), deduplicado y sin títulos ni organizaciones: **82 autores e investigadores reales distintos**. Viven en `AUTORES_CORPUS` (lista mantenida, no regex: sobre la prosa un regex confunde títulos con personas) y el N sale de `.length`, redondeado a la decena inferior en código.

### 3. ✅ Pasada visual completa según los mockups de Claude Design

Solo estilo; flujo, copy, campos y lógica intactos. Un commit, 12 archivos, cero hex/rgba en los componentes (verificado con grep).

- **Tokens nuevos en `index.css`** (claro + override oscuro): `--color-strawberry-bg`, `--color-strawberry-text` (porque `--color-strawberry-dark` no cambia en oscuro), `--opacity-watermark` / `-soft` / `-softer`, `--color-cta-disabled-bg` / `-text`, y la escala mínima `--space-6/7/8`. Los seis oscuros del mockup ya tenían equivalente real; no se duplicó ninguno.
- **Marca de agua:** `Logo.jsx` ganó `soloSimbolo` (viewBox recortado al escarabajo; el header sigue igual). `OnboardingMarcaAgua` con 8 variantes de posición/rotación (A1 arriba-der … B1 lateral izq, B2 arriba-der, C abajo-izq), tinte mocha al 7%/8%, siempre detrás del contenido.
- **Acto A:** progreso en **5 segmentos** tangerine (local; `ProgressBar` sigue igual para las otras tres pantallas), título 28/1.15, sub 15/1.55, campos y selects de **56px** / borde 1,5px / halo tangerine, chips de género de 48px con punto tangerine, pill "Opcional" strawberry, slots de foto de **136px** punteados strawberry (los dos).
- **Acto B:** textarea 210px radio lg, **sin barra de progreso** en la carga, tarjeta de respuesta radio lg / 22px / sombra md, idea en Fraunces itálica 17, lente como **pill pistachio**.
- **Tres choques de contraste resueltos a favor del código**, con Daniel de acuerdo: eyebrow en `--color-eyebrow` (5,16:1 vs 3,9:1), chip seleccionado con texto `--color-text`, y CTA con `--gradient-cta-registrar` (el botón oficial del Home) en vez de tangerine plano.
- **Un cambio de forma en el copy, avisado:** el eyebrow de A5 era "Fotos · opcional" y ahora es "Fotos" + pill "Opcional". Y la textarea del composer quedó en 16px, no 15: bajo 16 iOS hace zoom al enfocar.

### 4. Lo que se consolidó del día anterior

El primer encuentro con contexto real (nombre, edad, género del acto A) y la firma "Autor · Lente" sin comillas, y el banco de autores en afirmativo, siguen tal cual y ahora conviven con el acto C, que los muestra.

### ✅ QA de Daniel

**Onboarding completo aprobado en PC, claro y oscuro.** Lo que falta está en la cola viva de PENDIENTES: la pasada en **Android real** (A5 y acto C sin scroll en 390px) y una pasada con **cuenta nueva de verdad** (primer episodio en el Home con tipo y orientación; las dos fotos guardadas).

### Estado del onboarding al cierre

**Todos los bloques (1-5) implementados y en producción.** Ya no hay "Saltar" en el acto A (los datos del hijo son lo que hace funcionar la app); el "Saltar este paso" del acto B **se mantiene a propósito**. Modo ensayo `?onboarding=1` (y `&ia=1`) muestra los tres actos sin escribir nada.

### ⏭️ Próximo

1. **Viernes 4 sep — medición de testers:** correr la query de actividad sobre los 24 y cerrar la ventana.
2. Después, **la cola del Cerebro**: bug del tap en Android, QA en tablet Samsung, y el enlace del paso 8.

---

## Sesión miércoles 2 septiembre 2026 — **El onboarding quedó cerrado en cinco frentes, y el primer encuentro dejó de inventar citas**

**🎯 El día fue del onboarding entero, no de un bug suelto.** Cinco cosas cerradas, todas con QA visual aprobado por Daniel antes de commitear.

### 1. ✅ Modo ensayo — `?onboarding=1`, y `?onboarding=1&ia=1`

Se puede abrir el onboarding completo sobre una cuenta que YA está armada, para QA visual, **sin escribir nada en la base**: el `onComplete` del `Layout` corta antes del persistor, así que no ocurre ni el upload del avatar a Storage, ni el upsert a `perfiles`, ni los dos `upsert_family_child`. Un chip discreto y siempre visible dice en qué modo se está, y toca para salir sin completar los 5 pasos.

**La variante `&ia=1` agrega UNA sola diferencia:** el acto B llama de verdad a Anthropic con el nombre, la edad y el género recién escritos, en vez de pintar el fallback local. Sirve para revisar la respuesta REAL del modelo antes de soltarla a los testers. El chip pasa a decir `modo ensayo · IA real · salir`. Lo único que sí queda escrito es la fila de `api_llamadas` del backend, que es solo el contador de uso — decisión tomada a ojos abiertos.

### 2. ✅ Selector de fecha de nacimiento, unificado en tres pantallas

La fecha pasó a tres selects (Día / Mes / Año) en un componente único, `SelectorFechaNacimiento`, y `HijoPage` y `PerfilPage` lo adoptaron. Antes cada pantalla tenía su propia forma de pedir lo mismo.

### 3. ✅ Paso 5 — las dos fotos, con compresión

El último paso captura la foto del hijo o hija **y** la de quien cría, y las comprime antes de subirlas.

### 4. ✅ El primer encuentro dejó de ser ciego, y dejó de fabricar citas

**Contexto real:** el acto B ahora recibe `{ nombre, edad, género }` del acto A. El hijo todavía no existe en la base (se crea al cerrar el onboarding), así que la edad se calcula en el momento desde la fecha que se acaba de recolectar. Antes el primer encuentro respondía sin saber a quién le hablaba.

**🔴 Y se corrigió algo que era un problema de fondo, no de estilo:** la tarjeta mostraba el texto del banco `AUTORES` **entre comillas y firmado con el nombre del autor**. Eso es una cita textual, y no lo es: lo que sale del banco son articulaciones del enfoque de cada autor escritas en la voz de Huella. Entrecomillarlas ponía palabras en la boca de una persona real. Ahora se lee como lo que es —una idea— y se firma **"Autor · Lente"**, igual que la Acción Rápida. Se fue la itálica y se fue la línea suelta "Marco · …", porque la lente ES el marco y quedaba dicho dos veces. De paso, `Self-Reg` (la única lente en inglés del banco) pasó a **`Autorregulación`**.

### 5. ✅ El banco de autores, reescrito en afirmativo — 29 articulaciones

**El prompt le prohíbe al modelo negar para afirmar, y el banco lo rompía en sus propias frases** — que se muestran justo al lado de la respuesta del modelo, firmadas con el nombre del autor. Si el banco la rompe, la regla no se sostiene.

Auditadas las 23 entradas completas (~90 articulaciones), **reescritas 29**. Ejemplos: *"esto no es mal comportamiento, es estrés acumulado"* → *"esto es estrés acumulado pidiendo salida por donde puede"*; *"toda conducta del bebé es comunicación, nunca manipulación"* → *"…: es la única forma que tiene de llamarte"*.

**El criterio quedó anotado como comentario en el encabezado del banco**, para no re-litigarlo: se reescribe cuando la negación abre la frase, cuando la mitad negada nombra un juicio (maldad, flojera, manipulación, patología, debilidad, exageración, rebeldía) o cuando hay un absoluto sobre el desarrollo ("no puede aprender"). **Se conserva** el contraste entre dos prácticas adultas cuando no acusa a nadie — *"conectar primero, redirigir después"*, *"la pregunta correcta es ¿qué le pasó?"* —: ahí la negación ordena, no juzga. Quedaron 17 así, a propósito.

Y el prompt del primer encuentro se endureció en dos puntos que salieron del QA con IA real: **negar para afirmar** ahora abre el bloque de tono, con dos ejemplos y su versión afirmativa al lado; y se agregó **precisión sin absolutos** sobre el desarrollo, porque el modelo escribía "no tiene acceso a" o "todavía no tiene" cuando esas capacidades existen parciales y frágiles ("le cuesta", "se le va cuando está cansado").

### ✅ Lo que quedaba pendiente del onboarding — CERRADO EL 3 SEP (ver el bloque de esa sesión)

- ✅ **Bloque 3: el acto B guarda el episodio real.** Cerrado el 3 sep.
- ✅ **Acto C.** Construido el 3 sep.
- ✅ **Pasada visual de Claude Design.** Los mockups llegaron y se aplicaron el 3 sep.

---

## Sesión martes 1 septiembre 2026 — **El QA en Android tumbó el fix de la mañana, y el de la tarde lo cerró: el bug de la escalera quedó CERRADO con QA en las tres rutas**

**✅✅ QA EN ANDROID REAL APROBADO — EL BUG DE LA ESCALERA ESTÁ CERRADO.** Daniel probó **las tres rutas de voz** en el Android de prueba: el **registro conversacional** (la pantalla que había fallado), un **`VoiceTextarea` suelto** y la **reflexión por voz**. Las tres bien: el dictado queda **completo y sin repeticiones**. 🎖️ **Es el primer bug del proyecto que se cierra cumpliendo la regla nueva** — fix (`938b510`) **más** QA en Android, no solo harness verde. Y valió la pena exigirlo: **la primera vez que se declaró arreglado (`01ad956`) el harness estaba en 10 OK / 0 fallas y el teléfono lo desmintió igual.**

*(Nota de precisión: el commit del fix es **`938b510`**. En el cierre se mencionó `9be51cb`, que no existe en el repo — se deja el hash real para que la referencia no quede rota.)*

**Cómo quedó la cadena completa del día, que es la historia que vale guardar:**

| | Qué pasó | Cómo se supo |
|---|---|---|
| **Mañana** | `01ad956` — se mata el `+=` que sumaba la escalera | Query de un tester real: 77.685 caracteres |
| **Mediodía** | El harness da **10 OK / 0 fallas** y se declara arreglado | ⚠️ **Falso verde** |
| **Tarde** | **QA en Android: FALLA.** La escalera sigue viva | El teléfono de Daniel |
| **Cierre** | `938b510` — plegado por prefijo, no por clave | Reproducido en el harness, y **el QA en Android lo confirmó** |

**EL QA FALLÓ.** Daniel dictó en el Android de prueba, con la app cerrada del todo para descartar caché, y la escalera **seguía viva**: `me me gustaria me gustaria decir me gustaria decir que hoy hoy hablando hoy hablando con...`. La ruta fue el **registro conversacional** (la pantalla *"¿Qué pasó con Pascu?"*), y la burbuja del usuario mostraba la escalera. Hay screenshots.

**⚠️ PRIMERO, TRES COSAS QUE SE VERIFICARON Y DESCARTARON, PORQUE LA SOSPECHA INICIAL APUNTABA A OTRO LADO:**
- ⬜ **"¿Hay una segunda ruta de voz?" NO.** `grep` exhaustivo de `SpeechRecognition`, `webkitSpeechRecognition`, `useDictado`, `useVoz` y `useSpeech` en todo `src/`: **un solo archivo**, `VoiceTextarea.jsx`. **No existe ningún hook de dictado, y no existe `ReflexionVoz` ni nada equivalente.** Los **8** usos de voz de la app (RegistroConversacional, SelectorHabilidades, NuevoPage, PatronPage ×2, RegistroPage ×3) son **el mismo componente**. El conversacional llega ahí por `GrabadorVoz` (`RegistroConversacional.jsx:605`), que solo reenvía props.
- ⬜ **`RegistroConversacional.jsx:236` NO acumula nada raro.** Esa línea es `return todo` dentro de `recibirDelPadre`; el archivo **no toca `SpeechRecognition`**, no tiene índices ni sesiones. La escalera **ya llegaba armada** desde `VoiceTextarea`.
- ⬜ **NO fue caché: `01ad956` estaba en producción.** Verificado en el bundle servido (`assets/index-s3K0Xnms.js`): `el motor cerro solo` ✅, `techo-de-reinicios` ✅, `start tropezo` ✅, y los marcadores del código viejo (`Safari corto`, `cobrarInterim`) en **0**. Daniel dictó sobre el código nuevo.

**🎯 LA CAUSA REAL: EL FIX APLASTABA LA ESCALERA POR CLAVE, Y EL MOTOR REAL LA REPARTE EN CLAVES DISTINTAS.** El fix de la mañana guardaba cada resultado bajo `sesion:indice` y reemplazaba si la clave se repetía. Eso mata los peldaños que caen en la **misma** clave. En el Android de verdad caen en **claves distintas**, y ahí pasan enteros. Se reprodujo contra el bundle de producción, y hay **dos formas** de que ocurra:
- **Forma A — el índice avanza con cada parcial.** Una sola sesión, pero cada parcial estrena índice: claves `0:0`, `0:1`, `0:2`... → **298 chars de una frase de 52 (x5,7)**.
- **Forma B — una sesión nueva por cada parcial.** El motor cierra tras cada parcial y al reabrir re-reconoce desde el principio: claves `0:0`, `1:0`, `2:0`... → **245 chars (x4,7)**. **Y además explica el otro síntoma:** como nunca manda un final, el contador de reinicios no bajaba, el techo saltaba a los 9 rebotes y **la grabación se cortaba sola**. Los dos síntomas del reporte original, en la misma cinta.

**✅ EL FIX DE VERDAD — PLEGADO POR PREFIJO, INDEPENDIENTE DE LA CLAVE.** La clave la decide el navegador y cambia entre motores. Lo que **no** cambia es la forma del dato: **un peldaño siempre EMPIEZA CON el anterior**. Así que al armar el relato, si el pedazo siguiente empieza con el anterior, se queda el nuevo; si es una versión más corta de lo mismo, se queda el que ya estaba. La comparación normaliza mayúsculas, espacios y puntuación (el motor va agregando comas: `hoy` → `Hoy,`), pero **el texto que se guarda es siempre el original**. Con esto deja de importar cómo numere los resultados el motor de turno.
- **Además: el contador de reinicios se resetea con CUALQUIER texto capturado, no solo con un final** — eso mata la forma B.
- **Y un CANARIO:** si el relato armado queda más de 4 veces el pedazo más grande, se avisa por log. **Solo avisa, no corta.** Si aparece una tercera forma de escalera, nos enteramos por ahí y no por el episodio de un tester tres semanas después.
- **El log de finales ahora registra la FORMA cruda del evento** (`resultIndex`, cantidad de resultados, sesión). Es lo que faltó para cerrar este QA: sin eso, la causa quedó en conjetura y hubo que reproducirla a ciegas.

**✅ LA SUITE AHORA TIENE TRES VERSIONES, y la del medio es la lección:**

| | `antes` (original) | `qa-fallado` (`01ad956`) | `despues` |
|---|---|---|---|
| **Resultado** | **5 OK, 8 fallas** | **10 OK, 3 fallas** | **13 OK, 0 fallas** |

**`01ad956` pasa los 10 casos que la suite de la mañana probaba y se cae exactamente en los 3 nuevos.** Eso es el QA fallado, congelado en el repo (`VoiceTextarea.QA-FALLADO.jsx`) para que no se pueda volver a perder. La suite del arranque sigue en **24/24**.

**📌 LA LECCIÓN, CON NOMBRE PROPIO: LA CINTA LA ESCRIBIÓ LA MISMA CABEZA QUE ESCRIBIÓ EL FIX, ASÍ QUE LE DIO LA RAZÓN.** El fix de la mañana arregló la revisión sobre la misma clave, y después se escribió la cinta que valida justamente ese caso. **Diez OK verdes que probaban un solo escenario de tres.** Un harness no protege de un supuesto equivocado si el supuesto también está en el test. **Regla que queda: cuando un fix descansa en un supuesto sobre lo que hace un sistema externo (un motor de voz, una API, un navegador), el test tiene que incluir al menos una cinta que ese supuesto NO cumpla.** Escribir la cinta hostil primero, no la amable.

**📌 Y LA SEGUNDA, MÁS BARATA: el dato del dispositivo real llega tarde y caro.** Este bug se pudo diagnosticar entero desde el código —reproducido en el harness sin tocar el teléfono— pero recién **después** de que el QA en Android lo destapara. **Por eso la regla del QA en Android obligatorio se queda, y por eso el log de la forma cruda del evento entra ahora:** para que el próximo QA en Android traiga el dato en vez de una conjetura.

---

## Sesión martes 1 septiembre 2026 (primera vuelta) — **Se cazó el bug que convertía un dictado en 77.685 caracteres de basura, y quedó con red de seguridad automatizada**

**El reporte:** Vale, tester en **Android**, dijo dos cosas que parecían dos bugs distintos — que la grabación por voz "se le cortaba, muy corto", y que Momentos le mostró "un texto larguísimo, se veía horrible". **Eran el mismo bug.** Daniel también lo había visto en iPhone ("me registró solo un poco").

**LA CAUSA, EN UNA LÍNEA.** `VoiceTextarea.jsx` guardaba el relato con `transcriptRef.current += ... + r[0].transcript`. Ese `+=` es correcto solo si cada resultado del motor es un **pedazo nuevo**. Pero un motor de voz también reenvía el **mismo** pedazo con más palabras a medida que entiende mejor: *"momento"*, *"momento de"*, *"momento de ir"*. Sumando, el relato termina siendo **la escalera entera**.

**El daño medido:** un episodio de Vale del 31 ago con **77.685 caracteres**, `saltos_de_linea = 0`, con esta forma literal: `momento momento momento de momento de ir momento de ir a momento de ir a bañarse ... semana semana semana en semana en semana en que semana en que no`. **Y esos 77.685 caracteres viajaron enteros a una llamada de Anthropic**, porque `extraerEpisodio` no tenía ninguna guarda de largo.

**⚠️ EL HALLAZGO FINO, QUE COSTÓ DOS DIAGNÓSTICOS LLEGAR A ÉL — NO es "se rompió para todos el 12 ago".** El `+=` **acumula parciales desde el 12 ago** (commit `ca26c0a`, que cambió `rec.interimResults` de `false` a `true` para rescatar la última frase). Pero la **Query B por día lo desmintió como explicación completa**: entre el **12 y el 30 de agosto todos los relatos son normales** (largo máximo entre 52 y 661 caracteres), y la escalera **aparece recién el 31 ago, con una tester en Android**. La lectura correcta es: **en iPhone quedaba tapado por el dedupe, porque Safari manda pocos parciales por frase; en Chrome Android estalla, porque manda decenas.** Lo que hace explotar el bug no es el `+=` solo: es el **volumen de parciales**.

**🔎 Y EL HALLAZGO DE FONDO QUE HAY QUE NO OLVIDAR: Chrome Android IGNORA `continuous = true`.** El motor cierra después de **cada frase**. O sea que **reiniciar el reconocimiento en Android es el funcionamiento normal, no la excepción** — y todo lo que cuelgue del reinicio tiene que aguantar decenas de reinicios, no ocho.

**✅ ARREGLADO, CON LOS 6 PUNTOS DEL FIX UNIFICADO:**
1. **El relato se guarda por clave `sesion:indice`, nunca se concatena.** Si el motor reenvía ese mismo resultado con el texto crecido, **reemplaza** en vez de sumar. Guardar pasa a ser idempotente y **la escalera no se puede formar, dé lo que dé el navegador**. La `sesion` va en la clave porque al reiniciar los índices vuelven a empezar en 0.
2. **Los parciales viven en su propio mapa** y se suman al cerrar. **Desapareció toda la maquinaria de dedupe** (`interimCobradoRef`, el `endsWith`, el `slice`, `cobrarInterim`) — era el parche, y además **se comía una frase buena en cada reinicio**.
3. **El techo de reinicios cuenta rebotes SEGUIDOS SIN CAPTURAR NADA**, no reinicios totales: llega un final, el contador vuelve a cero. Antes, ocho pausas apagaban la grabación con el cuidador hablando.
4. **El reinicio crea instancia nueva, con 150ms de respiro y hasta 3 reintentos.** Antes reusaba el mismo objeto con `start()` sincrónico dentro de `onend`, y el primer `InvalidStateError` de Chrome mataba la grabación entera.
5. **Al llegar al techo se cierra directo**, sin pasar por `detenerGrabacion`. Ese camino llamaba a `stop()` sobre un motor ya muerto, y **un `stop()` sobre un motor detenido no vuelve a disparar `onend`**: el cuidador quedaba **8 segundos mirando "Procesando…"** para nada.
6. **Cinturón de seguridad doble:** tope de 8.000 caracteres en `VoiceTextarea` **y** guarda en `extraerEpisodio`. Cortar es peor que un relato completo, pero es muchísimo mejor que quemar una llamada de IA sobre un pegote.

**✅ VERIFICADO CON HARNESS AUTOMATIZADO — la suite DEMUESTRA el bug antes de demostrar el arreglo:**

| | ANTES | DESPUÉS |
|---|---|---|
| **Total** | **4 OK, 6 fallas** | **10 OK, 0 fallas** |
| Escalera (182 chars dictados) | **1.006 chars guardados (x5,5)** | **182 chars (x1,0)** |
| Android, 12 frases con pausas | **9 de 12** (se pierden 3 por el techo) | **12 de 12** |
| `stop()` sobre motor muerto | **1 vez** (8s de spinner) | **0** |
| Relato desbocado a la IA | **12.554 chars** | **cortado** |

Corre con `node verificacion/correr-voz.mjs despues` (0 fallas) y `... antes` (6 fallas: **si `antes` pasa entera, la suite dejó de probar algo**). **Hay que correrlo si alguien toca `VoiceTextarea`.** La suite del arranque sigue en **24/24** — se verificó que el cambio al shim de React no la rompió.

**🧰 El harness creció:** `shim/speech.js` es un `SpeechRecognition` falso manejado por **cintas**, y cada cinta es un navegador (escalera, Android, Safari, motor roto). Respeta las dos crueldades del de verdad: `start()` sobre un motor corriendo **tira**, y `stop()` sobre un motor muerto **no** vuelve a disparar `onend`. **Trampa nueva anotada en el README:** `e.results` del falso tiene que ser la lista **acumulada** de la sesión, no lo del evento — si no, un `resultIndex` mayor que cero deja el bucle sin recorrer nada y **la prueba pasa por no mirar**.

**🧹 De paso, higiene en Momentos (secundaria, no era la causa):** `.descLibre` en `EpisodioCard` no tenía recorte y sí tenía `white-space: pre-wrap`, así que pintaba el relato entero. Ahora se recorta a **3 líneas** con un **"Ver relato completo" / "Ver menos"**, el mismo gesto de "Ver orientación". **El texto en la base no se toca.** Contraste medido en los dos temas: **5,81:1 en claro y 6,23:1 en oscuro**. **NO se copió el verde de `.iaInline`** justamente porque sobre el blanco da **2,68:1** y el toggle es letra de 11px — eso queda anotado como deuda aparte.

**🧽 EL EPISODIO BASURA QUEDÓ BORRADO Y EL DAÑO ESTÁ ACOTADO.** Daniel eliminó por SQL el episodio `fbba55a2-cb24-4282-a0f0-9a34d0ddf94e` (el de Vale, **77.685 caracteres**, del 31 ago) y **lo verificó con un SELECT que devolvió 0 filas**. Su Historial quedó limpio. 🎯 **Y la Query A cerró la pregunta importante: era el ÚNICO relato roto de TODA la base.** O sea que el bug alcanzó a producir **un solo episodio dañado** antes de que lo cazáramos — el daño no se repartió entre testers y **no hay limpieza pendiente**.

**⚠️ EL BUG NO ESTÁ CERRADO TODAVÍA, Y ESO ES A PROPÓSITO.** El fix (`01ad956`) está en producción y verificado con el harness, pero **falta el QA en Android real de Daniel**: un dictado de **60-90 segundos con pausas**, que tiene que terminar en una transcripción **completa y sin repeticiones**. **Hasta que Daniel lo confirme en el teléfono, esto queda ABIERTO** — es la primera aplicación de la regla nueva número 1, y viene justamente de que el "cerrado" del 11 ago fue solo-iPhone. El paso a paso con USB + `chrome://inspect` y el criterio exacto de éxito están entregados.

**💚 Y LLEGÓ LA CONFIRMACIÓN DEL OTRO FRENTE: A VALE LA APP LE VOLVIÓ A FUNCIONAR.** Lo dijo ella misma, **sin haber hecho nada de su lado** — es el fix del arranque del 30 ago (`222b513`) funcionando sobre un caso real. Cierra el círculo de la regla del 30 ago: **los testers no reciben instrucciones técnicas, se arregla del lado nuestro y les llega solo.** La capa 2 de su caso (el "App not available" del link de opt-in) sigue sin importar: **ya tiene la app andando**.

**📈 Y HAY SEÑAL DE REACTIVACIÓN ANTES DE LA MEDICIÓN.** La Query B por día mostró **7 episodios el 27 ago** —el día del mensaje al grupo de WhatsApp— y **registros TODOS los días del 28 ago al 1 sep**. No es la medición formal, pero es la primera evidencia de que el mensaje del Cerebro Huella movió la aguja. **La ventana 2-4 sep arranca mañana** y esto es un buen piso.

**📌 CUATRO REGLAS NUEVAS que dejó el episodio, en su sección de arriba.**
...
**2. CHROME ANDROID IGNORA `continuous = true`.**
El motor de voz cierra después de **cada frase**. Reiniciar el reconocimiento ahí es **funcionamiento normal, no excepción**. Cualquier techo, contador o red de seguridad que cuelgue del reinicio tiene que estar dimensionado para decenas de reinicios por grabación, no para ocho.

**3. TODA QUERY SQL SE ENTREGA EXPLÍCITA Y COMPLETA EN EL MENSAJE, LISTA PARA PEGAR.**
Nunca por referencia ("la query de arriba", "la que te pasé"). Aplica a **Claude y a Claude Code**.

**4. CUANDO HAY UN USUARIO REAL AFECTADO, LA QUERY VA ANTES QUE LA LECTURA DE CÓDIGO.**
El primer diagnóstico de este bug se hizo leyendo solo el código y apuntó al CSS de `EpisodioCard`. **El dato de una query lo desmintió en un minuto** y movió la causa a otro archivo. Leer código dice qué **puede** pasar; la query dice qué **pasó**.

---

## Sesión lunes 31 agosto 2026 — **Se cerraron las dos investigaciones abiertas, se migró el género de los 32 hijos, y el onboarding rediseñado entró a producción**

**Día largo, en dos mitades.** La primera cerró las dos incidencias abiertas — y las dos terminaron **descartando la sospecha inicial**. La segunda fue el rediseño del onboarding y una migración de datos que destapó cuánto daño hacía un bug que nadie había visto.

### 1. ✅ CASO IGNACIA — CERRADO, NO ERA BUG

**El onboarding se apagó correctamente**, por la condición **"nombre de perfil poblado"** (`yaTieneCuenta` en `Layout.jsx:100`). El perfil "Igna" ya tenía nombre desde antes, y **basta el nombre** — no hace falta tener un hijo.

**Los datos del SQL que Daniel corrió:**

| Dato | Valor |
|---|---|
| Cuenta creada | **17 abr 2026** |
| Episodio registrado | **28 abr 2026** |
| Hijos | **cero**, hasta hoy |

**🪤 Y el dato raro tenía explicación:** un episodio **sin ningún hijo** en la cuenta. La causa es que **Daniel reseteó el perfil compartido de Pascual en su momento** — al borrarse el hijo, **el episodio quedó huérfano**. No es corrupción ni un bug del flujo nuevo: es la consecuencia de un borrado viejo. *(De ahí salió un pendiente nuevo, abajo.)*

**Lo que se hizo hoy:** Daniel **creó un hijo nuevo ("Pascu")** y **canjeó `HUELLA-15`** desde esa cuenta. La tabla de testers y el conteo de códigos quedaron actualizados arriba.

**⬜ Lo que falta para que Igna le cuente a Google:** que **abra la app de Play desde SU teléfono**. Va como mensaje simple en el empuje de la próxima semana — nada técnico, según la regla de soporte.

### 2. 🆕 Pendiente nuevo que salió de ahí: episodios huérfanos

Al borrar o resetear un hijo, **sus episodios quedan sin padre**. Hay que **verificar que no rompan el Historial ni el motor de patrones**, y **decidir la política** (borrado en cascada vs. archivarlos). Queda en la cola de investigación, **baja prioridad**: es data vieja, no afecta a los testers nuevos.

### 3. ✅ CASO VALE, CAPA 2 — la hipótesis del Workspace quedó DESCARTADA

**Daniel abrió el link de opt-in logueado con su propia cuenta del mismo dominio** (`d.undurraga@pacificschoolpichilemu.com`) **y funciona perfecto: llega a la pantalla "Become a tester".**

O sea: **el Workspace del colegio NO bloquea la prueba cerrada.** Era la hipótesis más fuerte y cayó con una prueba de treinta segundos que **no involucró a Vale** — exactamente el corolario de la regla de soporte.

**Causa restante más probable:** la **cuenta del NAVEGADOR distinta a la de Play Store** al abrir el link. Es la **causa #1 del 2 ago**, cuando varios testers vieron lo mismo y se resolvió solo.

**📌 Decisión: el caso pasa a segundo plano.** Dos razones: **(a)** si Vale ya tiene la app instalada, **el link de opt-in no le sirve para nada** — solo sirve para instalar; **(b)** el **fix del arranque de ayer debería traerla de vuelta sola**, sin que haga nada. **Se evalúa en la medición del 2-4 sep**: si aparece activa, el caso se cierra sin haber tocado a nadie.

### 4. 🆕 Dos testers más: la lista llegó a 22

| Nombre | Correo | Código |
|---|---|---|
| **Rodrigo Lazo** | `rlazod@gmail.com` | `HUELLA-27` |
| **Titi Scout** | `sfaille@gmail.com` | `HUELLA-28` |

**Los dos quedaron agregados al grupo de Google el 31 ago.** Los mensajes de bienvenida y los códigos **los envía Daniel hoy**. La lista oficial pasó de **20 a 22 personas**, y la tabla de arriba está actualizada. *(Hoy son **24**: Ceci Caro y anto entraron el 2 sep.)*

**Y se completaron los nombres que faltaban** de las cinco del 29 ago: Krishna Baquedano, Maria Adriana, Carolina Morales, Constanza Aguayo e Iti Fuenzalida. **La tabla oficial ya no tiene huecos.**

⚠️ **Códigos libres: al 2 sep queda UNO solo, `HUELLA-28`.** *(Este bloque es del 31 ago, cuando quedaban `HUELLA-29` y `HUELLA-30`; los repartos posteriores los consumieron. La asignación vigente es la tabla oficial de arriba, y sobre ella manda el Sheets de Daniel.)* Si aparecen más candidatos, **hay que generar un lote 3** — el SQL del lote 2 sirve de molde (bloques del 29 ago), cambiando el rango y la nota. Recordar el paso de verificación: **SELECT antes del INSERT**.

**Ojo con el colchón, que es el punto de todo esto:** con 24 personas en la lista hay **12 de margen** sobre los 12 que exige Google — pero el margen que importa es el de **instaladas**, y al 2 sep son **14**, o sea **2**. El requisito es de **12 SIMULTÁNEOS**, así que el margen es contra desinstalaciones, no un acumulado.

### 5. ✅ MIGRACIÓN DE GÉNERO — CORRIDA Y VERIFICADA EN PRODUCCIÓN

El bug apareció auditando el onboarding: la app entera guarda `'m'`/`'f'`/`'nb'` (`HijoPage`, `PerfilPage`) y `analizarEpisodio` compara contra esos códigos, pero **el onboarding mandaba las etiquetas crudas** `'Niño'`/`'Niña'`/`'Otro'`. Nunca matcheaban.

**Daniel corrió el SQL después del deploy. Los números cuadran exacto:**

| Antes | Después |
|---|---|
| `Niño` = 10 · `Niña` = 9 · `f` = 9 · `m` = 4 | **`f` = 18 · `m` = 14** |
| **Total 32** | **Total 32 — sin pérdidas** |

**Los 32 hijos de la base quedaron en formato `m`/`f`.** 🔎 **El tamaño real del daño: 19 de 32 hijos venían saliendo en genérico desde siempre** — o sea que la mayoría de las orientaciones que leyeron los testers hablaban de "niño/a" y "él/ella" aunque el padre hubiera respondido la pregunta.

⚠️ **Lo que la migración NO hace: las orientaciones ya generadas no cambian.** Quedaron escritas en genérico y no se regeneran solas. Desde ahora, las nuevas sí usan el género correcto.

### 6. 🎨 ONBOARDING — bloques 1 y 2 EN PRODUCCIÓN (`79a4994`)

**El onboarding dejó de vender y pasó a recibir.** De 5 slides + 6 pasos a **2 actos + 5 pasos**. Detalle completo en el mensaje del commit; lo esencial:

- **Fuera los slides 1, 2 y 5** (gancho, promesa, afirmación), `OnboardingBottomSlide` y el bundle `handoff_onboarding/` que seguía trackeado.
- **Fuera el paso de intenciones** y la escritura a `perfiles.intenciones` y `perfiles.contexto_inicial` — las dos eran columnas de escritura muerta.
- **El acto A dejó de ser saltable.** Con eso desaparece el limbo: antes quien saltaba dejaba la cuenta a medias y el onboarding le reaparecía cada sesión.
- **Visual 100% Mocha Mix:** cero hex y cero rgba (antes había 15), y **el bug que reportó la tester quedó cerrado** — el texto de la respuesta pasó de **1,16:1 a 15,24:1 en claro y 15,97:1 en oscuro**.
- **Token nuevo `--color-eyebrow`** en `index.css`, con override de oscuro.

**⏭️ PENDIENTE INMEDIATO — es lo PRIMERO de la próxima sesión (después del Frente 1): el QA visual de Daniel.**

🔴 **REGLA NUEVA DE QA, que sale del bug del tap del Cerebro: el QA de pantallas se hace en claro Y oscuro, y en iPhone Y en el Android de prueba.** El caso del Cerebro lo demostró: **un QA solo-iPhone dejó pasar un bug que le pega al 100% de los testers**, porque la beta es toda Android. No volver a aprobar una pantalla con un solo sistema.

⚠️ **Ojo con el método: NO existe todavía un atajo `?onboarding=1`.** El gate lee la base (`Layout.jsx:100`), así que el onboarding solo aparece con **cero hijos y nombre de perfil vacío**. Hoy hay dos caminos: **(a)** registrarse con un alias de Gmail (`danielundurraga.r+onb1@gmail.com`), o **(b)** resetear esa cuenta de prueba por SQL entre pasada y pasada. **Implementar el flag `?onboarding=1` es trabajo pendiente y sería lo primero del bloque de QA**, porque hacen falta cuatro pasadas (claro/oscuro × iPhone/Android).

**Después del QA siguen, en orden:** **bloque 3** (acto B — el momento real se guarda como el primer episodio, con `analizarEpisodio` en segundo plano), **bloque 4** (acto C — cierre de solidez con autores reales del `MAPA_DIMENSIONES`, sin citas inventadas) y **bloque 5** (ajustes finales del flujo de saltar).

### 7. 🧠 La cola del Cerebro, para después del onboarding

En orden, sin investigar nada ahora:

1. 🔴 **Bug del tap en Android** — el raycast detecta (el chip se ilumina) pero la tarjeta no abre. **Prioritario: la beta es 100% Android.** Sospechoso: `d46366c`.
2. **Paso 7 — QA en la tablet Samsung, ahora OBLIGATORIO**, y debe cubrir el caso del tap.
3. **El enlace del paso 8**, con la hipótesis del plegable cerrado por defecto.
4. **QA 2 del paso 8** — quedaron seis verificaciones, la más importante el PDF limpio.
5. **La decisión del PDF** — aplicar `esLetraChicaDelModelo` también en `InformePDF`.

### Estado de los dos frentes al cierre

**🥇 FRENTE 1 — LA VENTANA DE MEDICIÓN ARRANCA EL MIÉRCOLES 2 DE SEPTIEMBRE, y es lo PRIMERO del proyecto: va antes incluso del QA del onboarding.** El reloj de los 14 días corre desde el 27 ago y la postulación apunta a **~10 sep**, así que la medición del **2-4 sep** (día ~7) es la que dice si hay que empujar a alguien antes de postular. Se mide contra la **LISTA OFICIAL DE TESTERS**, que hoy tiene **24 personas**.

⚠️ **Cuadrar el número antes de medir:** al cerrar el día se habló de **23**, pero la tabla oficial tiene **22 filas**. O falta agregar a alguien a la tabla, o el 23 salió de un conteo distinto. **La tabla manda** (regla de la propia sección), así que **lo primero de la medición es cerrar esa diferencia** — si entró una persona nueva, se agrega con su correo y su código antes de correr las queries. **El plazo del 31 ago del SDK se cumple hoy y se pasa, como estaba asumido** — desde ahora no se puede subir ninguna actualización nueva a Play hasta que la v4 llegue a producción. *(Recordatorio útil: eso NO bloquea arreglos web, que es como llegó el fix del arranque.)*

---

## Sesión domingo 30 agosto 2026 — **🔴 Se cazó el bug que dejaba la app pegada en el splash, y quedó con red de seguridad automatizada**

**Día de diagnóstico y arreglo.** Dos testers antiguos reportaron que la app no abría; resultó ser un defecto real de arranque, no un problema de ellos.

### 1. El bug: la app se quedaba en el splash para siempre

**Síntoma:** Diego (`ddiegodiaz`) y Vale (`v.ramirez`) — los dos **testers antiguos, con la app instalada desde agosto** — reportaron que *"se demoró demasiado, no se pudo abrir"*. El escarabajo del splash latiendo sin fin, sin error y sin salida.

**🪤 La causa:** `getSession()` en `AuthContext` era **el único eslabón de la cadena de arranque sin red de seguridad**. No tenía `.catch()` ni timeout. Si la promesa se colgaba o fallaba, `loading` se quedaba en `true` **para siempre** — y como `ProtectedRoute` exige `!loading` **tanto para revelar la app como para mandar a `/login`**, no había salida: ni entraba ni redirigía. El `ErrorBoundary` tampoco ayuda, porque solo atrapa errores de render, **no promesas colgadas**.

**Por qué le pegaba a los ANTIGUOS y no a los nuevos** — y esto es lo que amarra el diagnóstico con los datos: una cuenta **sin** sesión guardada resuelve al instante, sin tocar la red. Una **sesión vieja y vencida** obliga a `getSession()` a salir a refrescar el token, y **ese fetch no tenía timeout**. La población en riesgo era exactamente la que reportó el problema.

### 2. El fix (commit `222b513`, en producción)

1. **`AuthContext`** — `.catch()` + **rescate por timeout a los 8s**. La sesión se aplica **venga cuando venga**: si llega tarde, el usuario se recupera solo. `onAuthStateChange` quedó como **segunda vía de destrabe**.
2. **`SplashArranque`** — **failsafe visual a los 12s** (a propósito *por debajo* del rescate de 8s): *"Esto está tardando más de lo normal."* + botón **Reintentar**. Cubre el resto de la cadena (familia y datos).
3. **`LoginPage`** — hueco que apareció al implementar: **no redirigía si ya había sesión**. Sin eso, "el rescate mandó a /login y la sesión apareció un segundo después" dejaba a la persona mirando el formulario con sesión válida. Ahora entra sola.

**El flujo normal NO cambió:** con sesión válida entra igual que siempre; sin sesión va a `/login` igual que siempre. Solo cambia el caso patológico.

**Y de paso: se eliminó el bloque temporal de Eruda de `index.html`** (mismo commit), que su propio comentario pedía borrar hace meses. El `index.html` publicado bajó de **3.15 kB a 1.42 kB** — que además fue la prueba dura de que el deploy de Vercel era este commit.

### 3. ✅ Verificado con un harness automatizado: 24/24

No hay framework de tests en el repo, así que se armó uno **sin agregar una sola dependencia**: vite compila el `AuthContext.jsx` **real** contra un React falso, un Supabase controlable y un **reloj virtual** (los 8 segundos se avanzan, no se esperan).

**Lo que lo hace evidencia y no autocomplacencia:** el "antes" **no es una simulación** — se sacó de git (`git show 222b513^:...`) y corre bajo el mismo driver. La suite **demuestra el bug** en la versión vieja (`loading` sigue en `true` a los 60s simulados, más un `unhandled rejection` que tumbaba el proceso) y **demuestra el arreglo** en la nueva (libera exactamente a los 8000 ms, cero rechazos sin atrapar).

**🔁 Regla operativa: `node verificacion/correr.mjs` si alguien toca `AuthContext.jsx` o `SplashArranque.jsx`.** Instrucciones completas en `verificacion/README.md` (ojo: hay que compilar antes, porque `verificacion/dist/` está en `.gitignore`).

⚠️ **Dos trampas quedaron documentadas ahí, y las dos daban FALSOS VERDES:** el plugin que intercepta supabase necesita `enforce: 'pre'` (sin eso se bundlea el cliente real y todo pasa sin probar nada), y la ruta va normalizada a barras (en Windows el archivo entra dos veces al bundle).

### 4. 📌 PRINCIPIO OPERATIVO QUE DEJÓ ESTE EPISODIO (regla nueva)

> **Los testers NUNCA reciben instrucciones técnicas de soporte. Todo se arregla del lado nuestro y les llega solo.**

No es cortesía: es lo que hace que la beta sobreviva. Un tester al que se le pide diagnosticar **se cae de la beta**, y cada uno que se cae reinicia el reloj de Google. Además, este arreglo es **web (Vercel), no Android**, así que le llega a todos al abrir la app **sin pasar por Play** — o sea que el bloqueo del 31 ago para subir versiones no impide arreglar cosas.

### 5. Caso Vale — dos capas, una resuelta

Se estaba tratando como un solo problema y son **dos**:

| Capa | Síntoma | Estado |
|---|---|---|
| 1 | "la app instalada no se pudo abrir" | ✅ **RESUELTA** por el fix del arranque |
| 2 | "App not available" en el link de opt-in | 🟡 **el 31 ago pasó a segundo plano** — ver el bloque de ese día |

> **➡️ CIERRE (31 ago):** de las tres verificaciones de abajo, la **#2 se hizo y DESCARTÓ la hipótesis del Workspace**: el link funciona con una cuenta del mismo dominio. Queda como causa más probable la **#3** (cuenta del navegador). **El caso quedó en observación hasta la medición del 2-4 sep.**

**Evidencia que respalda que la capa 1 le aplicaba:** Vale figura **registrada en la base pero sin canjear** su código (`H-13`), o sea que **tiene cuenta creada desde agosto** — justo el perfil que el bug tumbaba.

**🔑 Y un dato que cambia el costo de investigar la capa 2:** Daniel tiene una cuenta en **el mismo dominio Workspace que Vale** (`d.undurraga@pacificschoolpichilemu.com`, que ya estaba en el grupo desde la prueba de julio — ver el bloque del 2 ago). **Puede probar la hipótesis del colegio él mismo, sin tocar a Vale.**

**Las tres verificaciones de la capa 2, todas sin molestar a nadie:**

1. **Confirmar que `v.ramirez@pacificschoolpichilemu.com` sigue en el grupo** tras los movimientos del 29 ago (`groups.google.com`). El grupo es la fuente de verdad de elegibilidad, y **la edición del 29 ago es el único cambio en la ventana**.
2. **Abrir el link de opt-in logueado con `d.undurraga@pacificschoolpichilemu.com`.** Si a él **también** le dice "App not available" con una cuenta del mismo dominio → **es política del Workspace del colegio**, confirmado.
3. **Verificar la cuenta del NAVEGADOR, no la de Play Store.** El link de opt-in es una página web: usa la cuenta del navegador, que puede ser otra. Fue la causa #1 el 2 ago.

⚠️ **Nota que evita trabajo inútil: si Vale ya tiene la app instalada, NO necesita ese link para nada.** Solo sirve para instalar.

**Descartado con evidencia, no con opinión:** ❌ **nunca hubo migración de lista de correos a grupo de Google** — siempre fue el grupo, desde el 18 jul (`ESTADO.md:98`, `:101`, `:290`). ❌ El opt-in **no expira** solo. ❌ Google **no remueve** testers solo. ❌ El `minSdkVersion` **quedó intacto en 21** en la v4, así que la compatibilidad de dispositivos no se redujo.

### 6. Caso Ignacia — ✅ CERRADO EL 31 AGO: no era bug (ver el bloque de ese día)

Daniel inició sesión con `ignacopardo@gmail.com` y **no apareció el onboarding**. **Falta que corra el SQL de diagnóstico ya entregado** (fecha de creación en `auth.users`, si tiene perfil con nombre, hijos y registros) para saber si es **(a)** una cuenta que ya estaba armada — o sea el onboarding apagado correctamente — o **(b)** un bug del flujo de cuenta nueva.

**La condición exacta** vive en `Layout.jsx:100`. Se apaga si pasa **cualquiera** de estas: tiene **al menos un hijo O el nombre de perfil poblado** (`yaTieneCuenta` — ojo: **basta el nombre**); el latch `sessionStorage['huella.onboarding.dismissed']`; es **pareja invitada** (`role !== 'owner'`); `dataLoaded` todavía en `false`; o `familyLoading` en `true`.

⚠️ **Trampa del método:** por el latch de `sessionStorage`, **la prueba limpia va en ventana de incógnito nueva**.

---

## Sesión sábado 29 agosto 2026 — **El Frente 1 cerró su tercer paso: 5 testers nuevos adentro, 10 códigos nuevos en la base y la instrucción de canje por fin corregida · y nació una iniciativa estratégica**

**Día operativo, no de código de producto.** El único cambio de código fueron **dos strings** de las plantillas del tablero de `/beta`. Todo lo demás fue base de datos, grupo de Google y WhatsApp.

### 1. Diez códigos nuevos en la base

Se crearon **`HUELLA-21` a `HUELLA-30`**, con `nota = 'lote 2 - reclutamiento sep 2026'` para poder distinguir el lote 2 del original en el monitoreo. **El INSERT se verificó con un SELECT antes y después**, siguiendo el patrón de la migración `007`. Formato idéntico al de siempre (`HUELLA-NN`, dos dígitos, mayúsculas — la RPC normaliza con `upper(trim(...))`, así que en minúscula no matchearía nunca).

### 2. Los tres códigos reciclados, verificados antes de repartir

`HUELLA-01`, `HUELLA-06` y `HUELLA-09` eran de tres personas que **nunca respondieron**. **Daniel corrió la query y confirmó que los tres seguían SIN canjear**, así que reciclarlos era seguro. Esa verificación no era opcional: un código ya canjeado le habría devuelto *"Este código ya fue usado"* a la persona nueva, sin explicación útil.

⚠️ **El riesgo que queda vivo:** nada revoca un código. Si alguno de los tres dueños originales aparece y lo canjea, **gana el que llegue primero** y el otro queda con un error confuso. Con `HUELLA-27` a `30` libres, hay salida inmediata si pasa.

### 3. La lista de testers pasó de 18 a 20

**Salen** `be.delacerda`, `snackclub2022` y `nicolas.arriagada` (nunca respondieron, códigos reciclados). **Entran cinco:** `krishna.baquedano77` (H-06), `mariaadrianaq` (H-01), `calomose` (H-09), `conny.aguero` (H-12) y `snnunez1` (H-26). **Las cinco quedaron agregadas al grupo de Google `testers-huella@huella.lat`.** La tabla oficial de arriba está actualizada. **Falta vaciar los nombres reales de las 5 nuevas**, que hoy están solo en WhatsApp.

### 4. 🔑 El dato operativo que hay que no volver a olvidar

**El acceso de testers va por el GRUPO DE GOOGLE `testers-huella@huella.lat`, NO por una lista de correos en Play Console.** Un tester nuevo se agrega en **`groups.google.com`**. Si alguien reporta que "no puede entrar a la prueba", lo primero que se revisa es si está en el grupo — no la consola.

### 5. 🪤 La instrucción de canje llevaba tiempo mandando a la gente al lugar equivocado

**Síntoma:** el mensaje de instalación decía *"vas a quedar en la pantalla de inicio, ahí busca el bloque ¿Tienes un código de invitación?"*. **Ahí no está.** El componente `CanjeCodigoBeta` **se mudó a Perfil** cuando el Home nuevo (B2) se redujo a una sola acción — está escrito en `PanelPage.jsx:36` (*"y CanjeCodigoBeta, que se mudó a Perfil"*) y en `PerfilPage.jsx:405`. Hoy vive en **"Tú"** (`/perfil`, tercera pestaña de la barra de abajo) y en `/cuenta`.

**Y faltaba un paso completo:** una cuenta recién creada **no aterriza en el inicio limpio** — le aparece encima el **onboarding** (`Layout.jsx:100`, se dispara porque no hay hijo ni nombre de perfil) y hay que completarlo o saltarlo antes de poder navegar. El mensaje no lo mencionaba.

**Esto explica casi con certeza el hallazgo de la base:** hay **4 personas registradas que nunca canjearon** su código. Muy probablemente buscaron el bloque donde el mensaje les dijo, no lo encontraron, y ahí quedó.

**Corregido en las dos plantillas** (`BetaPage.jsx:85` y `:95`): completar el onboarding → tocar **"Tú"** abajo a la derecha → bajar al bloque → pegar y **"Activar"**. Confirmado contra el código fuente que el copy citado es exacto: volanta `Acceso de beta`, título `¿Tienes un código de invitación?`, botón `Activar`. El bloque **no exige tener hijo creado** y **solo se esconde si el usuario ya es Pro** (`CanjeCodigoBeta.jsx:37`).

### 6. Los mensajes salieron

Bienvenida + instrucciones enviadas el **29 ago**, ya con la instrucción corregida. Se pidió **app instalada hasta el domingo 13 sep**, con uso **un par de veces por semana**.

### 7. 📊 La foto de la base al 29 ago

- **11 códigos canjeados**, **todos de la lista oficial: cero intrusos.** *(⬆️ **12 desde el 31 ago**: `ignacopardo` canjeó `H-15`.)*
- **4 personas registradas SIN canjear:** `v.ramirez` (H-13), `javiera.zamora.r` (H-17), `ignacopardo` (H-15), `nene.pepita` (H-11) → **recordatorio en el empuje de la próxima semana**, con la instrucción corregida. *(⬇️ **quedaron 3 el 31 ago**, al canjear Igna.)*
- **Libres:** `HUELLA-27` a `HUELLA-30`. *(⬇️ **quedaron `H-29` y `H-30` el 31 ago**: `H-27` y `H-28` se asignaron a los dos testers nuevos.)*

### 8. Incidencia de soporte abierta

**`mariaadrianaq` reportó "contraseña o correo incorrectos".** **Hipótesis principal: intentó INICIAR SESIÓN en vez de CREAR CUENTA.** Se le respondió con la aclaración. **Seguimiento pendiente.** *Nota: no confundir con el bug abierto de recuperar contraseña (ver PENDIENTES) — ese es otro camino y no está descartado si el seguimiento no cuadra.*

### 9. La medición de actividad quedó fijada para el 2-4 sep

No se corrió hoy a propósito: con la ventana arrancando el 27 ago, al 29 solo hay ~2 días de datos y encima de fin de semana. **El día ~7 (2-4 sep) es cuando el número distingue de verdad quién volvió**, y deja margen para empujar a los inactivos antes del ~10 sep.

---

## 🌱 INICIATIVA ESTRATÉGICA — "Huella para familias neurodivergentes"

**Abierta el 29 ago 2026. Largo plazo. NO es un ítem de la cola de features y no se trabaja por partes.**

**Origen:** la pregunta real de una **candidata a tester** — si Huella sirve para su familia. No salió de un brainstorm: salió de una persona concreta preguntando algo concreto.

**El punto de honestidad, que es el que ordena todo:** **la app hoy está construida sobre desarrollo típico y no está validada para neurodivergencia.** Los frameworks calibrados por edad asumen trayectorias típicas. Decir que sirve sin haberlo trabajado sería vender algo que no existe.

**Decisión de Daniel, textual: *"quiero que sea algo real"*.** No un parche, no una etiqueta de marketing, no un modo a medias. **Hacerla a medias sería peor que no hacerla** — para estas familias, una herramienta que dice entenderlas y no las entiende es una decepción más, no una ayuda.

**El camino acordado, en tres tiempos:**

1. **Por ahora, NADA de código.** No se toca el prompt, no se agrega un flag, no se abre un "modo". Cero.
2. **El primer paso es APRENDER.** Invitar a esa familia a la beta **con expectativas honestas** (decirle de frente en qué está parada la app hoy) y **aprender de su experiencia dónde duele el calce**. Eso es dato real, que es lo único que puede fundar un diseño que no sea una suposición.
3. **Después de cerrar el ciclo actual** — postulación a Google (~10 sep) y Cerebro estable —, **sesión dedicada a diseñar el alcance**: si hace falta **asesoría de especialistas**, si esto es **un modo aparte o una calibración transversal** de lo que ya existe, y cuál es **el mínimo honesto para empezar**.

**Lo que NO hay que hacer mientras tanto:** proponer implementaciones, abrir tickets técnicos, ni tratar esto como si fuera un pendiente de la cola. **Es una decisión de producto que todavía no está tomada, y el insumo que le falta es la experiencia de una familia real.**

---

## Sesión jueves 27 agosto 2026 — **El Frente 1 avanzó dos de sus tres pasos y arrancó la ventana de 14 días · y el PASO 8 del cerebro entró a producción, con el diseño todavía abierto**

**El día empezó sin código y terminó con tres commits.** Primero el **Frente 1**, que es lo que el 26 ago se acordó que iba antes que nada: *"esto va ANTES que cualquier código, porque es lo único que tiene reloj y no depende de nosotros"*. Con esos dos pasos dados y el reloj corriendo, se abrió espacio para el **paso 8 del Cerebro Huella**, que entró entero a producción en la misma sesión — aunque **su diseño quedó abierto**, y eso es lo primero que hay que retomar.

### 1. La v4 (SDK 36) está APROBADA y publicada

Verificado por Daniel **directo en Play Console el 27 ago**: la versión quedó **publicada en el canal cerrado Alpha el 24 ago 13:13**, **activa para testers**, en **177 países**. Se había subido el 24 ago y quedó en revisión.

**El correo de aprobación nunca llegó.** Es la segunda vez que pasa exactamente igual — la v3 quedó aprobada el 18 jul 16:36 y también se descubrió revisando la consola, seis días después. **La regla, ya con dos casos: la señal de Google se busca en Play Console, no en la bandeja de entrada.**

### 2. El mensaje de reactivación salió al grupo de testers

Se envió al **grupo de WhatsApp de testers el 27 ago**, con **el Cerebro Huella 3D como novedad y como anzuelo**, y **agradeciendo el feedback de los testers como el motivo de la remodelación**.

Eso último no es adorno: es cierto y es la mejor razón que hay para que alguien vuelva a abrir una app. Ver que lo que dijiste se convirtió en algo que ahora puedes tocar pesa más que cualquier anuncio de feature.

### 3. Con eso arrancó el reloj

Los **14 días de testers ACTIVOS** que exige Google **corren desde el 27 ago**. **La nueva postulación a producción apunta a ~10 sep 2026.**

La estimación anterior era **~7 sep** y quedó corregida en todo el archivo. Se movió por el motivo que ya estaba escrito el 26 ago: la ventana no arrancaba con el calendario, arrancaba cuando los testers tuvieran un motivo real para volver a abrir la app. Ese motivo se soltó hoy.

### Lo que queda abierto del Frente 1

1. **Reclutar 5-8 testers nuevos.** El colchón sobre los 12 es lo que evita que el reloj se reinicie si alguno desinstala.
2. **Monitorear la actividad de los actuales**, con las queries de métricas cruzadas contra la **LISTA OFICIAL DE TESTERS** (18 personas en ese momento; hoy son 23. Si un número no cuadra, manda la tabla).

---

## 🧠 Y DESPUÉS, EL PASO 8 DEL CEREBRO HUELLA — entró entero a producción el mismo día

### La pieza

Dos commits: **`71c098f`** (la pieza entera) y **`6cf1302`** (el ajuste del QA 1). **Daniel corrió la migración 014 en Supabase y verificó la columna.**

**Qué hace, en una frase:** cuando la orientación de un episodio se explica por una zona del cerebro, al cierre aparece un enlace que abre `/cerebro` con esa zona ya abierta y en la edad real del hijo.

**Las cuatro decisiones que lo definen:**

1. **La zona la decide la IA**, no una detección por texto a posteriori. Como la orientación es **texto plano y no JSON**, viaja como una línea marcada al final que se separa en el servicio (`separarZona` en `seccionesIA.js`, gemela de `separarAlivio`).
2. **El texto se guarda SIEMPRE limpio.** El marcador no se persiste nunca, y no es cosmética: `orientacion_ia` **se imprime crudo en el informe PDF**, así que un marcador dentro del texto habría salido impreso en el informe del hijo. Verificado con **11 casos** (marcador al final y en el medio, con tilde, duplicado, slug inventado, stream cortado a mitad del marcador, texto vacío): cero fugas.
3. **Válvula clínica sin gate técnico nuevo.** 🪤 **Los episodios NO tienen clasificación `derivar` — eso es de `patrones`.** La auditoría lo destapó antes de escribir una línea, así que la regla vive en el prompt: si el episodio amerita mirada profesional, la IA devuelve `ninguna` y no se ofrece el enlace. El cierre sereno queda intacto.
4. **El código se desplegó ANTES de la migración**, y por eso `orientacion_zona` va en un **UPDATE APARTE** del texto. `updateEpisodio` no propaga errores: en el mismo UPDATE, una columna inexistente habría hecho que PostgREST rechazara la fila entera y **se perdiera en silencio la orientación que el padre acababa de leer**.

### El QA 1 destapó dos cosas, y las dos se arreglaron

**El enlace estaba escondido.** Nació como línea de texto pelada, deliberadamente callada. Daniel lo descartó: quedaba camuflado entre los párrafos. Subió a **fila CTA** de la familia del CTA del Home — fondo `--color-surface-alt`, `--radius-md`, padding de tap, chevron y el **punto de la zona latiendo** a 2,2s. Copy: *"Mira lo que se encendió en su cerebro"*, que **no nombra la zona**: la representa el punto de color, y el nombre viaja en el `aria-label` porque un punto de color no dice nada en voz alta.

**🪤 EL DOBLE DESCARGO ERA NUESTRO, Y ESTABA DOCUMENTADO SIN ARREGLAR.** `PieCientifico.jsx` lo decía con todas sus letras desde antes: *"Descargo fijo: lo pone el componente, NUNCA la IA. **En los episodios esta misma frase la escribe el modelo (va pedida en el prompt)**"*. O sea el modelo escribía el descargo largo **dentro** del texto y la pantalla ponía el corto **afuera**. Se atacó por los dos lados: el **prompt dejó de pedirlo**, y `OrientacionSecciones` **filtra igual** la letra chica que venga del modelo — porque el prompt no alcanza a los episodios ya generados. El filtro exige **dos condiciones juntas** (negar diagnóstico + hablar del respaldo), verificado con 4 líneas que deben irse y 8 que deben quedarse, incluidas trampas como *"la evidencia muestra que a los 4 años esto es esperable"*. Cero falsos positivos.

**La lección, que es la misma del 26 ago:** el arreglo ya estaba escrito en el repo y nadie lo había leído. Van dos sesiones seguidas.

### ⚠️ Lo que NO quedó cerrado del paso 8

**El diseño del enlace no convence a Daniel: "no me cierra".** Ni siquiera como fila CTA. **Es lo primero de la próxima sesión de cerebro**, y está anotado en Deuda de diseño con las palancas ya identificadas (borde del color de la zona, o tinte suave del color en el fondo) y con la sospecha de fondo: **el enlace vive dentro del plegable "Orientación completa", que está cerrado por defecto** — puede que ahí esté la causa real de que se sienta escondido.

**Y el QA 2 quedó INCOMPLETO.** Lo único que se alcanzó a ver fue el enlace en pantalla. Faltan seis verificaciones, la más importante el **PDF limpio**. Todo anotado en la cola.


---

## Lo que NO cambió en ninguno de los dos frentes

**El plazo del 31 ago para SDK 36 se sigue pasando, y sigue asumido.** La advertencia de nivel de API **solo se cierra cuando la v4 llegue a PRODUCCIÓN**, y producción no llega antes del ~10 sep. Consecuencia concreta que ya estaba anotada: **desde el 31 ago no se puede subir ninguna actualización nueva** hasta que la v4 esté en producción.

**El paso 7 (QA en la tablet Samsung + decisión GLB v1 vs v2) sigue pendiente, y lo bloquea el hardware, no el código:** Daniel avisará cuando tenga la tablet a mano. El paso 8 se hizo sin esperarlo y no pasó nada — no se bloquean entre sí.

---

## Sesión miércoles 26 agosto 2026 — **PASOS 4, 5 Y 6 CERRADOS: el Cerebro Huella está entero, muestra la edad real del hijo y por fin tiene puertas**

**10 commits** (el último es este cierre). El paso 4 se hizo en **dos bloques con QA de Daniel entre medio**, y esa separación fue la decisión que ordenó el día: primero dejar la imagen idéntica al prototipo, y recién después mover cosas. Después vinieron **dos rondas de arreglos que solo aparecieron en el teléfono real**, y un ajuste de producto.

```
0361e30  bloque A - la baseline visual
1502ed7  bloque B - la capa de producto
a6d24fb  chips deslizables + encuadre mas grande + fade
3fa59b3  legibilidad en oscuro + apilamiento de la tarjeta
ecbe003  rotulos con el color de su zona
4d34326  fuera el panel de diagnostico - paso 4 cerrado
0d041fb  paso 5 - el cerebro arranca en la edad real del hijo
c906574  paso 6 - el Cerebro Huella por fin tiene puertas
d46366c  la deriva vuelve al cambiar de tab + el CTA del Home
```

**QA final de Daniel: APROBADO en teléfono real, en modo claro Y en modo oscuro.** El panel `?diag=1` ya se eliminó, que era la condición acordada para declarar el paso 4 estable.

### Lo primero fue descartar al modelo

Antes de tocar una línea se decodificó el GLB que el prototipo lleva inline en base64 y se comparó con `public/modelos/cerebro.glb`: **mismo sha1, byte a byte**. Con eso el modelo quedó fuera de la lista de sospechosos y **toda diferencia visual pasó a ser del código**. También apareció que `plantilla-fase-b.html` (35 KB) es idéntico al prototipo salvo esa línea del GLB: **esa es la versión legible**, y es por donde conviene entrar la próxima vez.

### Bloque A — la baseline visual (`0361e30`, APROBADO en QA)

Faltaban seis cosas y ninguna era obvia: la **sexta luz** (`contorno2`), la **saturación** de las zonas internas, el **emissive** de todos los materiales, la **silueta del vidrio** (un casco `BackSide` a escala 1.012 que es lo que dibuja el borde del manto), la **sombra de contacto** y la **viñeta con hairline** de la vitrina.

**Los dos síntomas que Daniel veía tenían causa exacta.** La prefrontal "azul plana" no era un problema de color: era que **faltaba el driver por edad**. A los 4 años la zona debe verse **gris de obra con 12% de índigo** — el director de orquesta todavía casi no existe, y ese es justamente el punto de la pieza. El "vidrio muy denso" no era la opacidad (`.10` contra `.0958`, imperceptible): era la **suma** de zonas menos saturadas, sin emissive y sin borde.

Dos valores entraron **además** de la lista pedida, porque sin ellos el screenshot no podía calzar: la **escala 0.94** (el cerebro crece con la edad) y la **amígdala congelada en el punto medio de su latido**.

### Bloque B — la capa de producto

El contenido salió a **`contenidoCerebro.js`**, **extraído por script** del prototipo (rangos de líneas exactos, solo se le antepuso `export`): el copy de Fase A **no se transcribe a mano ni se mejora**. Se verificó que no tiene voseo y que los helpers dan los mismos números que el bloque A había congelado — `pfn(4) = 0.208`, `escala(4) = 0.940`. La continuidad entre bloques quedó **comprobada, no supuesta**.

Entraron el **slider 0-18** con la coreografía completa, el **latido de la amígdala** (urgente sin director, sereno con él), los **haces director↔alarma** con punto viajero, los **chips con porcentaje vivo**, la **franja "Ahora mismo"** y la **tarjeta de zona**.

### Las tres decisiones de arquitectura que vale la pena recordar

1. **La escena se monta una sola vez.** Todo lo que cambia entra por refs que el loop lee cuadro a cuadro. Si `edad` fuera dependencia del efecto, mover el slider **reiniciaría la escena entera**.
2. **Los porcentajes se escriben directo en el DOM.** Cambian 60 veces por segundo; como estado de React serían 60 renders de la página completa para mover dos dígitos.
3. **Los colores de zona están duplicados a propósito** — en `ZONAS[slug].color` para el material WebGL y en los tokens `--cerebro-zona-*` para el CSS. No hay forma de compartirlos sin leer CSS desde JS en caliente. **Si cambia uno, cambia el otro.**

### Ronda 3 — lo que solo apareció en el teléfono (`a6d24fb`)

**La fila de chips no se dejaba deslizar**, y los chips son el único camino garantizado a las zonas profundas (la amígdala y el hipocampo quedan detrás del manto, así que el tap sobre el modelo no las alcanza). **No era el layout:** los 6 chips suman ~1.100 px contra 366 de vitrina, o sea el desborde existía y se veían 3 — exactamente lo reportado. Tampoco había nada tapando la fila ni capturando el gesto. **La causa: el gesto se lo llevaba el scroll VERTICAL de la página.** El prototipo no podía tener este bug porque allá `html, body` estaban en `overflow: hidden` y la página no scrolleaba nunca; acá sí, y iOS fija el eje del gesto en los primeros píxeles, así que sobre una franja de 30 px un pulgar casi siempre entra con algo de vertical. **Arreglo: `touch-action: pan-x`.** Es un efecto colateral de la decisión de que la página scrollee, no del port.

**El cerebro se veía chico en las edades tempranas**, que es donde están casi todos los hijos de los usuarios. El encuadre usaba la media diagonal de la caja, o sea el radio de una esfera que el cerebro **nunca llena** (su ancho es el 87,7% de ese diámetro y su alto el 48%), y encima llevaba un margen de 1.15: dos holguras acumuladas. **`MARGEN` bajó a 0.95**, y a los 4 años el cerebro pasó de ocupar el 72% al 87% del ancho. **La curva `CRECE` no se tocó** — el margen mueve la cámara y `CRECE` escala el modelo, son multiplicadores independientes.

Y se agregó el **desvanecido del borde derecho** de la fila, que aparece solo si queda contenido oculto y se apaga al llegar al final.

### Ronda 4 — los dos defectos del QA en OSCURO (`3fa59b3`)

**Los dos son el mismo tipo de error, y los dos estaban resueltos en el repo antes de que yo los cometiera.** Quedaron escritos como reglas en la sección "REGLAS TÉCNICAS DE LA APP", más arriba.

1. **La tarjeta era ilegible en oscuro (1,31:1).** Fondo con token del tema + textos con tokens fijos. El bloque "EN LA PRÁCTICA" era el único legible, y era el único con fondo fijo: esa fue la prueba del diagnóstico. Se arregló con el token `--cerebro-superficie` (blanco fijo) → **13,08:1**. El mismo error aparecía en el **chip activo** (que desaparecía en oscuro) y en la **perilla del slider**; y **al revés** en la bajada, el rótulo "Edad" y el valor de la edad, que usaban tokens fijos sobre el fondo de la página.
2. **La barra de navegación tapaba la tarjeta al scrollear.** La tarjeta se renderizaba dentro del árbol de la página en vez de por portal. **`UpgradeModal.jsx:70` ya tenía escrito el comentario que describe exactamente este problema.**

### Ronda 5 — ajuste de producto (`ecbe003`)

Los tres mini títulos de la tarjeta toman **el color de su zona**. Al medir el contraste apareció que **ninguno de los 6 colores puros sirve como texto**: el mostaza del cerebelo era el peor con 1,61:1, pero **incluso el índigo, que a ojo parecía seguro, se quedaba en 4,17**. Entraron 6 tokens `--cerebro-rotulo-*` con el mismo tono y saturación y la luminosidad bajada. **El listón se fijó contra el crema**, que es el fondo más exigente de la tarjeta, y cada valor se verificó **sobre el hex ya redondeado**, que es el que pinta el navegador. Los colores puros no se tocaron: los siguen usando el punto del chip y la barra, que son manchas de color y no texto.

### Paso 5 — el cerebro pasó a ser el de TU hijo (`0d041fb`)

Hasta acá el slider partía en 4 años para todos. Ahora parte en la **edad real del hijo activo con precisión de mes**, y el cerebro se dibuja en ese estado: un hijo de 4 años y 7 meses tiene un cerebro distinto al de uno recién cumplidos, porque las curvas interpolan. **`calcularEdad` no se tocó** —el diff son +28 líneas y 0 borradas— porque devuelve enteros y la usan `dbHijoToApp` y `PerfilPage`.

**El rótulo se dice en años y meses**, que es como un papá contesta de verdad: "4 años y 7 meses". La fracción tipo "4½" se descartó porque solo funciona cuando la edad cae cerca de la mitad, y redondear ahí sería mentir. Los meses se dicen **hasta los 6 años**: bajo esa edad son información real —hay mundo entre 2 años y 2 años y 10 meses— y de ahí en adelante nadie dice "9 años y 4 meses".

**Dos defectos que se cazaron probando el formateador con una tabla de casos antes de commitear**, no en el QA: el plural decía *"1 anios y 1 mes"*, y el recién nacido caía en la rama equivocada. Esto último obligó a **desviarse del plan aprobado**: el plan decía distinguir la edad real de la arrastrada *por el valor* (decimal vs entero), pero **un recién nacido da 0 y un hijo que cumple años justo hoy da un entero exacto**, así que por número son indistinguibles de un slider arrastrado. Se cambió a una bandera explícita.

### La lección de la sesión

**Tres veces el arreglo ya existía en el repo y no se leyó**: el portal estaba documentado en `UpgradeModal`, la disciplina de tokens estaba implícita en toda la app, y el QA en oscuro nunca se había hecho. **El costo fueron dos rondas de QA de Daniel que se podían haber evitado.** De ahí salen las tres reglas nuevas.

### Lo que quedó anotado

- 🪶 **La ruta `/cerebro` no es lazy** y por eso el copy y el CSS entran al bundle inicial de todos: **+6,4 kB gzip**. Daniel decidió **dejarlo así**; el arreglo tiene su propia pasada porque hoy ninguna página de `App.jsx` es lazy.
- 🚪 **PASO 6 CERRADO: el cerebro por fin tiene puertas.** El tercer tab "Su cerebro" en `HijoPage` y **el CTA al pie de `TarjetaCerebro` en el Home, que es la entrada principal**. Antes de esto **no había ni un solo enlace a `/cerebro` en toda la app** y ocho commits de trabajo eran invisibles para los testers. **Del QA del tab salió el defecto de la deriva** (`d46366c`): al volver de otro tab el cerebro quedaba fijo, pero **la amígdala seguía latiendo** — el loop corría y el problema era solo del giro. La causa está anotada como **reglas 4 y 5** en la sección de reglas técnicas: `autoRotate` de OrbitControls solo se aplica si el control está en reposo, y un gesto que termina sin `pointerup` lo deja atascado para siempre. **Lo siguiente: paso 7** (QA en la tablet y decisión GLB v1 vs v2) y después el **paso 8**, los enlaces contextuales desde la orientación de un episodio hacia la zona relevante del cerebro.
- 🔴 **Google rechazó el acceso a producción el 23 ago** y la v4 se subió al canal cerrado el 24. **El plazo del 31 ago del nivel de API se va a pasar.**

---

## Sesión martes 25 agosto 2026 — **El Cerebro Huella dejó de ser un prototipo y entró a la app**
...
La **versión 3** (canal alpha, subida el 18 jul) apunta a **SDK 35 y cumplía en ese momento**. ⚠️ **SUPERADO (26 ago 2026):** la **v4 con SDK 36 se subió al canal cerrado el 24 ago** y la solicitud de producción del 21 ago **fue rechazada el 23 ago**. Lo vigente está en la cola viva, sección App stores.
...
### El escarabajo dentro del disco — tres intentos y la lección

Costó tres pasadas dejarlo bien. **Las dos primeras se hicieron midiendo el SVG con la calculadora y las dos salieron mal.** La tercera se hizo renderizando el disco ampliado con Chrome headless y mirándolo. Si esto se vuelve a tocar: **renderiza y mira, no midas.**

**HALLAZGO QUE APLICA A TODA LA APP: el viewBox del `Escarabajo` tiene muchísimo aire.** Medido, el dibujo ocupa **solo el 49.4% del ancho** del viewBox y el 68.2% del alto — tinta en `x[124.35, 356.20]`, `y[63.86, 357.17]` sobre `469.55 × 429.86`. **La caja del SVG NO es el tamaño del bicho**, así que cualquier número de tamaño va a parecer exagerado. En el disco del botón, una caja de 27px daba un escarabajo de 13px de ancho real, y por eso se veía tímido. Esto explica el comentario que ya estaba en `Escarabajo.jsx` sobre que los badges necesitan "48-52px y el 90% del cuadro" — venían compensando lo mismo a mano. **Si algún día se ve chico en otra pantalla, la causa es esta y no el grosor del trazo.**

**Los dos errores que costaron las pasadas 1 y 2:**

1. **La caja no se puede dimensionar como si el hueco fuera un cuadrado. Es un CÍRCULO.** Las patas del escarabajo se abren justo abajo, donde el círculo ya se cerró. Con la caja en 51×47 la **pata inferior derecha se salía del disco** mientras la izquierda quedaba justo adentro.
2. **La masa del dibujo está abajo.** Las antenas son finas y los élitros macizos: el **60% de la tinta vive entre el 45% y el 70% de la altura**. El centro óptico (centroide) queda **2% más abajo** que el centro de la caja, así que hay que **subir** el bicho. La pasada 2 lo bajaba un 1.03% intentando centrar el *bounding box*, y por eso se veía colgando con hueco arriba. **El ojo no pesa el bbox, pesa la tinta.**

**Valores buenos:** caja **46×42** con `translate(-1.05%, -2%)`. La tinta queda en ~65% del diámetro del disco (la proporción clásica de ícono en círculo), contenida con margen parejo. El `-1.05%` en x corrige un descentrado real del asset: su eje de simetría cae en `x=240.27` y el centro del viewBox está en `234.78`. **Nunca se edita el SVG: lo comparten 19 pantallas.**
