...
**App stores — 🟡 EN EJECUCIÓN · TWA por Bubblewrap · BLOQUEANTE DE LA BETA** (arrancó **2 jul 2026**; etapas 1, 2 y 3 CERRADAS, etapa 4 EN CURSO)
...
- **Etapa 4 — EN CURSO · PRÓXIMO PASO INMEDIATO (próxima sesión):** Daniel crea la cuenta de **Google Play** (`play.google.com/console/signup`, **US$25 pago único**, tipo **PERSONAL** decidido — no organización) + posible **verificación de identidad de Google** (puede tomar de **horas a días**). Después: crear la ficha de la app, subir el `.aab`, **copiar el SHA-256 de Play App Signing desde Play Console (sección App Integrity) al `assetlinks.json`** reemplazando `PENDIENTE_FINGERPRINT_BUBBLEWRAP` + redeploy. **OJO CRÍTICO:** el fingerprint correcto es el de **Play Console (Play App Signing)**, NO el del keystore local — Google re-firma la app con su propia clave. **Nota:** la **barra de navegador visible en la TWA es esperada** hasta que ese reemplazo del fingerprint esté hecho y verificado.
- **Recordatorio de seguridad (verificar al abrir la próxima sesión):** respaldar `android.keystore` + su contraseña en el Drive de Daniel, carpeta **"Huella - Llaves Android"** (fuera del repo). Si se pierden, no se pueden firmar futuras actualizaciones de la app con la misma clave.
