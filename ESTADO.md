...
#### MOTOR DE RASGOS (piezas 1, 2 y 3 EN MAIN; pieza 4 por construir)
...
- **PRÓXIMO PASO:**
  - **Pieza 4 (UI del retrato "que madura"):** renovar la pantalla del hijo (`/hijo`) con el retrato — **cabecera** (foto que se revela + rastro del escarabajo + nombre + píldora de nitidez) + **card de propuesta de rasgo de a UNO a la vez** (papá confirma/descarta) + **las 4 familias** (mueve/fortalezas/cuesta/calma en orden fijo) + **nitidez** (5 niveles vía filtro CSS, solo crece). Card de anticipo en el Home que lleva a `/hijo` (gatillo de retorno). Según las specs de la Fase 1 (`huella-gancho-retencion.md` en el Drive de Daniel). Materia prima ya disponible: `state.rasgos` y los cases `ADD_RASGO`/`UPDATE_RASGO` ya listos para la confirmación/descarte.
