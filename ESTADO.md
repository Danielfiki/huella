...
- 🧠 **CEREBRO HUELLA — FASE B EN CURSO, PRIORIZADA POR DANIEL (ago 2026)** — desde **24 ago 2026** — un **cerebro 3D manipulable del hijo**. No es una pantalla suelta: es una pieza que vive en cinco lugares. **(a)** card en el Home, *"En el cerebro de {hijo}"*, como **gancho diario**; **(b)** lugar permanente en la **página del hijo**; **(c)** momento contextual dentro de la **orientación post-episodio**; **(d)** **push mensual**; **(e)** **compartible**.
  ...
  - ⏭️ **LO SIGUIENTE EN ESTE FRENTE — PASO 6: el tercer tab "Su cerebro" en `HijoPage`.** ⚠️ **Es el paso que convierte todo lo anterior en algo que los usuarios pueden usar: hoy `/cerebro` NO tiene NINGÚN punto de entrada.** Verificado el 26 ago: fuera de la línea que declara la ruta en `App.jsx:188`, **no hay un solo enlace ni `navigate` a `/cerebro` en toda la app** — solo se llega escribiendo la URL. O sea que los seis commits del paso 4 y el paso 5 están en producción y **ningún tester los ha visto todavía**. Después: **paso 7** (QA en la tablet y decisión **GLB v1 vs v2**).
...
- 🚪 **LO SIGUIENTE ES EL PASO 6, Y ES EL QUE MÁS IMPORTA AHORA: el tercer tab "Su cerebro" en `HijoPage`.** Todo lo construido hoy está en producción y **nadie lo ha visto**: se verificó que **fuera de la ruta declarada en `App.jsx:188` no hay ni un solo enlace ni `navigate` a `/cerebro` en toda la app.** Solo se llega escribiendo la URL a mano. Hasta que exista el tab, ocho commits de trabajo son invisibles para los testers.
