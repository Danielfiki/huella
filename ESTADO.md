...
**App stores — 🟡 EN EJECUCIÓN · TWA por Bubblewrap · BLOQUEANTE DE LA BETA** (arrancó **2 jul 2026**; etapas 1-3 CERRADAS; cuenta Google Play creada **4 jul 2026**, identidad APROBADA **8 jul 2026**; **app CREADA en Play Console + ficha COMPLETADA el 14 jul 2026**; falta completar los cuestionarios de Play, subir el AAB, y correr la prueba cerrada de 12 testers ≥14 días que Google exige antes de producción)
...
- **Etapa 4 — EN CURSO · CUENTA GOOGLE PLAY CREADA (4 jul 2026) · IDENTIDAD APROBADA (8 jul 2026):**
  ...
  - **CUESTIONARIOS DE PLAY:** ✅ *Política de privacidad* COMPLETADO (URL `https://huella.lat/terminos#privacidad`). 🟡 *Datos de inicio de sesión* EN CURSO — se marcó que la app SÍ está restringida (requiere login); **PENDIENTE: crear una cuenta demo LIMPIA (Gmail nuevo)** con datos ficticios para dar a Google como credenciales de revisión (distinta de `contacto@huella.lat`, que tiene episodios reales del hijo, y distinta de los 12 testers reales de la prueba cerrada). ⬜ FALTAN: Anuncios, Clasificación de contenido, Audiencia objetivo, Seguridad de los datos, Aplicaciones gubernamentales, Funciones financieras, Salud, categoría + datos de contacto.
  - **REQUISITO CLAVE DE GOOGLE (registrar):** para publicar en **producción**, Google exige una **PRUEBA CERRADA con mínimo 12 testers** que acepten la invitación, **durante ≥14 días**, ANTES de solicitar acceso a producción. El sistema de códigos `HUELLA-01..20` + Tally calza con esto.
  - **FALTA además:** subir el **`app-release-bundle.aab`**, **copiar el SHA-256 de Play App Signing (App Integrity) al `assetlinks.json`** reemplazando `PENDIENTE_FINGERPRINT_BUBBLEWRAP` + redeploy (**OJO CRÍTICO:** el fingerprint es el de **Play App Signing**, NO el del keystore local — Google re-firma con su propia clave), configurar la **prueba cerrada** y enviar a revisión. La **barra de navegador visible en la TWA es esperada** hasta ese reemplazo del fingerprint.
