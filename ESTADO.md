...
1. 🟡 **CEREBRO — LA BARRA DE EDAD NO VUELVE A LA EDAD DEL HIJO — HECHA Y DESPLEGADA el 8 sep 2026** (`7104044` + `bd8b989` + `af0d611`): la foto del hijo cuelga bajo el riel en su edad y tocarla restaura la edad decimal con sus meses. **Falta solo el QA final del tick** (que no asome sobre el riel con la perilla lejos, claro y oscuro). Detalle en el bloque del 8 sep. — **feedback de Diego (`H-02`)**. **Síntoma en una frase: hoy mueves la barra, salta a edades cerradas y ya no hay cómo volver a la edad exacta del hijo (ej. 2 años 3 meses); debería poder volver siempre.** Hoy el slider parte bien —en la edad real con precisión de mes, eso quedó cerrado en el paso 5— pero una vez que se mueve, esa edad **se pierde**. **Qué se hace:** un **marcador con la foto de perfil del hijo** sobre la barra, puesto en su edad exacta; **tocarlo devuelve ahí**. Es la referencia visual de "acá está tu hijo/a" y a la vez el camino de vuelta. Se difiere por el Frente 1, no por dudas de diseño.

2. ⬜ **MICRO-RESPUESTA AL "¿CÓMO TE SENTISTE TÚ?"** — desde **8 sep 2026** — **Síntoma en una frase: hoy el padre escribe cómo se sintió y no recibe absolutamente nada de vuelta; debería recibir algo que le confirme que eso que escribió fue leído.** Es el único campo de la app que es **solo suyo**, y hoy cae al vacío. Va en **dos capas**:
...
- ⬜ **Falta solo el QA final del tick de Daniel** (`af0d611`): que con la perilla lejos del marcador la línea **no asome ni 1px** sobre el riel, en claro y oscuro.

### ⏭️ Próximo

1. **QA final del tick** (`af0d611`). Con eso la pieza 1 queda cerrada.
2. **Seguir la cola priorizada, ya desbloqueada:** pieza 2, la **micro-respuesta al "¿cómo te sentiste tú?"** (capa (a), la respuesta del momento, con marco anti-vergüenza).
