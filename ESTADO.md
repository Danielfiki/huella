...
3. ❌ **MOTOR DE RASGOS — LOS HITOS TAMBIÉN DEBEN ALIMENTARLO — SALE DE LA COLA el 9 sep 2026: LA PREMISA ERA FALSA.** La pieza decía *"hoy el motor lee solo episodios difíciles"*. **No es cierto, y no lo fue nunca.** El motor lee episodios **y** hitos desde que se escribió: el prompt marca cada momento con su `origen` y le pide explícitamente al modelo que las familias positivas (`mueve`, `fortalezas`, `calma`) se nutran sobre todo de los avances (`anthropic.js`, `PROMPT_DETECTAR_RASGOS`); los dos enganches del contexto cuentan **episodios + hitos juntos** para el gatillo, y la evidencia guardada distingue el tipo de cada momento. **Medido en producción el 9 sep: 28 de 70 rasgos tienen evidencia de hito.** No había nada que construir.
   ...
   🔴 **EL CUELLO DE BOTELLA NO ES EL MOTOR NI EL VOLUMEN: ES LA CONFIRMACIÓN MANUAL.** De 70 rasgos en la tabla, **61 son emergentes, 9 candidatos y CERO confirmados** — tampoco hay descartados. La card de candidato **vive solo en el tab Perfil de `/hijo`**, muestra **un** candidato por visita y **no reaparece** al resolverlo; los emergentes no se muestran nunca. **Nadie la ha tocado jamás: 9 candidatos, 0 resueltos, en 24 testers.** El retrato solo cuenta confirmados (`HijoPage`, `rasgosConfirmados` sobre un total fijo de 12), así que **todos los papás ven 0 de 12**. El motor detecta bien; el paso final depende de un toque que no está ocurriendo porque la card no se ve.

   **El volumen es un factor secundario, no la causa.** Un rasgo llega a `candidato` con **3 momentos** que compartan patrón, lo que pide del orden de **10 momentos**. Pascual, el hijo de Daniel, tiene **13 momentos**, y hay **9 candidatos** vivos entre todos los hijos: la detección llegó. El embudo se corta después.

   🪝 **Adónde se mueve el trabajo:** a la **pieza 3.5 nueva (confirmación visible)**, que ataca la causa directa. El volumen se apoya en la **pieza 5 (entrada única)** y la **pieza 7 (recordar sin reclamar)**, ya en la cola.
...
3.5. ⬜ **CONFIRMACIÓN VISIBLE — card de candidato en el Home + candidatos atenuados en El Sendero** — desde **9 sep 2026**, nace del diagnóstico de la pieza 3. **Síntoma en una frase: hoy hay 9 rasgos esperando un toque del papá y nadie los ha tocado nunca, porque la card vive escondida en un tab; deberían aparecer donde el papá ya está.** **Qué se hace:** la card de candidato sube al **Home**, y **El Sendero muestra los candidatos atenuados** junto a los confirmados, para que el retrato deje de verse en cero y el papá entienda que hay algo esperándolo. 🎨 **Primero mockup con Claude Design**, después Code sobre el código real. Va **antes** del retrato del padre: sin este paso, el motor de rasgos entero es invisible en producción.
