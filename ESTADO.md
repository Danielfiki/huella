...
*Única fuente de pendientes. Lo que se difiere ENTRA; lo que se completa SALE y queda en su bloque de sesión. Formato: **qué** — desde cuándo — por qué se difirió.*

**Principio que ordena esta cola (16 sep 2026):** la app devuelve algo antes de que el papá tenga que entrar. **Puerta = preocupación; casa = crecimiento.** 🔴 **Regla dura: nunca "no has registrado", nunca racha, nunca contador.**
...
21. ⬜ **Paso 8 — el enlace "no me cierra"** — desde **27 ago** — decisión de diseño de Daniel antes de tocar. (detalle y palancas: bloque del 27 ago)
22. ⬜ **Paso 7 — QA en la tablet Samsung + GLB v1 vs v2** — desde **26 ago** — bloqueado por hardware. Debe incluir el tap a una zona del modelo.
23. ⬜ **Escarabajo del alivio con escala** — desde **27 ago** — resolver primero el borroso de Safari iOS; el QA incluye iPhone real. (detalle: bloque del 27 ago)
24. ⬜ **Cabos del Cerebro** — desde **26-27 ago** — `bancoEducativo` se superpone con la matriz; ruta `/cerebro` lazy (Daniel decidió dejarla); filtro de letra chica en `InformePDF` (decisión de Daniel).
...
- ⬜ **Tarjeta "El retrato de {hijo} está creciendo"** — **14 jul** — muestra el estado casi-vacío aunque ya haya rasgos confirmados.
- ⬜ **Tablero de `/beta` hardcodeado a 20 códigos** — **1 ago** — hay 30 en la base; no conoce el lote 2 y puede reasignar uno canjeado. Una línea.
- ⬜ **`NotifBanner` en azul genérico, ajeno a la paleta** — **6 jul** — pasada de Design, pedido expreso de Daniel.
- ⬜ **`schema.sql` desfasado de la base viva** — **21 jun** — faltan `plan`, `contexto_inicial`, `intenciones`, `plan_beta_hasta`; `push_subscriptions` no tiene el `id` que declara.
- ⬜ **`CRON_SECRET` con validación condicional en `push-remind.js`** — **7 jul** — si faltara la variable, el endpoint queda sin auth.
- ⬜ **La tarjeta de patrones no deja elegir y dice 3 con 2 casos** — **6 ago** — **sin reproducir**; no tratar como bug confirmado.
- ⬜ **Fluidez del dictado — throttle del render** — **1 sep** — **baja**, y el diagnóstico original era falso: solo hay un re-render por segundo. Medir antes de tocar.
- ⬜ **Cobros: camino silencioso del botón de pago** — **4 ago** — arreglo defensivo puesto, pero **el bug nunca se reprodujo**. (detalle: bloque del 5 ago)
- ⬜ **Cobros: el webhook responde 200 aunque falle el `upsert`** — **4 ago** — si falla, el pago queda sin Pro para siempre.
- ⬜ **Cobros: la red post-checkout dura ~6s y no tiene segunda oportunidad** — **4 ago**.
- ⬜ **Cobros: primer pago REAL de punta a punta** — gate para activar cobros.
- ⬜ **Cobros: limpiar usuarios y cuentas de prueba de MP** — menor.

**No venían en la estructura nueva; los dejo para que no se pierdan (16 sep 2026)**
- ⬜ **Onboarding — pasada con cuenta nueva de verdad** — **3 sep** — nunca se vio con cuenta real: primer episodio al Home, `origen = 'onboarding'`, las dos fotos guardadas.
- ⬜ **Calibrar la micro-respuesta con 10 respuestas reales antes de tocar el prompt** — **8 sep** — van tres ajustes de tono contra casos únicos.
- ⬜ **¿"+ Registrar" directo a hito desde el filtro "Logros"?** — **4 sep** — cablearlo es trivial; falta decidir si se quiere.
...
### 8. ✅ Ítem 6 — reingreso tras 10+ días, EN PRODUCCIÓN (commit `a82175d`)
Al volver después de 10+ días (días de calendario de Chile, leídos de `ultima_actividad` **antes** de marcar la visita) el Home esconde la card de candidato y lo que el papá ya guardó se muestra **dentro de las puertas que ya existían**: **Su huella** con el rasgo positivo confirmado más reciente, o **Momentos** con el último relato, en el lugar del contador. **Sin card nueva y sin CSS nuevo.** Historial acepta `state.momentoId` y centra esa ficha.

### 9. ✅ Ítem 7(c) — identidad del rasgo, EN PRODUCCIÓN (commit `01ae849`)
La regla 6 del prompt suma ejemplos de **mismo patrón con distinta redacción**, entra la regla 7 y el campo `corresponde_a`, y el cliente fusiona evidencia contra ese id cuando estaba en el payload y no está descartado.
...
### 1. 🗂️ LA COLA VIVA PASÓ DE 95.558 A 11.594 CARACTERES

`ESTADO.md`, sección PENDIENTES. **De 255 líneas a 92.** Un ítem = una línea: qué, desde cuándo, por qué se difirió. El detalle que no puede perderse (causas descartadas, trampas) se quedó en su bloque de sesión, referido con "(detalle: bloque del X)".

**27 ✅ de julio 2026 se movieron a un bloque nuevo `## Cerrado (archivo)` al final del archivo**, una línea cada uno. Se archivaron porque julio quedó casi sin bloques de sesión —solo existen "28 julio" y "1-2 julio"—, así que esos cerrados estaban huérfanos: Play Store (identidad, ficha, cuestionarios, prueba interna, fingerprints, v3 y v4, grupo de testers, enlaces profundos), Storage (buckets privados, policies de escritura), prompts (modelo 4-6, vocabulario, contacto físico) y push (push-test, CRON_SECRET, control en Cuenta). Los ✅ de agosto y septiembre **no** se archivaron: ya tienen su bloque y solo salieron de la cola.

Se conservaron dentro del archivo dos trampas que no viven en ningún otro lado: que **`bubblewrap update` revierte el `targetSdk` a 35**, y que en el cambio de Storage **el deploy del código va ANTES del backfill**.

⚠️ **La sección quedó en 11.635 tras corregir el ítem 4** (ver punto 4). El 11.594 es la medida al terminar la reescritura.

🔴 **Salió de la cola: "racha por interacción activa"**, porque contradice la regla dura de la pieza 7. Y se agregó un sub-bloque con 6 pendientes vivos que la estructura nueva no nombraba, para que no se perdieran en silencio.

### 2. ✅ MIGRACIÓN 020 — LOS MOMENTOS YA GUARDAN CUÁNDO SE REGISTRARON (aplicada en producción y verificada)

`supabase/migrations/020_created_at_momentos.sql`. Se numeró **020 y no 018** porque el 018 y el 019 ya estaban tomados.

**`created_at timestamptz` en `public.episodios` y `public.hitos`, NULLABLE, default `now()`.** Verificado en `information_schema`: `is_nullable YES`, `column_default now()`, en las dos tablas.

🔴 **Va en dos pasos a propósito: `add column` sin default, y recién después `alter column ... set default now()`.** Un `not null default now()` en un solo paso le habría puesto a todas las filas viejas la fecha de la migración, que es un dato falso. En dos pasos **las filas anteriores quedan en null** —"no se sabe", que es la verdad— y solo las nuevas toman `now()`.

⚠️ **Consecuencia: retención y hora de registro se calculan solo sobre filas con `created_at` no nulo**, o sea momentos creados desde hoy. Las consultas van a necesitar `where created_at is not null`.

Sin GRANT: la columna la escribe la base, nunca el cliente. `schema.sql` quedó alineado en las dos tablas.

### 3. 📊 LO QUE MOSTRÓ LA BASE, Y CONTRADIJO LO QUE CREÍAMOS

- **Los 23 candidatos de La brava son viejos: el último es del 9 sep.** No es un bug del motor, que se arregló ese mismo día. Es limpieza de datos de prueba.
- **El motor SÍ detecta lo positivo** —La brava tiene 30 fortalezas contra 28 cuesta— **pero lo positivo no llega a candidato.** El problema no es la detección, es el umbral y lo que la card elige mostrar.
- **Los testers reales registran 2 momentos difíciles por cada 1 positivo.** La app se usa como puerta de preocupación, no como casa de crecimiento.
- 🔴 **El pico de las 9:00 NO es comportamiento: es un valor por defecto.** 21 de 26 momentos "de las 9" tienen `fecha` exacta `09:00:00`, repartidos en 17 días. Sale del chip **"Esta mañana"** (`RegistroPage.jsx:146`, `d.setHours(9, 0, 0, 0)`), que escribe una hora convencional fija. Su hermano es "Esta tarde" en 15:00:00.
- **La hora real de registro cae en tres franjas: 12-15, 18-20 y 22-23.** Ninguna coincide con el default de las 9:00.

📌 **Por qué importa:** `fecha` mezcla "cuándo pasó" con "cuándo se registró" sin marca que las distinga, y por eso el pico falso se leyó como hábito. Eso es lo que vino a separar la migración 020.

### 4. 🔍 LA PIEZA 7, LEÍDA DE VERDAD — el estado real no era el que decía este archivo

- **Son 5 ramas vivas, no 3.** La cifra "3 reglas" venía de acá y estaba desactualizada. Corregido en el ítem 4 de la cola.
- **La rama 3 (resumen semanal del domingo) es un hueco reservado**, comentado en `push-remind.js:143-146`. Espera al retrato del padre.
- **El cron NO está en `vercel.json`** —ese archivo solo tiene `rewrites`—: vive en `pg_cron`, `'0,30 * * * *'`, cada media hora en UTC.
- **La hora es por usuario, no fija.** El handler calcula la hora local de Chile con `Intl` sobre `America/Santiago` y filtra por `hora_aviso` + `minuto_aviso`. Tres opciones: 9:00, 14:00, 21:30. **Zona horaria hardcodeada a Chile.**
- **No hay deduplicación de envíos.** No existe tabla ni marca. Lo único que evita el doble envío es que a cada usuario le calce un solo bloque de media hora al día; si hay dos suscripciones, salen dos push.
- **`hitos` no se consulta nunca**: los avances no existen para la push.
- **`rasgos.titulo` se trae y no se usa**: el mensaje dice "algo se repite", nunca qué.
- **`episodios` llega solo 7 días atrás y sin texto** (`descripcion_libre` y `contexto` no se piden).

🎯 **DECISIÓN — el default de hora pasa de 9:00 a 21:30.** Lo manda la evidencia de las franjas: nadie registra a las 9, y ese default se estaba comiendo el permiso de notificaciones, que se pide una sola vez.

🎯 **DECISIÓN — la rama "hace un mes anotaste que…" no es un cambio de copy.** Necesita tres cosas que hoy no existen: consulta de momentos **por fecha pasada** (hoy el tope son 7 días), traer el **texto** del momento (hoy no se pide), y una **marca de qué momento ya se citó** para no repetir el mismo.

### 5. 🗃️ QUÉ NO SE PUDO MEDIR DESDE ACÁ

El `.env` local solo tiene la **anon key**, no la service role. Con RLS y sin sesión, cualquier consulta devuelve **cero filas, no un error**, así que correr SQL desde Code habría producido un vacío con cara de dato. Las consultas se entregaron listas para el SQL Editor y las corrió Daniel.

---

## Sesión martes 15 septiembre 2026 — **Tres guardados dejaron de decir "Guardado" cuando la base no guardó, y la micro-respuesta dejó de colar voseo**
...
*Ítems que salieron de PENDIENTES el 16 sep 2026 y **no tienen bloque de sesión propio** en este archivo (julio 2026 quedó casi sin bloques). Una línea cada uno, solo para que no se pierda el registro de que están cerrados.*

- ✅ **Control permanente de Notificaciones en la pantalla Cuenta** — 6 jul 2026, `8dc1b22`, verificado en iPhone.
- ✅ **`api/push-test.js` eliminado** — 7 jul 2026, `9d2df0d`; cero referencias antes de borrar.
- ✅ **`CRON_SECRET` rotado en Vercel + redeploy** — 7 jul 2026; la llave vieja quedó invalidada.
- ✅ **Identidad de la cuenta de Google Play aprobada** — 8 jul 2026.
- ✅ **Migración del modelo de IA a `claude-sonnet-4-6`** — 10 jul 2026, `e5f190a`, verificada en producción por Daniel.
- ✅ **Palabra vulgar "pico" fuera del andamiaje de prompts** — 10 jul 2026, `a1ace5a`.
- ✅ **Cláusula VOCABULARIO en `SYSTEM_PROMPT` y `REGLA_IDIOMA`** — 10 jul 2026, `a1ace5a`; cobertura verificada en las 15 llamadas.
- ✅ **Iconos PWA / maskable auditados y correctos** — 10 jul 2026; icono aparte para la ficha de Play (`icono-play-512.png`), 14 jul.
- ✅ **Contradicción sobre contacto físico en desborde, resuelta en el `SYSTEM_PROMPT` compartido** — 13 jul 2026, `32c9a3d`, QA en preview y producción.
- ✅ **Acceso a dispositivo Android en Play Console destrabado** (el error "Cuenta sin nombre" era exclusivo del celular; funcionó desde la tablet) — 14 jul 2026.
- ✅ **App creada en Play + ficha completada** (textos, icono 512, feature graphic, 5 capturas) — 14 jul 2026.
- ✅ **Cuestionarios de Play 11/11 + cuenta demo del revisor con Pro** — 15 jul 2026.
- ✅ **Página pública `/eliminar-cuenta` en producción** — 15 jul 2026, `449c1e7`; unifica el plazo de borrado en 30 días.
- ✅ **Prueba interna publicada e instalada en la tablet Samsung** — 15 → 18 jul 2026.
- ✅ **Fingerprints reales en `assetlinks.json`** (Play App Signing + upload) — 18 jul 2026, `2c34af2`; TWA abre sin barra de navegador.
- ✅ **v3 enviada a revisión y APROBADA** (177 países, canal Alpha) — 18 jul 2026 16:36, descubierto el 24 jul en Play Console. **Lección: revisar el estado en Play Console, no esperar el correo.**
- ✅ **Buckets de Storage `avatares` y `momentos` pasados a privados con URLs firmadas** — 21 jul 2026, `0637db1` + backfill + cierre de policies. **Lección de secuencia: deploy del código ANTES del backfill.**
- ✅ **Entrega de correo de confirmación: NO era bug** — 21 jul 2026; el síntoma era registrarse con correos ya existentes en `auth.users`.
- ✅ **Policies de ESCRITURA de Storage endurecidas** (carpeta propia por `auth.uid()`; el DELETE también estaba abierto) — 22 jul 2026.
- ✅ **Enlaces profundos: aviso de Google cerrado como falsa alarma** — 22 jul 2026; se decidió NO agregar el apex `huella.lat`.
- ✅ **v4 con SDK 36 construida y verificada leyendo el artefacto** (APK y AAB) — 22 jul 2026. ⚠️ **Trampa a recordar: `bubblewrap update` revierte el `targetSdk` a 35**; el 36 está parcheado a mano en `build.gradle`.
- ✅ **QA de edge-to-edge de la v4 aprobado en la tablet** — 24 jul 2026; sin regresiones por `targetSdk 36`.
- ✅ **Grupo `testers-huella@huella.lat` arreglado para admitir miembros externos** — 24 jul 2026; sin esto habría rebotado con todos los testers de Gmail.
- ✅ **Flujo de tester probado de punta a punta** (opt-in → Play Store → app funcionando) — 24 jul 2026. Link: `https://play.google.com/apps/testing/lat.huella.app`
- ✅ **v4 (SDK 36) aprobada y publicada en el canal cerrado Alpha** — 24 ago 2026.
- ✅ **Respaldo de `android.keystore` + contraseña en el Drive de Daniel** — 3 jul 2026.
- ✅ **Etapas 1-3 de la TWA**: fuentes self-hosted (`ce7b2ee`, Lighthouse móvil 89), `assetlinks.json` servido como estático (`8933fea`) y `bubblewrap build` exitoso — 2-3 jul 2026.
