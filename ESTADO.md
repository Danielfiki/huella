...
## ⏭️ PRIORIDAD INMEDIATA (próxima sesión) — ESPERAR APROBACIÓN DE LA v3 + CONVOCAR 12 TESTERS (único bloqueante de la beta)

**ESTADO AL 24 JUL 2026 (lo más nuevo, leer primero):** el **QA de edge-to-edge de la v4 en la tablet Samsung quedó ✅ APROBADO** (instalado por sideload desde el Drive tras desinstalar la versión anterior; Home e Historial sin barras que tapen, barra de navegación completa sobre el taskbar, sin recortes ni regresiones por `targetSdk 36`). **El único bloqueante que queda es la aprobación de la v3 por Google.** La **v3 sigue EN REVISIÓN**; la **v4 (targetSdk 36, versionCode 4) está construida, verificada, firmada y QA-aprobada**, pero **NO se sube todavía** (detalle en PENDIENTES → App stores → "NIVEL DE API"). Los dos frentes abiertos, en orden:
...
2. **⛔ NO subir la v4 mientras la v3 esté en revisión.** ⏳ Y ojo: la advertencia de nivel de API **solo se cierra cuando la v4 llegue a PRODUCCIÓN — no basta la prueba cerrada** → el plazo del **31 ago 2026** queda **acoplado** a completar los 14 días con 12 testers. Es la dependencia más importante del calendario ahora mismo.
