...
#### MOTOR DE RASGOS (pieza 1 EN MAIN; piezas 2/3/4 por construir)
...
- **PRÓXIMOS PASOS:**
  - **Pieza 2 (enganche):** llamar `detectarRasgos` **fire-and-forget tras `addEpisodio`, cada 5 momentos** (toca `RegistroPage` + `HuellaContext`). Aplica protocolo anti-desastre: plan aprobado antes de tocar el flujo.
  - **Pieza 3 (guardado):** persistir en la tabla `rasgos` (transformar `evidencia` de ids a jsonb `{ episodio_id, fecha }` + `evidencia_count`; `addRasgo`/`updateRasgo` en `HuellaContext`; resolver el solape muchos-a-muchos).
  - **Pieza 4 (UI del retrato):** renovar `/hijo` con el retrato (cabecera + card de propuesta + 4 familias + nitidez), card de anticipo en el Home, según las specs de la Fase 1.
