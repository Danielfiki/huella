...
1. **Medir la actividad en la ventana 2-4 sep** (día ~7 del reloj), **sobre las 23 personas**, con las queries de métricas cruzadas contra la **LISTA OFICIAL DE TESTERS** de la sección de abajo (esa tabla manda: son **23 personas**, y si un número no cuadra, manda la tabla). Antes del 2 sep el dato no sirve: son pocos días y cae en fin de semana.
...
**Son 23 personas (actualizado el 2 sep 2026).** La cuenta: eran **18** al 11 ago, **− 3 que nunca respondieron** (`be.delacerda`, `snackclub2022`, `nicolas.arriagada` — sus códigos `H-01`, `H-06` y `H-09` se reciclaron el 29 ago tras verificar por query que seguían sin canjear), **+ 5 testers nuevos el 29 ago** = 20, **+ 2 más el 31 ago** (`rlazod` y `sfaille`) = 22, **+ Ceci Caro el 2 sep** = **23**.

*Historia del número, para que no se vuelva a enredar: los 17 del lanzamiento (2 ago) **+ la mamá** (`nene.pepita`, 5 ago) = 18, **− Nicole** (falta de espacio en el teléfono) = 17, **+ Nicolás Arriagada** = 18, y de ahí a las 23 de hoy. En días anteriores del archivo quedó escrito "17" en varios lados porque cada movimiento se anotó por separado y nunca se rehízo la suma.*
...
## Cerrado HOY — martes 1 septiembre 2026 — **El QA en Android tumbó el fix de la mañana, y el de la tarde lo cerró: el bug de la escalera quedó CERRADO con QA en las tres rutas**
...
**Los dos quedaron agregados al grupo de Google el 31 ago.** Los mensajes de bienvenida y los códigos **los envía Daniel hoy**. La lista oficial pasó de **20 a 22 personas**, y la tabla de arriba está actualizada. *(Hoy son **23**: Ceci Caro entró el 2 sep.)*
...
⚠️ **Códigos libres: quedan solo `HUELLA-29` y `HUELLA-30`.** Si aparecen más candidatos, **hay que generar un lote 3** — el SQL del lote 2 sirve de molde (bloques del 29 ago), cambiando el rango y la nota. Recordar el paso de verificación: **SELECT antes del INSERT**.

**Ojo con el colchón, que es el punto de todo esto:** con 23 personas hay **11 de margen** sobre los 12 que exige Google. El requisito es de **12 SIMULTÁNEOS**, así que el margen es contra desinstalaciones, no un acumulado.
...
**🥇 FRENTE 1 — LA VENTANA DE MEDICIÓN ARRANCA EL MIÉRCOLES 2 DE SEPTIEMBRE, y es lo PRIMERO del proyecto: va antes incluso del QA del onboarding.** El reloj de los 14 días corre desde el 27 ago y la postulación apunta a **~10 sep**, así que la medición del **2-4 sep** (día ~7) es la que dice si hay que empujar a alguien antes de postular. Se mide contra la **LISTA OFICIAL DE TESTERS**, que hoy tiene **23 personas**.
