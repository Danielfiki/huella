...
- 🔴 **DEUDA PRIORITARIA — NO EXISTE NINGÚN REGISTRO DE INTENTOS DE PAGO** — desde **4 ago 2026** — ni tabla, ni servicio externo: solo `console.error` en Vercel, con retención de **una hora** en el plan Hobby. **Consecuencia: cada vez que un usuario reporte que no pudo pagar, se va a estar ciego igual que hoy.** Además `mp-crear-suscripcion.js` **ni siquiera loguea el camino exitoso** (solo el error). **Plan acordado, con SQL y diseño ya entregados, pendiente de ejecutar:** (1) reproducir el camino silencioso en celular, (2) crear tabla **`pagos_intentos`** con RLS activa sin policies (patrón `codigos_beta`, migración **009**), (3) instrumentar `mp-crear-suscripcion.js` con helper blindado que nunca pueda romper el pago, (4) mejorar el error visible con código de referencia + botón de reintentar. **Detalle completo en el bloque de sesión del 4 ago.**
...
### 6. Plan acordado — NO EJECUTADO

1. **Reproducir el camino silencioso en celular** (TWA de Android + Safari en iPhone).
2. **Crear la tabla `pagos_intentos`** con **RLS activa sin policies**, patrón `codigos_beta` (migración **009**).
...
**Code ya entregó el SQL de la migración 009 y el diseño completo. Queda pendiente de ejecutar.** El orden importa: primero reproducir en celular, porque si el camino silencioso se confirma es el arreglo de mayor impacto y el más barato de todos.
