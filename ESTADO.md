...
- 🧠 **CEREBRO HUELLA — FASE B EN CURSO, PRIORIZADA POR DANIEL (ago 2026)** — desde **24 ago 2026** — un **cerebro 3D manipulable del hijo**. No es una pantalla suelta: es una pieza que vive en cinco lugares. **(a)** card en el Home, *"En el cerebro de {hijo}"*, como **gancho diario**; **(b)** lugar permanente en la **página del hijo**; **(c)** momento contextual dentro de la **orientación post-episodio**; **(d)** **push mensual**; **(e)** **compartible**.
  ...
  - ✅ **PASO 6 — CERRADO (26 ago 2026). El cerebro por fin tiene puertas.** Dos entradas: el **tercer tab "Su cerebro" en `HijoPage`** (el hogar canónico) y un **enlace permanente al pie de `TarjetaCerebro`** en el Home — *"Conoce el cerebro de {nombre} por dentro →"*, presente en los **cuatro** estados de la tarjeta y **no descartable**. Para lograrlo se extrajo **`CerebroContenido.jsx`**: la pieza sin cáscara de página, que ahora tiene dos casas. `CerebroPage` quedó como cáscara (barra mocha + contenido) y **la ruta `/cerebro` sigue viva porque el paso 8 la necesita**.
...
*Tres reglas que salieron de bugs reales. Las tres tienen la misma forma: eran evitables si alguien hubiera leído lo que el propio repo ya decía.*
