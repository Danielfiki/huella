...
- 🔴🥇 **QA EN ANDROID REAL DEL DICTADO POR VOZ — SEGUNDO INTENTO, LO ÚNICO QUE FALTA PARA CERRAR EL BUG DEL 1 SEP.** Desde **1 sep 2026**. ⚠️ **El primer QA FALLÓ** (ver el bloque de la segunda vuelta): el fix `01ad956` no alcanzaba. **Ahora hay que probar las TRES rutas: (a) el registro conversacional —la pantalla que falló—, (b) un `VoiceTextarea` suelto (el relato del detallado o el avance), y (c) la reflexión del resultado.** Las tres son el mismo componente, pero el QA se hace en las tres igual: eso es justamente lo que no se hizo la primera vez. El fix (`01ad956`) está en producción y verificado con harness (10 OK / 0 fallas), y el episodio basura ya fue borrado, **pero por la regla nueva esto NO se declara cerrado sin QA en Android**. Qué hacer: dictado de **60-90 segundos con pausas** (al menos 8), y la transcripción tiene que quedar **completa y sin repeticiones**. Se mira con USB + `chrome://inspect`, filtrando por `VOZ`. **Criterio exacto:** `largoRelato` **nunca baja**; el largo final es parecido a lo dicho (~6 caracteres por palabra); `reinicioSeguido` **vuelve a 1** cada vez que se habla; y la grabación **dura hasta que Daniel toca stop**. Si la consola dice `Safari corto` en vez de `el motor cerro solo`, es bundle viejo en caché.
...
## Cerrado HOY — martes 1 septiembre 2026 (segunda vuelta) — **El QA en Android falló, y el fix de la mañana resultó ser correcto pero insuficiente**
