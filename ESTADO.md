...
- **PRÓXIMO PASO INMEDIATO (antes de seguir con 4B) — ajuste "uno por sesión":** hoy, al confirmar/descartar un candidato, **aparece el siguiente al tiro en la misma sesión** (se notó porque Pascualito tenía 4 candidatos acumulados de pruebas). El diseño pide **UNO por sesión**: confirmar/descartar uno y que el resto **espere a la próxima visita**, para no sentirse a "examen" (**marco anti-vergüenza**). Es un ajuste chico, pendiente para la próxima sesión.
- **PRÓXIMOS PASOS (resto de la pieza 4):**
  - **4B (el retrato):** renovar la cabecera de `/hijo` con la **foto que madura** + **rastro del escarabajo** + nombre + píldora de nitidez; **ELIMINAR la racha** y **reemplazar las 3 cards** actuales (episodios/logros/avances) por el retrato.
  - **4C (las 4 familias):** mostrar los **rasgos confirmados** agrupados en las **4 familias** (mueve/fortalezas/cuesta/calma en orden fijo), con la nitidez (5 niveles vía filtro CSS, solo crece).
  - **4D (card de anticipo en el Home):** card debajo del CTA Registrar que lleva a `/hijo` (el **gatillo de retorno**).
  - Según las specs de la Fase 1 (`huella-gancho-retencion.md` en el Drive de Daniel). Materia prima ya disponible: `state.rasgos` cargado al abrir la app + los cases `ADD_RASGO`/`UPDATE_RASGO` y `confirmarRasgo`/`descartarRasgo`.
