...
## Cerrado HOY (domingo 21 junio 2026) — Bienvenida del usuario nuevo VERIFICADA en prod + decisiones de la BETA
...
### Verificación legal (privacidad) — OK
...
### Correo oficial (buzón) — EN PROGRESO, pendiente

- **Decisión:** Zoho Mail plan gratuito (Forever Free), dirección `contacto@huella.lat`. Se descartó Google Workspace (~US$7/mes) por costo. Nota: Zoho gratis no tiene IMAP, se revisa por webmail/app aparte.
- **DNS del dominio lo administra Vercel** (nameservers `ns1`/`ns2.vercel-dns.com`). Hallazgo clave: todos los registros de Resend están en el subdominio `send.huella.lat` (SPF, MX de bounce, DKIM); el dominio raíz NO tiene MX ni SPF, así que agregar el correo en la raíz es seguro y no rompe los correos de la app.
- **Estado:** cuenta Zoho creada, pero el alta del dominio quedó trabada porque el navegador autocompleta "www.huella.lat" en el campo y no deja dejar solo "huella.lat".
- **PRÓXIMO PASO:** reintentar en ventana de incógnito (sin autocompletado), agregar el dominio raíz `huella.lat`, verificar el dominio con el registro DNS que entregue Zoho (agregarlo en Vercel), luego agregar los MX/SPF/DKIM de Zoho en la raíz, y crear el buzón `contacto@huella.lat`.

### Deudas anotadas (post-buzón)

- Cuando exista `contacto@huella.lat`: cambiar el contacto del responsable en la Política de Privacidad y en el registrante del dominio (hoy ambos usan el gmail personal `danielundurraga.r@gmail.com`), y llenar el placeholder de correo oficial en los términos de beta.
