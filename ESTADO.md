...
- **PRÓXIMO PASO — FASE B del pendiente obligatorio:** **afinar el `PROMPT_DETECTAR_RASGOS` para equilibrar las 4 familias** con datos reales de la beta (calibrar para que no se sesgue, ej. demasiadas `fortalezas` vs. `mueve`/`calma`). Después siguen las piezas 4B/4C/4D del retrato.
- **PRÓXIMOS PASOS (resto de la pieza 4, después de la Fase B):**
