...
## Cerrado HOY (lunes 15 junio 2026) — Reset de contraseña (Resend SMTP) + álbum descartado como bug + onboarding (2 arreglos) + logo de marca en el header + auditoría del flujo de entrada
...
### Logo de marca — etapa 1 de 3: instalado en el header (commit `6252841`)

**Empezamos a reemplazar el wordmark de texto "huella" por el logo real de la marca. Etapa 1 (header) lista y verificada; faltan etapas 2 y 3.**
...
- **PENDIENTE etapa 2 — resto de pantallas:** el wordmark del Home (`Hero.jsx`, va sobre mocha → crema; **OJO:** posible **redundancia de doble escarabajo** cerca del header) y las pantallas públicas (Login, Signup, Reset, Términos, Invitar → fondo claro, el logo se pinta **terracota** reutilizando el mismo componente que hereda color).
- **PENDIENTE etapa 3 — íconos:** favicon e ícono de app con `logo_2` (solo el escarabajo, sin wordmark).
