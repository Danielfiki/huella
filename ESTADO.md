...
- 🧠 **CEREBRO HUELLA — FASE B EN CURSO, PRIORIZADA POR DANIEL (ago 2026)** — desde **24 ago 2026** — un **cerebro 3D manipulable del hijo**. No es una pantalla suelta: es una pieza que vive en cinco lugares. **(a)** card en el Home, *"En el cerebro de {hijo}"*, como **gancho diario**; **(b)** lugar permanente en la **página del hijo**; **(c)** momento contextual dentro de la **orientación post-episodio**; **(d)** **push mensual**; **(e)** **compartible**.
  ...
  - ⏭️ **LO SIGUIENTE EN ESTE FRENTE — PASO 5: la edad real del hijo.** Hoy el slider arranca fijo en 4 años (`EDAD_INICIAL` en `CerebroPage.jsx`). El paso 5 lo hace partir en la **edad decimal real del hijo activo**, con una **función nueva al lado de `calcularEdad`** (`src/context/HuellaContext.jsx:12`) — **`calcularEdad` NO se toca**, porque la usa media app y devuelve años enteros. La curva del cerebro sí acepta decimales: `interp` interpola entre los puntos de la curva, así que un hijo de 4,7 años ya tiene un estado propio distinto al de 4,0. Después: **paso 6** (tercer tab en `HijoPage`) · **paso 7** (QA en la tablet y decisión **GLB v1 vs v2**).
...
- ⏭️ **Lo siguiente en el frente Cerebro es el PASO 5:** que el slider arranque en la **edad decimal real del hijo** en vez del 4 fijo, con una **función nueva al lado de `calcularEdad`, sin tocarla**.
