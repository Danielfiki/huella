...
export default function DrawerPasados({ planes }) {
    ...
    <div className={styles.box}>
      ...
      {open && (
        <ul className={styles.list}>
          {planes.map((p) => {
            const estado = estadoPlan(p); // 'completado' | 'abandonado'
            return (
              <li key={p.id} className={styles.item}>
                ...
                <span className={styles.nm}>
                  {p.habilidad_nombre} · {estado === 'completado' ? `${p.total_semanas} sem` : `abandonado en sem ${p.semana_actual}`}
                </span>
