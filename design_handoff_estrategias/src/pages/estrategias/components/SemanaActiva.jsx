...
export default function SemanaActiva({ semana, numero, total, reflexion, onReflexionChange, onAvanzar }) {
  const completas = semana.tareas.filter((t) => t.completada).length;
  const puedeAvanzar = completas >= 2; // mínimo 2 tareas chequeadas
  const esUltima = numero === total;

  const toggleTarea = (id) => {
    // delegar al contexto/dispatch en el padre si se quiere persistir
    semana.tareas = semana.tareas.map((t) => t.id === id ? { ...t, completada: !t.completada } : t);
  };
